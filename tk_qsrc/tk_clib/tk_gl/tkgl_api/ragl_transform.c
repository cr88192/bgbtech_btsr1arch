...
//	clip[4*4+0]=0;	clip[4*4+1]=0;	clip[4*4+2]=-1;	clip[4*4+3]=0;
	...
	RASWGL_BuildInsertPrimitiveArrays(
		...
		GL_QUADS, 0, nxyz,
		4, ctx->trans0_xyzbuf,
		stsize, ctx->trans0_stbuf,
		normsize, ctx->trans0_nvbuf,
		rgbsize, ctx->trans0_clrbuf);
#endif
...
//	RASWGL_CheckExpandTransform1Vertices(ctx, nxyz*2);
	...
	for(j=0; j<(nxyz-2); j++)
	{
		...
		if(ctx->caps_enabled&BGBRASW_ENABLE_LIGHTING)
		{
			Vec3F_Copy( nv+(base+j+i0)* nvsz,
				ctx->transN_nvbuf+(nt*3+0)* nvsz);
			Vec3F_Copy( nv+(base+j+i1)* nvsz,
				ctx->transN_nvbuf+(nt*3+1)* nvsz);
			Vec3F_Copy( nv+(base+j+i2)* nvsz,
				ctx->transN_nvbuf+(nt*3+2)* nvsz);
		}
