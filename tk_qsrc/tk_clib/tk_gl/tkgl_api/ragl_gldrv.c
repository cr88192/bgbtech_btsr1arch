...
{
	RASWGL_ClipPlane(GLCTX, plane, equation);
//	RaGlSetErrorNopStub();
}
...
BGBRASW_API void GLDRV_APIENTRY GlDrv_glBeginQuery(
	GLenum cap, GLuint id)
{
	RASWGL_BeginQuery(GLCTX, cap, id);
}

BGBRASW_API void GLDRV_APIENTRY GlDrv_glEndQuery(
	GLenum cap)
{
	RASWGL_EndQuery(GLCTX, cap);
}

BGBRASW_API void GLDRV_APIENTRY GlDrv_glGetQueryObjectuiv(
	GLuint id, GLenum cap, GLuint *rptr)
{
	RASWGL_GetQueryObjectIV(GLCTX, id, cap, rptr);
}
		...
		GLint x, GLint y, GLsizei width, GLsizei height)
{
	...
	if(x0<0)x0=0;
	if(y0<0)y0=0;
	if(x0>GLCTX->ractx->tgt_xs)x0=0;
	if(y0>GLCTX->ractx->tgt_ys)y0=0;
	if(x1<0)x1=0;
	if(y1<0)y1=0;
	if(x1>GLCTX->ractx->tgt_xs)x1=0;
	if(y1>GLCTX->ractx->tgt_ys)y1=0;
...
BGBRASW_API void GLDRV_APIENTRY GlDrv_glNormal3bv(const GLbyte *v)
	{ RASWGL_Normal3sbv(GLCTX, (signed char *)v); }
...
BGBRASW_API void GLDRV_APIENTRY GlDrv_glNormal3fv(const GLfloat *v)
	{ RASWGL_Normal3fv(GLCTX, (float *)v); }
...
BGBRASW_API void GLDRV_APIENTRY GlDrv_glTexCoord2fv(const GLfloat *v)
	{ RASWGL_TexCoord2fv(GLCTX, (float *)v); }
