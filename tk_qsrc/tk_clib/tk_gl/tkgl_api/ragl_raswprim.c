	...
	RASWGL_Context *ctx, BGBRASW_Primitive *prim)
{
	if(!prim->tex)
		return(0);
	if(!prim->tex->rgba[0] && !prim->tex->cmptx[0])
		return(0);
	return(1);
}
	...
	RASWGL_Context *ctx, BGBRASW_Primitive *prim)
{
	...
	{
		i=(ctx->ref_clr>>24)&255;

		if(((ctx->alpha_func!=GL_GREATER) &&
			(ctx->alpha_func!=GL_GEQUAL)) ||
			((i<51) || (i>205)))
		{
...
#if 1
int RASWGL_ClasifyPrimitiveMode(RASWGL_Context *ctx,
	BGBRASW_Primitive *prim)
{
	int mode;

	mode=BGBRASW_PMODE_NONE;
	...
	if(RASWGL_ClasifyPrimitive_NeedsTabsP(ctx, prim))
	{
		if((ctx->caps_enabled&BGBRASW_ENABLE_TEXTURE_2D) &&
			RASWGL_ClasifyPrimitive_TexValidP(ctx, prim))
		{
			mode=BGBRASW_PMODE_TEXINTERPTABS;
		}else
		{
			mode=BGBRASW_PMODE_FLATINTERPTABS;
		}
	...
	if(ctx->caps_enabled&BGBRASW_ENABLE_DEPTH_TEST)
	{
		...
		{
			if(RASWGL_ClasifyPrimitive_BasicOpaqueP(ctx, prim))
				{ mode=BGBRASW_PMODE_TEXBASICZTEST; }
			else
				{ mode=BGBRASW_PMODE_TEXINTERPZTEST; }
		}else
		{
			if(RASWGL_ClasifyPrimitive_FlatColorP(ctx, prim))
				{ mode=BGBRASW_PMODE_FLATBASICZTEST; }
			else
				{ mode=BGBRASW_PMODE_FLATINTERPZTEST; }
		}
	}else
	{
		if((ctx->caps_enabled&BGBRASW_ENABLE_TEXTURE_2D) &&
		...
		{
			if(RASWGL_ClasifyPrimitive_BasicOpaqueP(ctx, prim))
				{ mode=BGBRASW_PMODE_TEXBASIC; }
			else
				{ mode=BGBRASW_PMODE_TEXINTERP; }
		}else
		{
			if(RASWGL_ClasifyPrimitive_FlatColorP(ctx, prim))
				{ mode=BGBRASW_PMODE_FLATBASIC; }
			else
				{ mode=BGBRASW_PMODE_FLATINTERP; }
		}
	}
...
void RASWGL_ClasifyPrimitive(
	RASWGL_Context *ctx, BGBRASW_Primitive *prim)
{
	prim->mode=RASWGL_ClasifyPrimitiveMode(ctx, prim);
}
	...
	BGBRASW_Primitive *ptmp)
{
	...
	if(	(ptmp->bbox[1][0]<=(ctx->view_x0<<8)) ||
		(ptmp->bbox[1][1]<=(ctx->view_y0<<8)) ||
		(ptmp->bbox[0][0]>=(ctx->view_x1<<8)) ||
		(ptmp->bbox[0][1]>=(ctx->view_y1<<8)) ||
		(ptmp->bbox[0][2]>=(1U<<31)))
	{
...
//	pmode=RASWGL_ClasifyPrimitiveMode(ctx);
	pmode=-1;
...
//			if(pmode<0)
//				pmode=RASWGL_ClasifyPrimitiveMode(ctx, ptmp);
//			ptmp->mode=pmode;
...
//				rgb+(base+i*2+1)*rgbsize, ptmp->rgba+1);
	...
	}else if(prim==GL_TRIANGLES)
	{
		...
		for(i=0; i<n; i++)
		{
			...
			{
				V2F_SUB(xyz+(base+i*3+1)*xyzsize,
					xyz+(base+i*3+0)*xyzsize, dv0);
				V2F_SUB(xyz+(base+i*3+2)*xyzsize,
					xyz+(base+i*3+1)*xyzsize, dv1);
				f=V2F_CROSS(dv0, dv1);
				if(f>0)continue;
			}
...
//				rgb+(base+i*3+2)*rgbsize, ptmp->rgba+2);
			...
			{
//				BGBRASW_FreePrimitive(ctx->ractx, ptmp);
				BGBRASW_FreePrimitive(ptmp->ctx, ptmp);
				continue;
			}
	...
	}else if(prim==GL_QUADS)
	{
		...
		for(i=0; i<n; i++)
		{
			if(ctx->caps_enabled&BGBRASW_ENABLE_CULL_FACE)
			{
				V2F_SUB(xyz+(base+i*4+1)*xyzsize,
					xyz+(base+i*4+0)*xyzsize, dv0);
				V2F_SUB(xyz+(base+i*4+2)*xyzsize,
					xyz+(base+i*4+1)*xyzsize, dv1);
				f=V2F_CROSS(dv0, dv1);
				if(f>0)continue;
			}
