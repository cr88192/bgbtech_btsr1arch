...
float *VecNF_AllocVec(int sz)
{
	...
	if(!vecnf_vecpool)
	{
		vecnf_vecpool=malloc((1<<9)*sizeof(float));
		vecnf_vecpos=0;
	}

	if((vecnf_vecpos+sz)>=(1<<9))
		vecnf_vecpos=0;
...
double *VecND_AllocVec(int sz)
{
	...
	if(!vecnd_vecpool)
	{
		vecnd_vecpool=malloc((1<<8)*sizeof(double));
		vecnd_vecpos=0;
	}

	if((vecnd_vecpos+sz)>=(1<<8))
		vecnd_vecpos=0;
...
/* misc */
float Scaler_Random() /* random number, 0 to 1 */
{
	float tmp;
...
float Scaler_SignRandom() /* -1 to 1 */
{
	float tmp;
