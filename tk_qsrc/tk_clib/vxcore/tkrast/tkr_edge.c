...
__asm {
TKRA_ScaleClrStepRcp:
	PSHUF.W		R11, 0, R6		//(0000)
	PMULU.HW	R10, R6, R10
	RTS

TKRA_CalcClrStepRcp:
	MOV			0xFFFEFFFEFFFEFFFE, R13
	AND			R10, R13, R10
	AND			R11, R13, R11
	SHLD.Q		R10, -1, R10
	SHLD.Q		R11, -1, R11
	SUB			R10, R11, R10
	PSHUF.W		R12, 0, R17		//(0000)
	PMULS.HW	R10, R17, R10
	SHLD.Q		R10, 1, R10
	RTSU
};
#endif
			...
			TKRA_TRFL_ATEST))
		{
			...
			if(sctx->tex_cur->tex_img_bcn)
			{
				if(trifl&TKRA_TRFL_MINBL)
				{
					...
					{
						sctx->DrawSpan_Min=
							TKRA_DrawSpanB_ZatModBlUtx3MortClampZt;
						sctx->DrawSpanZt_Min=
							TKRA_DrawSpanB_ZatModBlUtx3MortClampZt;
					}else
					{
				...
				}else
				{
					...
					{
						sctx->DrawSpan_Min=
							TKRA_DrawSpanB_ZatModUtx3MortClampZt;
						sctx->DrawSpanZt_Min=
							TKRA_DrawSpanB_ZatModUtx3MortClampZt;
					}else
					{
		...
		if(trifl&TKRA_TRFL_DOBLEND)
	{
		...
		if(sctx->tex_cur->tex_img_bcn)
		{
			if(trifl&TKRA_TRFL_MINBL)
			{
				...
				{
					sctx->DrawSpan_Min=
						TKRA_DrawSpan_BlendModBlUtx2MortClamp;
					sctx->DrawSpanZt_Min=
						TKRA_DrawSpan_BlendModBlUtx2MortClampZt;
				}else
				{
					sctx->DrawSpan_Min=
