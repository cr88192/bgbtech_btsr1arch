...
Many will be provided in both 8 and 16 bit variants.
 */




const u64 tkgdi_imgutil_glyphs[256]=
{
...
u64 tkgdi_imgutil_densehex[256];


u16 tkgdi_imgutil_rcptab[1024];

void TKGDI_ImgUtil_InitRcp()
{
	int i, j, k;

	if(tkgdi_imgutil_rcptab[1])
		return;

	for(i=2; i<1024; i++)
	{
		j=0x10000/i;
		tkgdi_imgutil_rcptab[i]=j;
	}
	tkgdi_imgutil_rcptab[0]=0xFFFF;
	tkgdi_imgutil_rcptab[1]=0xFFFF;
}

int TKGDI_ImgUtil_DrawLine16(u16 *ibuf, int xs, int ys,
	int x0, int y0, int x1, int y1, u16 clr)
{
	int tstep, xstp, ystp, xcur, ycur, x, y, xd, yd, rcp;
	int i, j, k;

	TKGDI_ImgUtil_InitRcp();
	...
	rcp=tkgdi_imgutil_rcptab[tstep];
	xstp=xd*rcp;
	ystp=yd*rcp;
	...
	for(i=0; i<tstep; i++)
	{
		x=(xcur+0x7FFF)>>16;	y=(ycur+0x7FFF)>>16;
		xcur+=xstp;				ycur+=ystp;
		if((x<0) || (x>=xs))	continue;
		if((y<0) || (y>=ys))	continue;
		ibuf[y*xs+x]=clr;
	}
	return(0);
}

int TKGDI_ImgUtil_DrawLine8(byte *ibuf, int xs, int ys,
	int x0, int y0, int x1, int y1, byte clr)
{
	int tstep, xstp, ystp, xcur, ycur, x, y, z, xd, yd, rcp;
	int xsn1, ysn1;
	int i, j, k;
	...
	if(	(x0>1) && (x0<xsn1) && (y0>1) && (y0<ysn1) &&
		(x1>1) && (x1<xsn1) && (y1>1) && (y1<ysn1) )
	{
		for(i=0; i<tstep; i++)
		{
			x=(xcur+0x7FFF)>>16;	y=(ycur+0x7FFF)>>16;
...
void TKGDI_ImgUtil_DrawClear16(u16 *ibuf, int xs, int ys, u16 clr)
{
	int i, n;
	n=xs*ys;
	for(i=0; i<n; i++)
...
void TKGDI_ImgUtil_DrawClear8(byte *ibuf, int xs, int ys, byte clr)
{
	int i, n;
	n=xs*ys;
	for(i=0; i<n; i++)
...
void TKGDI_ImgUtil_FillRect16(u16 *ibuf, int xs, int ys,
	int x0, int y0, int x1, int y1, u16 clr)
{
	int x, y, z, n;
...
void TKGDI_ImgUtil_FillRect8(byte *ibuf, int xs, int ys,
	int x0, int y0, int x1, int y1, byte clr)
{
	int x, y, z, n;
...
void TKGDI_ImgUtil_FillFlood16(u16 *ibuf, int xs, int ys,
	int x0a, int y0a, u16 setclr, u16 refclr)
{
	u16 stack_x[256];
	u16 stack_y[256];
	int x, y, z, z1, x0, x1, c, n, stkpos;
...
void TKGDI_ImgUtil_FillFlood8(byte *ibuf, int xs, int ys,
	int x0a, int y0a, byte setclr, byte refclr)
{
	u16 stack_x[256];
	u16 stack_y[256];
	int x, y, z, z1, x0, x1, c, n, stkpos;
...
/*
Color-Fill for a bit-masked pattern.
Mask is Raster-Ordered 1bpp in LSB-first order.
 */

void TKGDI_ImgUtil_FillMasked8(
	byte *ibuf, int ixs, int iys,
	byte *mbuf, int mxs, int mys,
	int ix0, int iy0,
	int mx0, int my0,
	int cxs, int cys,
	byte clr)
{
	...
	if(ix0<0)	{ x=-ix0; ix0+=x; mx0+=x; cxs-=x; }
	if(mx0<0)	{ x=-mx0; ix0+=x; mx0+=x; cxs-=x; }
	if(iy0<0)	{ y=-iy0; iy0+=y; my0+=y; cys-=y; }
	if(my0<0)	{ y=-my0; iy0+=y; my0+=y; cys-=y; }
	if(cxs<=0)	return;
	if(cys<=0)	return;
	
	if((ix0+cxs)>ixs)	cxs=ixs-ix0;
	if((mx0+cxs)>mxs)	cxs=mxs-mx0;
	if((iy0+cys)>iys)	cys=iys-iy0;
	if((my0+cys)>mys)	cys=mys-my0;
	if(cxs<=0)	return;
	if(cys<=0)	return;
...
/* Draw an outline of a rectangle with 2 colors.
 * clr0: Used for lines conected to (x0,y0)
 * clr1: Used for lines conected to (x1,y1)
 */
void TKGDI_ImgUtil_DrawOutlineRect8(byte *ibuf, int xs, int ys,
	int x0, int y0, int x1, int y1, byte clr0, byte clr1)
{
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, x0, y0, x1, y0, clr0);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, x0, y0, x0, y1, clr0);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, x1, y0, x1, y1, clr1);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, x0, y1, x1, y1, clr1);
}

/* Draw an outline of an octagon with 4 colors.
...
void TKGDI_ImgUtil_DrawOutlineOct8(byte *ibuf, int xs, int ys,
	int xo, int yo, int rad,
	byte clr0, byte clr1, byte clr2, byte clr3)
{
	int ra1;
	ra1=(rad*181)>>8;

	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo-rad, yo    , xo-ra1, yo+ra1, clr0);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo-ra1, yo+ra1, xo    , yo+rad, clr0);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo+rad, yo    , xo+ra1, yo+ra1, clr1);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo+ra1, yo+ra1, xo    , yo+rad, clr1);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo-rad, yo    , xo-ra1, yo-ra1, clr2);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo-ra1, yo-ra1, xo    , yo-rad, clr2);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo+rad, yo    , xo+ra1, yo-ra1, clr3);
	TKGDI_ImgUtil_DrawLine8(ibuf, xs, ys, xo+ra1, yo-ra1, xo    , yo-rad, clr3);
}

int TKGDI_ImgUtil_DrawGlyph8(byte *ibuf, int xs, int ys,
	int xb, int yb, u64 pix, byte clr)
{
	int x, y, z, px, py, pz;
	int i, j, k;

	if((xb>=0) && (yb>=0) && ((xb+7)<xs) && ((yb+7)<ys))
	{
		for(py=0; py<8; py++)
		{
			z=(yb+py)*xs+xb;
...
int TKGDI_ImgUtil_DrawTextChar8(byte *ibuf, int xs, int ys,
	int xb, int yb, byte ch, byte clr)
{
	u64 px;
	int x, y;

	x=xb; y=yb;
	px=tkgdi_imgutil_glyphs[ch];
	TKGDI_ImgUtil_DrawGlyph8(ibuf, xs, ys, x, y, px, clr);
	return(0);
}

int TKGDI_ImgUtil_DrawTextString8(byte *ibuf, int xs, int ys,
	int xb, int yb, char *str, byte clr)
{
	u64 px;
	char *cs;
	int x, y;
...
void TKGDI_ImgUtil_InitDenseHex()
{
	static const u32 densedigs[16]=
	{
	...
	if(!tkgdi_imgutil_densehex[0])
	{
		for(i=0; i<256; i++)
		{
			...
			tkgdi_imgutil_densehex[i]=pix;
		}
	}
}

int TKGDI_ImgUtil_DrawDenseHexDigit8(byte *ibuf, int xs, int ys,
	int xb, int yb, byte dig, byte clr)
{
	u64 pix;

	TKGDI_ImgUtil_InitDenseHex();

	pix=tkgdi_imgutil_densehex[dig];
	TKGDI_ImgUtil_DrawGlyph8(ibuf, xs, ys, xb, yb, pix, clr);
	return(0);
}
