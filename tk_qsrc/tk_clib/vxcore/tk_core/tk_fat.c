	...
	int lba, int num)
{
	...
	if(!img)
	{
		tk_printf("TKFAT_GetSectorTempBuffer: NULL Image\n");
		__debugbreak();
	}
...
//	tk_puts("TKFAT_GetSectorTempBuffer: A1\n");
	...
	if(lba<0)
	{
		tk_printf("TKFAT_GetSectorTempBuffer: Negative LBA\n");
		__debugbreak();
	}
...
//	TKSPI_ReadSectors(tbd, lba, n);
	TKBDEV_ReadSectors(img->bdev, tbd, lba, n);
...
//	TKSPI_ReadSectors(img->sbc_buf[i], lba2, num&255);
	TKBDEV_ReadSectors(img->bdev, img->sbc_buf[i], lba2, num&255);
...
int TKFAT_FreeSectorStaticBuffer(TKFAT_ImageInfo *img, int lba)
{
	...
	if(!img)
	{
//		tk_printf("TKFAT_FreeSectorStaticBuffer: NULL Image\n");
		__debugbreak();
	}
...
//			img->tfbc_buf[i]=tk_malloc_krn(n*512+256);
	...
	tk_dbg_printf("TKFAT_GetSectorTempFatBuffer: Read Sectors, %d %d\n",
		(int)lba, (n&255));
//	TKSPI_ReadSectors(tbd, lba, n);
	TKBDEV_ReadSectors(img->bdev, tbd, lba, n);
...
void TKFAT_SyncSectorTempBuffer(TKFAT_ImageInfo *img)
{
	...
	for(i=0; i<img->tbc_num; i++)
	{
		...
		if(img->tbc_lbn[i]&TKFAT_SFL_DIRTY)
		{
			tk_dbg_printf("TKFAT_SyncSectorTempBuffer: Write Sectors, %d %d\n",
...
{
	tk_dbg_printf("TKFAT_SyncSectorBuffers\n");
...
void TKFAT_ReadImageMBR(TKFAT_ImageInfo *img)
{
	char *s0;
	int lba, lbn, fsty;
	int lba_fat, lbn_fat, fsty_fat;
	int lba_dfs, lbn_dfs, fsty_dfs;
	int i;

	tk_dbg_printf("TKFAT_ReadImageMBR\n");

	if(sizeof(TKFAT_MBR)!=512)
	{
		tk_printf("TKFAT_ReadImageMBR: MBR Sizeof Sanity Fail %d\n",
			sizeof(TKFAT_MBR));
		__debugbreak();
	}
...
//	img->lba_count=tkfat_getDWord(img->mbr->entry[0].lba_count);
	...
	img->fsty=fsty_fat;
	img->lba_start=lba_fat;
	img->lba_count=lbn_fat;

	img->lba_dfs_start=lba_dfs;
	img->lba_dfs_count=lbn_dfs;

	img->isfat16=(img->fsty==0x06) || (img->fsty==0x0E);

	s0=tkfat_fstnameforfsty(img->fsty);
	tk_dbg_printf("TKFAT_ReadImageMBR: %08X %08X %02X %s\n",
	...
	int clid, int val)
{
	...
	tk_dbg_printf("TKFAT_SetFatEntry: cl=%d val=%d\n", clid, val);
...
void TKFAT_ReadImageFAT(TKFAT_ImageInfo *img)
{
	...
	tk_dbg_printf("TKFAT_ReadImageFAT: A\n");
	TKFAT_ValidateImageMagic(img);
	...
	tk_dbg_printf("TKFAT_ReadImageFAT: B\n");
	TKFAT_ValidateImageMagic(img);
...
//	n=(img->lba_count-img->lba_data)/img->szclust;
	...
	tk_dbg_printf("TKFAT_AllocFreeCluster: tot=%d\n", n);
...
//	char tsn[16];	//temp shortname
	...
	if(!img->dee_cache)
	{
		img->dee_cache=tk_malloc_krn(64*sizeof(TKFAT_FAT_DirEntExt));
		memset(img->dee_cache, 0, 64*sizeof(TKFAT_FAT_DirEntExt));
	}
...
//	tk_printf("TKFAT_WalkDirEntNext: A\n");
	...
	hi=(((clid*65521+sidx)*65521)>>16)&63;
	deeci=img->dee_cache+hi;
	
	if((deeci->clid==clid) && (deeci->sidx==sidx))
	{
		memcpy(dee, deeci, sizeof(TKFAT_FAT_DirEntExt));
		return(dee->idx);
	}
...
//			tk_printf("TKFAT_WalkDirEntNext: C\n");
			...
			memcpy(deeci, dee, sizeof(TKFAT_FAT_DirEntExt));
	...
	TKFAT_FAT_DirEntExt *dee)
{
	...
	if(dee->img->dee_cache)
	{
		clid=dee->clid;
//		sidx=(dee->idx>=0)?(dee->idx+1):0;
		sidx=dee->idx;
		hi=(((clid*65521+sidx)*65521)>>16)&63;
		deeci=dee->img->dee_cache+hi;

		memset(deeci, 0, sizeof(TKFAT_FAT_DirEntExt));
//		memset(dee->img->dee_cache, 0, 64*sizeof(TKFAT_FAT_DirEntExt));
	}
...
TKFAT_ImageInfo *TKFAT_CreateSdFatContext()
{
	...
	tk_dbg_printf("TKFAT_CreateSdFatContext:\n");
...
//	img=tk_malloc_krn(sizeof(TKFAT_ImageInfo));
	...
	img->bdev=tkspi_id_bdev;
	
	TKFAT_ReadImageMBR(img);
	TKFAT_ReadImageFAT(img);
	
	tk_dbg_printf("TKFAT_CreateSdFatContext: OK\n");
	
	return(img);
}

TKFAT_ImageInfo *TKFAT_TryOpenImage(int devid, s64 lba)
{
	TKFAT_ImageInfo *img;
...
TKFAT_ImageInfo *TKFAT_InitializeNewImage(int devid, s64 lba, s64 lbn)
{
	TKFAT_ImageInfo *img;

	tk_dbg_printf("TKFAT_InitializeNewImage:\n");
...
//	TKFAT_ReadImageFAT(img);
	...
	TKFAT_SyncSectorBuffers(img);
	
	tk_dbg_printf("TKFAT_InitializeNewImage: OK\n");
	
	return(img);
}

int TKFAT_GetLvmDeviceId(TKFAT_ImageInfo *img, char *path)
{
	TKFAT_FAT_DirEntExt tdee;
	...
	i=TKFAT_LookupDirEntPath(img, dee, path);
	if(i>=0)
	{
		...
		hsdev=TKBDEV_CreateSubDeviceId(img->bdev, lba, bsz);
		return(hsdev);
	}
	return(-1);
}
