...
byte	st_do3dglasses;		//BGB: 3D glasses
...
// 3D cheat
unsigned char	cheat_3d_seq[] =
{
    0xb2, 0x26, 0x26, 0x26, 0x26, 0xff	// idddd
}; 
...
cheatseq_t	cheat_3d = { cheat_3d_seq, 0 };
...
ST_Responder (event_t* ev)
{
	...
	else if (ev->type == ev_keydown)
	{
		...
		if ( cht_CheckCheat(&cheat_3d, ev->data1) )
		{
//			st_do3dglasses = !st_do3dglasses;
			st_do3dglasses = (st_do3dglasses+1)&3;
			plyr->message = "3D glasses";
			printf("3D glasses %d\n", st_do3dglasses);
			st_firsttime = 1;
		}
		...
		if (!netgame)
		{
			...
			else if (cht_CheckCheat(&cheat_mypos, ev->data1))
			{
				...
				sprintf(buf, "ang=0x%8x;x,y=(0x%8x,0x%8x)",
...
void ST_loadGraphics(void)
{
    ...
    if ( W_CheckNumForName("STTNUM0") > 0 )
    {
...
{
#if 0  //BGB: Debug
	cheat_mus.sequence = cheat_mus_seq;
	cheat_god.sequence = cheat_god_seq;
	cheat_ammo.sequence = cheat_ammo_seq;
	cheat_ammonokey.sequence = cheat_ammonokey_seq;
	cheat_noclip.sequence = cheat_noclip_seq;
	cheat_commercial_noclip.sequence = cheat_commercial_noclip_seq;
#endif
...
void ST_Init (void)
{
    veryfirsttime = 0;
    ST_loadData();
    screens[4] = (dt_scrpix *) Z_Malloc(
		ST_WIDTH*ST_HEIGHT*sizeof(dt_scrpix), PU_STATIC, 0);
}
