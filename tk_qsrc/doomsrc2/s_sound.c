...
#ifdef SAWDEBUG
{
	...
	{
		if (s_channels[i].sfxinfo == &S_sfx[sfx_sawidl]
		|| s_channels[i].sfxinfo == &S_sfx[sfx_sawful]
		|| s_channels[i].sfxinfo == &S_sfx[sfx_sawhit]) n++;
	}
	...
	if (n>1)
	{
		...
		if (s_channels[i].sfxinfo == &S_sfx[sfx_sawidl]
			|| s_channels[i].sfxinfo == &S_sfx[sfx_sawful]
			|| s_channels[i].sfxinfo == &S_sfx[sfx_sawhit])
		{
			fprintf(stderr,
				"chn: sfxinfo=0x%lx, origin=0x%lx, "
				"handle=%d\n",
				s_channels[i].sfxinfo,
				s_channels[i].origin,
				s_channels[i].handle);
		}
		}
