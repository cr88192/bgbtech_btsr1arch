...
.ifarch has_xgpr
	...
	PMULX.F	R18, R4, R36
	PMULX.F	R18, R6, R38
	PADD.F	R32, R33, R48	|	PADD.F	R34, R35, R49
	PADD.F	R36, R37, R50	|	PADD.F	R38, R39, R51
	MOVLLD	R49, R48, R52	|	MOVHHD	R49, R48, R53
	MOVLLD	R51, R50, R54	|	MOVHHD	R51, R50, R55
	PADD.F	R52, R53, R16	|	PADD.F	R54, R55, R17
	MOV.X	R16, (R8, 0)
	...
	tkra_projvertex pv2)
{
	...
	tkra_projvertex pv3, pv4, pv5, pv6;
...
#if 0
//		if(((dx0>>16)>64) || ((dx1>>16)>64) ||
//			((dy0>>16)>64) || ((dy1>>16)>64))
		if(((dx0>>16)>32) || ((dx1>>16)>32) ||
			((dy0>>16)>32) || ((dy1>>16)>32))
		{
			TKRA_TransProjVertexMidpointPersp(ctx, &pv0, &pv1, &pv3);
			TKRA_TransProjVertexMidpointPersp(ctx, &pv1, &pv2, &pv4);
			TKRA_TransProjVertexMidpointPersp(ctx, &pv2, &pv0, &pv5);
			
			TKRA_EmitProjectedTriangle(ctx, pv0, pv3, pv5);
			TKRA_EmitProjectedTriangle(ctx, pv3, pv1, pv4);
			TKRA_EmitProjectedTriangle(ctx, pv4, pv2, pv5);
			TKRA_EmitProjectedTriangle(ctx, pv3, pv4, pv5);
			return;
		}
#endif
...
//	if(0)
	if(ctx->tex_cur2)
	{
		...
		if(ctx->tex_flag2&TKRA_TRFL_FLIPST)
			flipst2=1;

		if((mip2>0) && (ctx->tex_flag&TKRA_TRFL_HASMIP))
		{
			...
			ctx->tex_img2=img->tex_img+img->tex_mipofs[mip];
			ctx->tex_img_bcn2=img->tex_img_bcn+img->tex_mipofs_bcn[mip];
			ctx->tex_xshl2=shx;
			ctx->tex_yshl2=shy;
			...
			if(ctx->tex_img_bcn2)
			{
				tl0=(u64)(ctx->tex_img_bcn2);
				tx0=((shx+shy)<<4)|0;
				tl1=((u64)tx0)<<48;
				tl0=(tl0&0x0000FFFFFFFFFFFFULL)|tl1;
				ctx->tex_img_bcn2=(void *)tl0;
			}
#endif
		}else
		{
			...
			shx=img->tex_xshl;
			shy=img->tex_yshl;
			ctx->tex_img2=img->tex_img;
			ctx->tex_img_bcn2=img->tex_img_bcn;
			ctx->tex_xshl2=shx;
			ctx->tex_yshl2=shy;
	...
	tkra_projvertex pv3)
{
	...
	tkra_projvertex pv4, pv5, pv6, pv7, pv8;
	...
	if(1)
	{
		...
		if(sdi0||sdi1||sdi2||sdi3)
		{
			...
			if(sdi0||sdi2)
			{
				if(sdi1||sdi3)
				{
					TKRA_EmitProjectedQuad(ctx, pv0, pv4, pv8, pv7);
					TKRA_EmitProjectedQuad(ctx, pv4, pv1, pv5, pv8);
					TKRA_EmitProjectedQuad(ctx, pv5, pv2, pv6, pv8);
					TKRA_EmitProjectedQuad(ctx, pv6, pv3, pv7, pv8);
					return;
				}else
				{
					TKRA_EmitProjectedQuad(ctx, pv0, pv4, pv6, pv3);
					TKRA_EmitProjectedQuad(ctx, pv4, pv1, pv2, pv6);
					return;
				}
			}else
			{
				TKRA_EmitProjectedQuad(ctx, pv0, pv1, pv5, pv7);
				TKRA_EmitProjectedQuad(ctx, pv5, pv2, pv3, pv7);
				return;
			}
		}
#endif
...
TKRA_PrimSubDiv *TKRA_AllocPrimSubDiv(TKRA_Context *ctx)
{
	...
	ptmp=ctx->subdiv_free;
	if(ptmp)
	{
		ctx->subdiv_free=ptmp->next;
		ptmp->next=NULL;
		return(ptmp);
	}
	
//	ptmp=tkra_malloc(sizeof(TKRA_PrimSubDiv));
	ptmp=tkra_malloc(64*sizeof(TKRA_PrimSubDiv));
	for(i=0; i<63; i++)
	{
		ptmp->next=ctx->subdiv_free;
		ctx->subdiv_free=ptmp;
		ptmp++;
	}
...
{
	pcur->next=ctx->subdiv_free;
	ctx->subdiv_free=pcur;
}
