...
//		Sys_Error ("Draw_TransPic: bad coordinates");
		Con_Printf("Draw_TransPic: bad coordinates %d %d\n", x, y);
		return;
	}
...
#if defined(SH4)
	sprintf (ver, "(BT-SHx Quake) %4.2f", (float)VERSION);
...
//	printf("Draw_ConsoleBackground: lines=%d xs=%d ys=%d dst=%p src=%p\n",
//		lines, vid.conwidth, vid.conheight,
//		vid.conbuffer, conback->data);
	...
	else
	{
		...
		// FIXME: does the endian switching go away in production?
			...
			if((lines+32)>=vid.conheight)
			{
				for (x=0 ; x<vid.conwidth ; x+=4)
				{
					pusdest[x+0] = d_8to16table[src[f>>16]];
					f += fstep;
			...
			}else
			{
				for (x=0 ; x<vid.conwidth ; x+=4)
				{
					pusdest[x+0] = VID_BlendEven16(
						d_8to16table[src[f>>16]], pusdest[x+0]);
					f += fstep;
					pusdest[x+1] = VID_BlendEven16(
						d_8to16table[src[f>>16]], pusdest[x+1]);
					f += fstep;
					pusdest[x+2] = VID_BlendEven16(
						d_8to16table[src[f>>16]], pusdest[x+2]);
					f += fstep;
					pusdest[x+3] = VID_BlendEven16(
						d_8to16table[src[f>>16]], pusdest[x+3]);
					f += fstep;
				}
			}
		}
...
//		int	t;
		...
		pbuf = (u16 *)(vid.buffer + vid.rowbytes*y);
//		t = (y & 1) << 1;

		for (x=0 ; x<vid.width ; x++)
		{
//			pbuf[x] = VID_BlendEven16(pbuf[x], 0x01EF);
//			pbuf[x] = VID_Blend88_16(pbuf[x], 0x01EF);
//			pbuf[x] = VID_Blend75_16(0x0210, pbuf[x]);
			pbuf[x] = VID_Blend66_16(0x0210, pbuf[x]);
