...
// drop the bonus value
	...
	for (j=0 ; j<NUM_CSHIFTS ; j++)	
	{
		vec[0] += cl.cshifts[j].percent*cl.cshifts[j].destcolor[0];
		vec[1] += cl.cshifts[j].percent*cl.cshifts[j].destcolor[1];
		vec[2] += cl.cshifts[j].percent*cl.cshifts[j].destcolor[2];
		vec[3] += cl.cshifts[j].percent;
	}
	...
	VID_ShiftPaletteVec(vec[0], vec[1], vec[2], vec[3]);
