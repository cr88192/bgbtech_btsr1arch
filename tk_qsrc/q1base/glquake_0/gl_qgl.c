...
#ifdef _WIN32
// #define GPA( a )	((void *)(GetProcAddress( hinstOpenGL, a )))

void *QGL_GetProcAddress(char *name)
{
	void *ptr;
	...
	ptr = GetProcAddress( hinstOpenGL, name );
	if(ptr)
		return(ptr);
...
qboolean QGL_Init( const char *dllname )
{
	...
	//	assert( glw_state.hinstOpenGL == 0 );
		...
		if ( dllname[0] != '!' )
		{
			snprintf( libName, sizeof( libName ), "%s\\%s",
				systemDir, dllname );
		}
		else
		{
	//		Q_strncpyz( libName, dllname, sizeof( libName ) );
			strncpy( libName, dllname, sizeof( libName ) );
		}

		hinstOpenGL = LoadLibrary( dllname );
		if ( !hinstOpenGL )
		{
			return false;
		}
