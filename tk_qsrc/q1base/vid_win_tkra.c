...
//	vid.rowbytes = vid.conrowbytes = dibdc->mi.bytesPerLine;
	...
	if(host_colormap16)
	{
		vid.rowbytes = vid.conrowbytes = DIBWidth*2;
		r_pixbytes=2;
	}
...
//		memcpy(vid.buffer, sbuf, vid.width*vid.height*2);
		...
		for(y=0; y<vid.height; y++)
		{
			yn=vid.height-y-1;
			memcpy(
				((u16 *)(vid.buffer))+(y*vid.width),
				((u16 *)(sbuf))+(yn*vid.width),
				vid.width*2);
		}
...
//		cv=vid_clamp255(cv+4)>>3;
		...
		dr=cr>>3;	dg=cg>>3;	db=cb>>3;
		d_8to16table[i]=(dr<<10)|(dg<<5)|db;
...
//				*ict++=vid_tpix16_yuv655le(*ics16++);
	...
	}else
	{
		...
		{
			ict=fbufDib+(DIBHeight-i-1)*DIBWidth;
			for(j=0; j<DIBWidth; j+=8)
			{
				ict[0]=d_8to24table[ics[0]];
				ict[1]=d_8to24table[ics[1]];
				ict[2]=d_8to24table[ics[2]];
				ict[3]=d_8to24table[ics[3]];
				ict[4]=d_8to24table[ics[4]];
				ict[5]=d_8to24table[ics[5]];
				ict[6]=d_8to24table[ics[6]];
				ict[7]=d_8to24table[ics[7]];
				ict+=8; ics+=8;
			}
		}
	}
