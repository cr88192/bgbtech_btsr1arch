	...
	// is the element a pak file?
		...
		// look through all the pak file elements
			...
			for (i=0 ; i<pak->numfiles ; i++)
				...
				{       // found it!
					Sys_Printf ("PackFile: %s : %s %d\n",
						pak->filename, filename, pak->handle);
					if (handle)
					{
						*handle = pak->handle;
...
//	printf("COM_LoadFile: %s hdl=%d uh=%d A\n", path, h, usehunk);
