...
void Mod_Init (void)
{
	...
	if(!mod_known)
		mod_known = malloc(MAX_MOD_KNOWN * sizeof(model_t));
}
...
void Mod_LoadEdges (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		out->luma[0] = 0;	//BGB: Initial Value
		out->luma[1] = 0;	//BGB: Initial Value
	}
}
...
#define FLOODFILL_INNER							\
		{										\
			w=(y % skinheight) * skinwidth +	\
				(x % skinwidth);				\
			pix=skin[w];						\
			if(mask)							\
			{									\
...
void Mod_FloodFillSkin( byte *skin, int skinwidth, int skinheight )
{
	...
	*skin=255;
	...
	for(z=0; z<8; z++)
	{
		...
		for(x=0; x<=skinwidth; x++)
			for(y=skinheight; y>=0; y--)
				FLOODFILL_INNER
		for(y=0; y<=skinheight; y++)
			for(x=skinwidth; x>=0; x--)
				FLOODFILL_INNER
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype)
{
	...
	for (i=0 ; i<numskins ; i++)
	{
		...
		//		}
				...
				pheader->gl_texturenum[i][3] = k;
			}else
			{
				skin = (byte *)(pskintype + 1);

				Mod_FloodFillSkin( skin,
					pheader->skinwidth, pheader->skinheight );
...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	static	short		osbidx[MAXALIASVERTS];
...
// mtriangle_t		*triangles;
	stverts = Hunk_Alloc (
		pheader->numverts * sizeof(stvert_t));
	pheader->p_stverts = ((byte *)stverts) - ((byte *)pheader);
...
// mtriangle_t		*triangles;
	triangles = Hunk_Alloc (
		pheader->numtris * sizeof(mtriangle_t));
	pheader->p_triangles = ((byte *)triangles) - ((byte *)pheader);
...
// trivertx_t	**poseverts;
	pposeverts = Hunk_AllocName (
		pheader->numframes * sizeof(int), loadname);
	pheader->p_poseverts = ((byte *)pposeverts) - ((byte *)pheader);
...
//	int					n_osvidx;
	...
	osvidx = Hunk_Alloc(n_osvidx * sizeof(short));
	pheader->p_osvidx = ((byte *)osvidx) - ((byte *)pheader);
	pheader->n_osvidx = n_osvidx;
	...
	for(i=0; i<pheader->numverts; i++)
	{
		...
		if(stverts[i].onseam)
		{
			osbidx[i]=pheader->numverts + j;
			osvidx[j]=i;
			j++;
		}
	}

	tridx = Hunk_Alloc(pheader->numtris * 3 * sizeof(short));
	pheader->p_tridx = ((byte *)tridx) - ((byte *)pheader);

	for (i=0 ; i<pheader->numtris ; i++)
	{
		tridx[i*3+0]=triangles[i].vertindex[0];
		tridx[i*3+1]=triangles[i].vertindex[1];
		tridx[i*3+2]=triangles[i].vertindex[2];

		if(!triangles[i].facesfront)
		{
			tridx[i*3+0]=osbidx[tridx[i*3+0]];
			tridx[i*3+1]=osbidx[tridx[i*3+1]];
			tridx[i*3+2]=osbidx[tridx[i*3+2]];
		}
	}

	sttvert = Hunk_Alloc (
		(pheader->numverts + n_osvidx) * 2 * sizeof(float));
	pheader->p_sttverts = ((byte *)sttvert) - ((byte *)pheader);

	scs = 1.0 / pheader->skinwidth;
	sct = 1.0 / pheader->skinheight;

	for (i=0 ; i<pheader->numverts ; i++)
	{
		sttvert[i*2+0] = stverts[i].s * scs;
		sttvert[i*2+1] = stverts[i].t * sct;
	}

	for (i=0 ; i<n_osvidx ; i++)
	{
		j = osvidx[i];
		k = i + pheader->numverts;
		sttvert[k*2+0] = sttvert[j*2+0] + (0.5-(scs/2));
		sttvert[k*2+1] = sttvert[j*2+1];
	}
...
//
	...
	{
		pposeverts[i] = (byte *)poseverts[i] - (byte *)pheader;
	}
