...
dt_bool G_CheckDemoStatus (void);
...
extern dt_bool mn_SuicideConsole;

gameaction_t    gameaction;
gamestate_t     gamestate;
skill_t         gameskill;
//dt_bool         respawnmonsters;
int             gameepisode;
int             gamemap;
int				 prevmap;

dt_bool         paused;
dt_bool         sendpause;              // send a pause event next tic
dt_bool         sendsave;               // send a save event next tic
dt_bool         usergame;               // ok to save / end game

dt_bool         timingdemo;             // if true, exit with report on completion
int             starttime;              // for comparative timing purposes      

dt_bool         viewactive;

dt_bool         deathmatch;             // only if started as net death
dt_bool         netgame;                // only true if packets are broadcast
dt_bool         playeringame[MAXPLAYERS];
...
char            demoname[32];
dt_bool         demorecording;
dt_bool         demoplayback;
byte            *demobuffer, *demo_p;
dt_bool         singledemo;             // quit after playing a demo from cmdline

dt_bool         precache = true;        // if true, load all graphics at start
...
dt_bool G_Responder(event_t *ev)
{
	player_t *plr;
	extern dt_bool MenuActive;
...
dt_bool G_CheckSpot (int playernum, mapthing_t *mthing)
{
	fixed_t         x,y;
	subsector_t *ss;
	unsigned        an;
