...
dt_bool         floatok;	// if true, move would be ok if
...
dt_bool PIT_StompThing (mobj_t *thing)
{
	fixed_t         blockdist;
...
dt_bool PIT_ThrustStompThing (mobj_t *thing)
{
	fixed_t         blockdist;
...
dt_bool PIT_CheckLine(line_t *ld)
{
...
dt_bool PIT_CheckThing(mobj_t *thing)
{
	fixed_t blockdist;
	dt_bool solid;
	int damage;
	...
	if(tmthing->flags&MF_SKULLFLY)
	{
		...
		else if(tmthing->type == MT_HOLY_FX)
		{
			if(thing->flags&MF_SHOOTABLE && thing != tmthing->target)
			{
				...
				{
					tmthing->special1 = (nlint)tmthing->target;
					tmthing->target = thing;
					return true;
				}
				if(thing->flags&MF_COUNTKILL || thing->player)
				{
					tmthing->special1 = (nlint)thing;
				}
				if(P_Random() < 96)
				{
					damage = 12;
			...
			{			
				...
				if(lmo)
				{
					if(lmo->type == MT_LIGHTNING_FLOOR)
					{
						...
						{
							((mobj_t *)lmo->special2)->special1 = (nlint)thing;
						}
					}
					else if(!lmo->special1)
					{
						lmo->special1 = (nlint)thing;
					}
					if(!(leveltime&3))
					{
						lmo->health--;
...
dt_bool PIT_CheckOnmobjZ(mobj_t *thing)
{
	fixed_t blockdist;
...
dt_bool P_ThingHeightClip (mobj_t *thing)
{
	dt_bool         onfloor;
...
dt_bool         PTR_SlideTraverse (intercept_t *in)
{
	line_t  *li;
...
dt_bool PTR_BounceTraverse(intercept_t *in)
{
	line_t  *li;
...
dt_bool         PTR_AimTraverse (intercept_t *in)
{
	line_t          *li;
	mobj_t          *th;
...
dt_bool         PTR_ShootTraverse (intercept_t *in)
{
	fixed_t         x,y,z;
	fixed_t         frac;
	line_t          *li;
...
dt_bool         PTR_UseTraverse (intercept_t *in)
{
	int sound;
	fixed_t pheight;
...
static dt_bool PuzzleActivated;

dt_bool PTR_PuzzleItemTraverse(intercept_t *in)
{
	mobj_t *mobj;
	int sound;
...
dt_bool PIT_RadiusAttack (mobj_t *thing)
{
	fixed_t dx, dy, dist;
	int damage;
...
dt_bool PIT_ChangeSector (mobj_t *thing)
{
	mobj_t          *mo;
