...
// interaction info
	...
	nlint			special1;		// Special info
	nlint			special2;		// Special info
	int				health;
	int				movedir;		// 0-7
...
//	weapontype_t	pendingweapon;		// wp_nochange if not changing
	byte		pendingweapon;		// wp_nochange if not changing
	dt_bool		weaponowned[NUMWEAPONS];
	int			mana[NUMMANA];
...
extern dt_bool paused;

extern dt_bool shareware; // true if other episodes not present

extern dt_bool DevMaps; // true = map development mode
extern char *DevMapsDir; // development maps directory

extern dt_bool nomonsters; // checkparm of -nomonsters

extern dt_bool respawnparm; // checkparm of -respawn

extern dt_bool randomclass; // checkparm of -randclass

extern dt_bool debugmode; // checkparm of -debug

extern dt_bool usergame; // ok to save / end game

extern dt_bool ravpic; // checkparm of -ravpic

extern dt_bool altpal; // checkparm to use an alternate palette routine

extern dt_bool cdrom; // true if cd-rom mode active ("-cdrom")

extern dt_bool deathmatch; // only if started as net death

extern dt_bool netgame; // only true if >1 player

extern dt_bool cmdfrag; // true if a CMD_FRAG packet should be sent out every
						// kill

extern dt_bool playeringame[MAXPLAYERS];
...
extern	dt_bool		singletics;			// debug flag to cancel adaptiveness

extern dt_bool DebugSound; // debug flag for displaying sound info

extern dt_bool demoplayback;
...
//extern	dt_bool		respawnmonsters;
...
extern dt_bool precache; // if true, load all graphics at level load

// extern byte *screen; // off screen work buffer, from V_video.c
extern dt_scrpix *screen; // off screen work buffer, from V_video.c
extern  lighttable_t    *colormaps;

extern dt_bool singledemo; // quit after playing a demo from cmdline
...
u32	M_SoftDivRcp (u32 a);

int	M_SoftDivS (int a, int b);
...
#ifdef __WATCOMC__
extern dt_bool useexterndriver;
...
extern dt_bool setsizeneeded;

extern dt_bool BorderNeedRefresh;
extern dt_bool BorderTopRefresh;
...
int M_DrawText (int x, int y, dt_bool direct, char *string);
...
extern int sc_Line;
extern dt_bool sc_End;
extern dt_bool sc_Crossed;
extern dt_bool sc_FileScripts;
