...
static dt_bool TagBusy(int tag);
static dt_bool AddToACSStore(int map, int number, byte *args);
...
int n_PCodeCmds = sizeof(PCodeCmds) / sizeof(void *);
...
//	ACStrings = (char **)buffer;
	ACStrings = malloc(ACStringCount * sizeof(char *));
...
void T_InterpretACS(acs_t *script)
{
	...
	do
	{
		...
		if((cmd<0) || (cmd>=n_PCodeCmds))
		{
			//BGB: Debug
			printf("ACS Bad Opcode Number\n");
