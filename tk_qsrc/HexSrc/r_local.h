...
typedef struct
{
	...
	int bbox[4];
	int validcount;
	dt_bool crush; 			// should the polyobj attempt to crush mobjs?
	int seqType;
...
// typedef dt_scrpix    lighttable_t;           // this could be wider for >8 bit display
...
typedef struct vissprite_s
{
	...
	dt_bool         psprite;                // true if psprite
...
typedef struct
{
	dt_bool         rotate;         // if false use 0 for any position
...
extern  angle_t         xtoviewangle[PADSCREENWIDTH];
...
extern  dt_bool         segtextured;
extern  dt_bool         markfloor;              // false if the back side is the same plane
extern  dt_bool         markceiling;
extern  dt_bool         skymap;
...
extern dt_bool LevelUseFullBright;
