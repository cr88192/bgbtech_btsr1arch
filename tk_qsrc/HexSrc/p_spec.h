...
// when needed
dt_bool P_ExecuteLineSpecial(int special, byte *args, line_t *line, int side,
	mobj_t *mo);
dt_bool P_ActivateLine(line_t *ld, mobj_t *mo, int side, int activationType);
//dt_bool P_UseSpecialLine ( mobj_t *thing, line_t *line);
...
dt_bool EV_SpawnLight(line_t *line, byte *arg, lighttype_t type);
...
dt_bool EV_VerticalDoor(line_t *line, mobj_t *thing);
...
int EV_BuildPillar(line_t *line, byte *args, dt_bool crush);
int EV_OpenPillar(line_t *line, byte *args);
int EV_DoFloorAndCeiling(line_t *line, byte *args, dt_bool raise);
int EV_FloorCrushStop(line_t *line, byte *args);
dt_bool EV_StartFloorWaggle(int tag, int height, int speed, int offset,
	int timer);
...
dt_bool P_Teleport(mobj_t *thing, fixed_t x, fixed_t y, angle_t angle,
	dt_bool useFog);
dt_bool EV_Teleport(int tid, mobj_t *thing, dt_bool fog);
...
dt_bool P_StartACS(int number, int map, byte *args, mobj_t *activator,
	line_t *line, int side);
dt_bool P_StartLockedACS(line_t *line, byte *args, mobj_t *mo, int side); 
dt_bool P_TerminateACS(int number, int map);
dt_bool P_SuspendACS(int number, int map);
...
dt_bool EV_ThingProjectile(byte *args, dt_bool gravity);
dt_bool EV_ThingSpawn(byte *args, dt_bool fog);
dt_bool EV_ThingActivate(int tid);
dt_bool EV_ThingDeactivate(int tid);
dt_bool EV_ThingRemove(int tid);
dt_bool EV_ThingDestroy(int tid);
