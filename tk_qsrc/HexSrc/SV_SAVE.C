...
static void SetMobjPtr(nlint *archiveNum);
...
static dt_bool ExistingFile(char *name);
...
static nlint **TargetPlayerAddrs;
static int TargetPlayerCount;
static byte *SaveBuffer;
static dt_bool SavingPlayers;
static union
{
	byte *b;
	short *w;
...
void SV_SaveMap(dt_bool savePlayers)
{
	char fileName[100];
...
void SV_MapTeleport(int map, int position)
{
	...
	int currentInvPos;
	dt_bool rClass;
	dt_bool playerWasReborn;
	dt_bool oldWeaponowned[NUMWEAPONS];
	int oldKeys;
	int oldPieces;
	int bestWeapon;
	...
	if(TargetPlayerAddrs)
	{
		...
		{
			*TargetPlayerAddrs[i] = (nlint)targetPlayerMobj;
		}
		Z_Free(TargetPlayerAddrs);
	}
...
dt_bool SV_RebornSlotAvailable(void)
{
	char fileName[100];
		...
		for(j = 0; j < NUMPSPRITES; j++)
		{
			if(players[i].psprites[j].state)
			{
				players[i].psprites[j].state =
					&states[(nlint)players[i].psprites[j].state];
			}
		}
	}
}
...
static void MangleMobj(mobj_t *mobj)
{
	...
	{
		mobj->target = (mobj_t *)(nlint)GetMobjNum(mobj->target);
	}
	switch(mobj->type)
	{
		// Just special1
...
static void RestoreMobj(mobj_t *mobj)
{
	mobj->state = &states[(nlint)mobj->state];
	if(mobj->player)
	{
		mobj->player = &players[(nlint)mobj->player-1];
		mobj->player->mo = mobj;
	}
...
static void SetMobjPtr(nlint *archiveNum)
{
	...
	*archiveNum = (nlint)MobjList[*archiveNum];
}
...
static void RestoreSSThinker(ssthinker_t *sst)
{
	sst->sector = &sectors[(nlint)sst->sector];
...
static void RestoreSSThinkerNoSD(ssthinker_t *sst)
{
	sst->sector = &sectors[(int)(nlint)sst->sector];
}
...
static void MangleScript(acs_t *script)
{
	script->ip = (int *)((nlint)(script->ip)-(nlint)ActionCodeBase);
	script->line = script->line ?
		(line_t *)(script->line-lines) : (line_t *)-1;
	script->activator = (mobj_t *)((nlint)GetMobjNum(script->activator));
}
...
static void RestoreScript(acs_t *script)
{
	script->ip = (int *)(ActionCodeBase+(nlint)script->ip);
	if((int)(nlint)(script->line) == -1)
	{
		script->line = NULL;
	}
	else
	{
		script->line = &lines[(int)(nlint)(script->line)];
	}
	SetMobjPtr((nlint *)&script->activator);
}
...
static void RestorePlatRaise(plat_t *plat)
{
	plat->sector = &sectors[(int)(nlint)(plat->sector)];
...
static void RestoreMoveCeiling(ceiling_t *ceiling)
{
	ceiling->sector = &sectors[(int)(nlint)(ceiling->sector)];
