...
dt_bool M_ParmExists(char *check)
{
...
//	handle = open (name, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666);
	...
	if (handle == -1)
		return false;
	count = w_write (handle, source, length);
	w_close (handle);
...
//	length = fileinfo.st_size;
	...
	count = w_read(handle, buf, length);
	w_close(handle);
	if(count < length)
	{
		I_Error("Couldn't read file %s", name);
...
//	{ "key_speed", &key_speed, 'z', 1 },
...
#ifndef DEFAULT_SAVEPATH
	#define DEFAULT_SAVEPATH		""
#endif

	{ "savedir", (int *) &SavePath, (nlint) DEFAULT_SAVEPATH },
	...
	{ "chatmacro3", (int *) &chat_macros[3], (nlint) HUSTR_CHATMACRO3 },
	{ "chatmacro4", (int *) &chat_macros[4], (nlint) HUSTR_CHATMACRO4 },
	{ "chatmacro5", (int *) &chat_macros[5], (nlint) HUSTR_CHATMACRO5 },
	{ "chatmacro6", (int *) &chat_macros[6], (nlint) HUSTR_CHATMACRO6 },
			...
			if(fscanf(f, "%79s %[^\n]\n", def, strparm) == 2)
			{
				...
				for(i = 0; i < numdefaults; i++)
				{
					if(!strcmp(def, defaults[i].name))
					{
						...
						{
							*defaults[i].location = (nlint)newstring;
						}
						break;
					}
				}
