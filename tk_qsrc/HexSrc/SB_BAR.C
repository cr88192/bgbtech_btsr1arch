...
static dt_bool HandleCheats(byte key);
static dt_bool CheatAddKey(Cheat_t *cheat, byte key, dt_bool *eat);
...
dt_bool DebugSound; // Debug flag for displaying sound info
dt_bool inventory;
int curpos;
int inv_ptr;
int ArtifactFlash;
...
#ifndef __WATCOMC__
dt_bool i_CDMusic; // in Watcom, defined in i_ibm
#endif
...
void DrawKeyBar(void)
{
	...
	{
		xPosition = 46;
		for(i = 0; i < G_KEY_NUMKEYS && xPosition <= 126; i++)
		{
			if(CPlayer->keys&(1<<i))
			{
...
dt_bool SB_Responder(event_t *event)
{
...
{
	int i;
	//extern dt_bool *WeaponInShareware;
...
static void CheatPigFunc(player_t *player, Cheat_t *cheat)
{
	extern dt_bool P_UndoPlayerMorph(player_t *player);
