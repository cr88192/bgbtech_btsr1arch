...
dt_bool P_CheckMeleeRange(mobj_t *actor)
{
	mobj_t *mo;
	fixed_t dist;
...
dt_bool P_CheckMeleeRange2(mobj_t *actor)
{
	mobj_t *mo;
	fixed_t dist;
...
dt_bool P_CheckMissileRange(mobj_t *actor)
{
	fixed_t dist;
...
dt_bool P_TryWalk(mobj_t *actor)
{
	if(!P_Move(actor))
	{
		return(false);
...
dt_bool P_LookForPlayers(mobj_t *actor, dt_bool allaround)
{
	int c;
	int stop;
	player_t *player;
...
dt_bool P_UpdateMorphedMonster(mobj_t *actor, int tics)
{
	mobj_t *fog;
	fixed_t x;
	fixed_t y;
...
void A_BishopAttack2(mobj_t *actor)
{
	...
	if(mo)
	{
		mo->special1 = (nlint)actor->target;
...
static void DragonSeek(mobj_t *actor, angle_t thresh, angle_t turnMax)
{
	...
	{ // Hit the target thing
		if(actor->target && P_Random() < 200)
		{
			...
			{
				search = -1;
				actor->special1 = (nlint)P_FindMobjFromTID(target->args[bestArg], 
					&search);
			}
		}
		else
		{
			...
			search = -1;
			actor->special1 = (nlint)P_FindMobjFromTID(target->args[i], &search);
		}
	}
}
...
void A_DragonInitFlight(mobj_t *actor)
{
	...
	{ // find the first tid identical to the dragon's tid
		actor->special1 = (nlint)P_FindMobjFromTID(actor->tid, &search);
		if(search == -1)
		{
			P_SetMobjState(actor, actor->info->spawnstate);
			return;
		}
	} while(actor->special1 == (nlint)actor);
	P_RemoveMobjFromTIDList(actor);
}
...
void A_SorcOffense1(mobj_t *actor)
{
	...
	{
		mo->target = parent;
		mo->special1 = (nlint)parent->target;
...
void KSpiritInit(mobj_t *spirit, mobj_t *korax)
{
	...
	spirit->special1 = (nlint)korax;				// Swarm around korax
