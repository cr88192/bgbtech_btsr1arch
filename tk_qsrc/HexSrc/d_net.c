...
dt_bool                 nodeingame[MAXNETNODES];        // set false as nodes leave game
dt_bool                 remoteresend[MAXNETNODES];      // set when local needs tics
...
unsigned NetbufferChecksum (void)
{
	...
	l = (NetbufferSize () - (nlint)&(((doomdata_t *)0)->retransmitfrom))/4;
	for (i=0 ; i<l ; i++)
...
{
	int             i;
	dt_bool gotinfo[MAXNETNODES];
	dt_bool gotClass[MAXNETNODES];
#ifdef __WATCOMC__
	int nextTic;
