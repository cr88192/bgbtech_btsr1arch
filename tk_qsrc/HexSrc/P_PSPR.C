...
dt_bool P_CheckMana(player_t *player)
{
	manatype_t mana;
	int count;
...
void A_MLightningAttack2(mobj_t *actor)
{
	...
	{
		fmo->special1 = 0;
		fmo->special2 = (nlint)cmo;
		A_LightningZap(fmo);	
	}
	if(cmo)
	{
		cmo->special1 = 0;	// mobj that it will track
		cmo->special2 = (nlint)fmo;
		A_LightningZap(cmo);	
	}
...
void MStaffSpawn(mobj_t *pmo, angle_t angle)
{
	...
	{
		mo->target = pmo;
		mo->special1 = (nlint)P_RoughMonsterSearch(mo, 10);
	}
}
...
void A_MStaffTrack(mobj_t *actor)
{
	...
	{
		actor->special1 = (nlint)P_RoughMonsterSearch(actor, 10);
	}
...
void A_CHolyAttack2(mobj_t *actor)
{
	...
	for(j = 0; j < 4; j++)
	{
		...
		if(linetarget)
		{
			mo->special1 = (nlint)linetarget;
			mo->flags |= MF_NOCLIP|MF_SKULLFLY;
			mo->flags &= ~MF_MISSILE;
		}
		tail = P_SpawnMobj(mo->x, mo->y, mo->z, MT_HOLY_TAIL);
		tail->special2 = (nlint)mo; // parent
		for(i = 1; i < 3; i++)
		{
