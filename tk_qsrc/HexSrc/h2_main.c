...
void F_Drawer(void);
dt_bool F_Responder(event_t *ev);
...
extern dt_bool askforquit;
extern char *SavePath;

// PUBLIC DATA DEFINITIONS -------------------------------------------------

dt_bool DevMaps;			// true = Map development mode
char *DevMapsDir = "";		// development maps directory
dt_bool shareware;			// true if only episode 1 present
dt_bool nomonsters;			// checkparm of -nomonsters
dt_bool respawnparm;		// checkparm of -respawn
dt_bool randomclass;		// checkparm of -randclass
dt_bool debugmode;			// checkparm of -debug
dt_bool ravpic;				// checkparm of -ravpic
dt_bool cdrom;				// true if cd-rom mode active
dt_bool cmdfrag;			// true if a CMD_FRAG packet should be sent out
dt_bool singletics;			// debug flag to cancel adaptiveness
dt_bool artiskip;			// whether shift-enter skips an artifact
...
void H2_ProcessEvents(void)
{
	...
	for(; eventtail != eventhead; eventtail = (++eventtail)&(MAXEVENTS-1))
	{
		...
		if(F_Responder(ev))
		{
//			printf("H2_ProcessEvents: F Responder\n");
			continue;
		}
		if(MN_Responder(ev))
		{
//			printf("H2_ProcessEvents: MN Responder\n");
			continue;
		}

//		printf("H2_ProcessEvents: G Responder\n");
		G_Responder(ev);
	}
}
