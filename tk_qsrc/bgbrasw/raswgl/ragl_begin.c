...
void RASWGL_Init(RASWGL_Context *ctx)
{
	...
	i=256;
	ctx->xyzbuf=bgbrasw_malloc(i*4*sizeof(float));
	ctx->stbuf=bgbrasw_malloc(i*2*sizeof(float));
	ctx->nbuf=bgbrasw_malloc(i*4*sizeof(float));
	ctx->clrbuf=bgbrasw_malloc(i*4*sizeof(float));
	ctx->mvec=i;
