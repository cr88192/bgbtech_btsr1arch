...
int RIFF_ReadChunkInfo(FILE *fd, int ofs, int *id, int *sz, int *lid)
{
	fseek(fd, ofs, 0);
...
int RIFF_NextChunk(FILE *fd, int ofs)
{
	int cc1, cc2, sz;
...
int RIFF_FindSubChunk(FILE *fd, int ofs, int id, int lid)
{
	int cc1, cc2, sz;
	int end, cur;
...
void *RIFF_ReadInChunk(FILE *fd, int ofs, int *size)
{
	int cc1, cc2, sz;
	void *buf;
	...
	buf=malloc(sz+1024);
	fseek(fd, ofs+8, 0);
	fread(buf, 1, sz+1024, fd);

	return(buf);
}

void *RIFF_FindReadInChunk(FILE *fd, int ofs, int id, int *size)
{
	int ch_ofs;
	void *p;
...
//	ctx->fd=fopen(name, "rb");
	...
	default:
		...
		inhead=bgbrpi_malloc(sizeof(BGBBTJ_BITMAPINFOHEADER));
		outhead=bgbrpi_malloc(sizeof(BGBBTJ_BITMAPINFOHEADER));
		inhead->biWidth=w;
		inhead->biHeight=h;
	...
	ctx->sampbuf=bgbrpi_malloc(1<<18);
...
BGBBTJ_Video_Stats *BGBBTJ_AVI_GetStats(BGBBTJ_AVI_Context *ctx)
{
	...
	tmp=bgbrpi_malloc(sizeof(BGBBTJ_Video_Stats));
...
void *BGBBTJ_AVI_FrameRaw(BGBBTJ_AVI_Context *ctx, float dt)
{
	int fc;
	int iw, ih;
...
void *BGBBTJ_AVI_FrameRawClrs(BGBBTJ_AVI_Context *ctx, float dt, int clrs)
{
	double ft;
	int fc;
	int iw, ih;
...
int BGBBTJ_AVI_Seek(BGBBTJ_AVI_Context *ctx, float dt)
{
	int fc;
	int iw, ih;
...
float BGBBTJ_AVI_GetPosition(BGBBTJ_AVI_Context *ctx)
{
	float t;
