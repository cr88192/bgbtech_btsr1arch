...
//	if(ctx->cur_func->regflags&BGBCC_REGFL_LEAFTINY)
	...
	if((reg&BGBCC_SH_REG_RTMASK)==BGBCC_SH_REG_LR16)
	{
		if((reg&15)>=8)
			{ BGBCC_DBGBREAK }
		sctx->sreg_held|=1<<((reg&31)+0);
		sctx->sreg_held|=1<<((reg&31)+1);
		return(1);
	}
