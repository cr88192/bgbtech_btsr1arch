...
volatile int bipro_prof_active=0;
volatile char bipro_prof_reqstop=0;
volatile char bipro_prof_reqalloc=0;
volatile int **bipro_prof_stats=NULL;
volatile int *bipro_prof_statsb=NULL;
volatile nlint *bipro_prof_stats_hib=NULL;

volatile nlint *bipro_prof_stats_eip=NULL;
volatile int *bipro_prof_stats_cnt=NULL;
volatile u32 bipro_prof_totcnt=0;

volatile int *bipro_prof_statsagg_cnt=NULL;
volatile char **bipro_prof_statsagg_lib=NULL;
volatile char **bipro_prof_statsagg_sym=NULL;
volatile int bipro_prof_statsagg_num=0;
volatile int bipro_prof_statsagg_totcnt=0;
...
int BIPRO_InitDbgHelpOS()
{
	...
	printf("BIPRO_InitDbgHelpOS\n");

	BIPRO_LoadDynamicOS("dbghelp.dll");
...
{
	BIPRO_RegisterState regs;
	nlint hib;
	int h, n;
	int i, j, k;

	memset(&regs, 0, sizeof(BIPRO_RegisterState));
	BIPRO_GetThreadRegisterState(inf, &regs);

#ifdef X86
	i=(regs.eip)>>16;
	j=(regs.eip&65535)>>4;

	bipro_prof_stats_hib[i]=i;
	bipro_prof_totcnt++;
	bipro_prof_statsb[i]++;

	if(!bipro_prof_stats[i])
	{
		bipro_prof_reqalloc=1;
		return;
	}
	
	bipro_prof_stats[i][j]++;
#endif

#ifdef X86_64
	hib=(nlint)((regs.rip)>>16);
//	h=(u16)((hib*65521)>>16);
//	h=hib*65521+31;
	h=((hib*65521+31)*65521+31)>>16;
	
	i=h; n=4096;
	while((n--)>0)
	{
		i=(u16)(h>>16);
		if(bipro_prof_stats_hib[i]==hib)
			break;
		if(!bipro_prof_statsb[i])
		{
			bipro_prof_stats_hib[i]=hib;
			break;
		}
		h=h*65521+31;
	}
...
void BIPRO_ProfileDumpStats_SortSwap(int a, int b)
{
	...
	ipa=bipro_prof_stats_eip[a];
	ipb=bipro_prof_stats_eip[b];
	bipro_prof_stats_eip[a]=ipb;
	bipro_prof_stats_eip[b]=ipa;

	cna=bipro_prof_stats_cnt[a];
	cnb=bipro_prof_stats_cnt[b];
	bipro_prof_stats_cnt[a]=cnb;
	bipro_prof_stats_cnt[b]=cna;
}
...
//			if(bipro_prof_stats_cnt[j]<bipro_prof_stats_cnt[i])
			if(bipro_prof_stats_cnt[j]>bipro_prof_stats_cnt[i])
				BIPRO_ProfileDumpStats_SortSwap(i, j);
		return;
	}
	...
	mid=(low+high)/2;
	mp=bipro_prof_stats_cnt[mid];

	i=low;
	while(i<high)
	{
		if(bipro_prof_stats_cnt[i]>mp)
//		if(bipro_prof_stats_cnt[i]<mp)
		{
			BIPRO_ProfileDumpStats_SortSwap(low++, i++);
			continue;
		}
		if(bipro_prof_stats_cnt[i]<mp)
//		if(bipro_prof_stats_cnt[i]>mp)
		{
			BIPRO_ProfileDumpStats_SortSwap(--high, i);
			continue;
		}
		i++;
	}

	BIPRO_ProfileDumpStats_Sort(base, low);
	BIPRO_ProfileDumpStats_Sort(high, lim);
...
void BIPRO_ProfileDumpStats_AggAddName(char *lib, char *sym, int cnt)
{
	...
	if(!bipro_prof_statsagg_cnt)
	{
		bipro_prof_statsagg_cnt=malloc((1<<16)*sizeof(int));
		bipro_prof_statsagg_lib=malloc((1<<16)*sizeof(char *));
		bipro_prof_statsagg_sym=malloc((1<<16)*sizeof(char *));
	}
	
	bipro_prof_statsagg_totcnt+=cnt;
	
	for(i=0; i<bipro_prof_statsagg_num; i++)
	{
		if(!strcmp((char *)bipro_prof_statsagg_lib[i], lib) &&
			!strcmp((char *)bipro_prof_statsagg_sym[i], sym))
		{
			bipro_prof_statsagg_cnt[i]+=cnt;
			return;
		}
	}
	
	i=bipro_prof_statsagg_num++;
	bipro_prof_statsagg_cnt[i]=cnt;
	bipro_prof_statsagg_lib[i]=lib;
	bipro_prof_statsagg_sym[i]=sym;
}
...
void BIPRO_ProfileDumpStats_SortSwapAgg(int a, int b)
{
	...
	cna=bipro_prof_statsagg_cnt[a];
	cnb=bipro_prof_statsagg_cnt[b];
	bipro_prof_statsagg_cnt[a]=cnb;
	bipro_prof_statsagg_cnt[b]=cna;

	sla=(char *)bipro_prof_statsagg_lib[a];
	slb=(char *)bipro_prof_statsagg_lib[b];
	bipro_prof_statsagg_lib[a]=slb;
	bipro_prof_statsagg_lib[b]=sla;

	ssa=(char *)bipro_prof_statsagg_sym[a];
	ssb=(char *)bipro_prof_statsagg_sym[b];
	bipro_prof_statsagg_sym[a]=ssb;
	bipro_prof_statsagg_sym[b]=ssa;
}
...
void BIPRO_ProfileDumpStats_SortAgg(int base, int lim)
{
	...
	{
		for(i=base; i<lim; i++)for(j=i+1; j<lim; j++)
			if(bipro_prof_statsagg_cnt[j]>bipro_prof_statsagg_cnt[i])
				BIPRO_ProfileDumpStats_SortSwapAgg(i, j);
		return;
	}
	...
	mid=(low+high)/2;
	mp=bipro_prof_statsagg_cnt[mid];

	i=low;
	while(i<high)
	{
		if(bipro_prof_statsagg_cnt[i]>mp)
		{
			BIPRO_ProfileDumpStats_SortSwapAgg(low++, i++);
			continue;
		}
		if(bipro_prof_statsagg_cnt[i]<mp)
		{
			BIPRO_ProfileDumpStats_SortSwapAgg(--high, i);
			continue;
		}
		i++;
	}

	BIPRO_ProfileDumpStats_SortAgg(base, low);
	BIPRO_ProfileDumpStats_SortAgg(high, lim);
...
int BIPRO_ProfileDumpStats()
{
	...
	if(!bipro_prof_stats_eip ||
		!bipro_prof_stats_cnt)
	{
		i=(1<<20)*sizeof(nlint);
		bipro_prof_stats_eip=malloc(i);
		memset((void *)bipro_prof_stats_eip, 0, i);

		i=(1<<20)*sizeof(int);
		bipro_prof_stats_cnt=malloc(i);
		memset((void *)bipro_prof_stats_cnt, 0, i);
	}
	...
	for(i=0; i<(1<<16); i++)
	{
		...
		for(j=0; j<4096; j++)
		{
			k=bipro_prof_stats[i][j];
			if(!k)continue;
			
			if(n>=(1<<20))
				break;
			
			eip=(bipro_prof_stats_hib[i]<<16)|(j<<4);
			bipro_prof_stats_eip[n]=eip;
//			bipro_prof_stats_eip[n]=(i<<16)|(j<<4);
			bipro_prof_stats_cnt[n]=k;

//			bipro_printf("%08X: %7d\n", 
//				bipro_prof_stats_eip[n],
//				bipro_prof_stats_cnt[n]);
				
			n++;
		}
	}
	
	BIPRO_ProfileDumpStats_Sort(0, n);
	
	n2=n;
	if(n2>64)n2=64;

#if 0
	bipro_printf("Profile Stats:\n");
	for(i=0; i<n2; i++)
	{
		eip=bipro_prof_stats_eip[i];
//		s=BIPRO_GetAddrName((void *)eip);
		eip1=eip+15;

		ptr=(void *)eip;
		s=BIPRO_GetAddrLastName(ptr, &bptr);
		if(!s)
		{
			ptr=(void *)eip1;
			s=BIPRO_GetAddrLastName(ptr, &bptr);
		...
		s1=BIPRO_LookupSectionAddrName(ptr);
		s2=BIPRO_LookupSectionAddrName(bptr);
		if(!s1 || !s2 || (s1!=s2))
		{
			s1="<NULL>";
			if(dst>16384)
		...
		if(bipro_prof_stats_cnt[i]<4)
			continue;

		bipro_printf("%08X %5d %14s!%-36s+%2d %3.2f%%\n",
			bipro_prof_stats_eip[i],
			bipro_prof_stats_cnt[i],
			s1, s, dst,
			(100.0*bipro_prof_stats_cnt[i])/(bipro_prof_totcnt+1));
	}
#endif

	bipro_prof_statsagg_num=0;
	bipro_prof_statsagg_totcnt=0;
...
//		s=BIPRO_GetAddrName((void *)eip);
		...
		BIPRO_ProfileDumpStats_AggAddName(s1, s,
			bipro_prof_stats_cnt[i]);
	}

	BIPRO_ProfileDumpStats_SortAgg(0,
		bipro_prof_statsagg_num);

	n2=bipro_prof_statsagg_num;
	if(n2>64)n2=64;
	
	bipro_printf("Profile Stats Aggregate:\n");
	for(i=0; i<n2; i++)
	{
		s1=(char *)bipro_prof_statsagg_lib[i];
		s=(char *)bipro_prof_statsagg_sym[i];
	
		bipro_printf("%16.16s!%-40.40s %9d %3.2f%%\n",
			s1, s, bipro_prof_statsagg_cnt[i],
			(100.0*bipro_prof_statsagg_cnt[i])/
				(bipro_prof_statsagg_totcnt+1));
	}
	return(0);
}
...
int BIPRO_ProfileLoop(void *p)
{
	...
	if(bipro_prof_active)
		return(0);

	if(!bipro_prof_stats)
	{
		i=(1<<16)*sizeof(int *);
		bipro_prof_stats=malloc(i);
		memset((void *)bipro_prof_stats, 0, i);

		i=(1<<16)*sizeof(int);
		bipro_prof_statsb=malloc(i);
		memset((void *)bipro_prof_statsb, 0, i);
		
		i=(1<<16)*sizeof(nlint *);
		bipro_prof_stats_hib=malloc(i);
		memset((void *)bipro_prof_stats_hib, 0, i);
	}

	bipro_printf("Profiler Start:\n");

	ncnt=30*1000;
	bipro_prof_active++;
	while(1)
	{
		if(bipro_prof_reqstop)
			break;
	
		BIPRO_HandleThreadStates(BIPRO_ProfileLoop_HandleThread);
		
		if(bipro_prof_reqalloc)
		{
			for(j=0; j<(1<<16); j++)
			{
				if(bipro_prof_statsb[j] && !bipro_prof_stats[j])
				{
					k=4096*sizeof(int);
					bipro_prof_stats[j]=malloc(k);
					memset((void *)bipro_prof_stats[j], 0, k);
				}
			}
			bipro_prof_reqalloc=0;
		}
...
//		thSleep(10);
	...
	bipro_printf("Profiler Stop:\n");
	BIPRO_ProfileDumpStats();

	bipro_prof_active--;
	
	if(bipro_prof_active<=0)
	{
		bipro_prof_reqstop=0;
		bipro_prof_active=0;

		for(j=0; j<(1<<16); j++)
		{
			if(bipro_prof_stats[j])
			{
				k=4096*sizeof(int);
				memset((void *)bipro_prof_stats[j], 0, k);
			}
...
int BIPRO_ProfilerSetActive(int fl)
{
	...
	if(fl)
	{
		...
		BIPRO_InitDllsOS();
		ptr=BIPRO_ProfilerSetActive;
		BIPRO_GetAddrLastName(ptr, &bptr);

		if(!bipro_prof_active)
		{
			thThread(BIPRO_ProfileLoop, NULL);
		}
	}else
	{
		if(bipro_prof_active)
		{
			bipro_prof_reqstop=1;
		}
	}
	return(0);
}
