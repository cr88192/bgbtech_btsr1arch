...
{
	return(bgbcc_tmalloc2("_mm_jpgbuf", sz));
}
...
void PDJPG_FreeContext(PDJPG_Context *ctx)
{
	...
	for(i=0; i<4; i++)
		if(ctx->jpg_sibuf[i])
			pdjpg_free(ctx->jpg_sibuf[i]);

	if(ctx->jpg_sabuf)
		pdjpg_free(ctx->jpg_sabuf);
...
//		h=(yi[i]+15)/8;
		...
		if(!ctx->jpg_sibuf[i])
			ctx->jpg_sibuf[i]=pdjpg_malloc(n*64);
	...
	byte *buf, int sz, int *xs, int *ys)
{
	...
	if(!obuf || (ctx->xs!=ctx->lxs) || (ctx->ys!=ctx->lys))
	{
		...
		obuf=pdjpg_malloc((ctx->xs+1)*(ctx->ys+1)*4);
	...
	byte *rgba, int xs, int ys)
{
	...
	if(ctx->jpg_is420)
	{
			...
			for(j=0; j<xs1; j++)
		{
			...
			i1=(i*2+1)*xs+j*2;
			i2=i*xs1+j;
			cy0=ctx->jpg_sibuf[0][i0+0]; cy1=ctx->jpg_sibuf[0][i0+1];
			cy2=ctx->jpg_sibuf[0][i1+0]; cy3=ctx->jpg_sibuf[0][i1+1];
			cu=ctx->jpg_sibuf[1][i2]; cv=ctx->jpg_sibuf[2][i2];
			if(ctx->jpg_sabuf)
//			if(0)
			{
				ca0=ctx->jpg_sabuf[i0+0]; ca1=ctx->jpg_sabuf[i0+1];
				ca2=ctx->jpg_sabuf[i1+0]; ca3=ctx->jpg_sabuf[i1+1];
			}else
			{
				ca0=255; ca1=255;
				ca2=255; ca3=255;
			...
			dr=0        +359*cv1+128;
			dg=0- 88*cu1-183*cv1+128;
			db=0+454*cu1        +128;

			dy0=cy0<<8; dy1=cy1<<8; dy2=cy2<<8; dy3=cy3<<8;
			cr0=(dy0+dr)>>8; cg0=(dy0+dg)>>8; cb0=(dy0+db)>>8;
			cr1=(dy1+dr)>>8; cg1=(dy1+dg)>>8; cb1=(dy1+db)>>8;
			cr2=(dy2+dr)>>8; cg2=(dy2+dg)>>8; cb2=(dy2+db)>>8;
			cr3=(dy3+dr)>>8; cg3=(dy3+dg)>>8; cb3=(dy3+db)>>8;
...
//			i1=((i*2+1)*xs+(j*2))*4;
			...
			i0=((yb+(i*2+0)*yst)+(j*2))*4;
			i1=((yb+(i*2+1)*yst)+(j*2))*4;

			rgba[i0+0]=cr0; rgba[i0+1]=cg0;
			rgba[i0+2]=cb0; rgba[i0+3]=ca0;
			rgba[i0+4]=cr1; rgba[i0+5]=cg1;
			rgba[i0+6]=cb1; rgba[i0+7]=ca1;
			rgba[i1+0]=cr2; rgba[i1+1]=cg2;
			rgba[i1+2]=cb2; rgba[i1+3]=ca2;
			rgba[i1+4]=cr3; rgba[i1+5]=cg3;
			rgba[i1+6]=cb3; rgba[i1+7]=ca3;
		}
		return(0);
	}else
	{
