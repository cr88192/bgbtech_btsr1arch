int BGBCC_CMGC_CoffLoadBufferDLL(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	byte *buf, int sz)
{
	...
	if((buf[0]!='P') || (buf[1]!='E'))
	{
		if((buf[0]!='M') || (buf[1]!='Z'))
		{
			printf("BGBCC_CMGC_CoffLoadBufferDLL: MZ Sig Fail\n");
			return(-1);
		}
		...
		{
			printf("BGBCC_CMGC_CoffLoadBufferDLL: PE Header Offset Fail\n");
			return(-1);
		}
	}else
	{
	...
	{
		printf("BGBCC_CMGC_CoffLoadBufferDLL: PE Sig Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_CMGC_CoffLoadBufferDLL: Opt Header Magic Fail\n");
		return(-1);
	}
...
//	if(i!=0x010B)
	...
	if((expofs==0) && (expsz==0))
	{
		printf("BGBCC_CMGC_CoffLoadBufferDLL: No Exports\n");
		return(0);
	}
	...
	{
		printf("BGBCC_CMGC_CoffLoadBufferDLL: Exports Offsets Fail\n");
		return(-1);
	}
	...
	if((i<=0) || (i>=sz))
	{
		printf("BGBCC_CMGC_CoffLoadBufferDLL: DllName RVA Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_CMGC_CoffLoadBufferDLL: Non Sane Exports Count\n");
		return(-1);
	}
...
int BGBCC_CMGC_CoffBuildExports(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx)
{
	...
	lbl_dllname=BGBCC_CMG_GenLabel(sctx);
	lbl_expaddrtab=BGBCC_CMG_GenLabel(sctx);
	lbl_nametab=BGBCC_CMG_GenLabel(sctx);
	lbl_ordtab=BGBCC_CMG_GenLabel(sctx);

	dllname=ctx->imgbasename;
	if(!dllname)
		dllname="default";

	BGBCC_CMG_SetSectionName(sctx, ".edata");
	BGBCC_CMG_EmitDWord(sctx, 0);		//0x00, export flags
	BGBCC_CMG_EmitDWord(sctx, 0);		//0x04, time/date
	BGBCC_CMG_EmitWord(sctx, 0);		//0x08, major version
	BGBCC_CMG_EmitWord(sctx, 0);		//0x0A, minor version

	BGBCC_CMG_EmitRelocTy(sctx, lbl_dllname, BGBCC_SH_RLC_RVA32);
	BGBCC_CMG_EmitDWord(sctx, 0);		//0x0C, name RVA

	BGBCC_CMG_EmitDWord(sctx, 1);		//0x10, ordinal base
	BGBCC_CMG_EmitDWord(sctx, nexps);	//0x14, address table entries
	BGBCC_CMG_EmitDWord(sctx, nexps);	//0x18, number of name pointers

	BGBCC_CMG_EmitRelocTy(sctx, lbl_expaddrtab, BGBCC_SH_RLC_RVA32);
	BGBCC_CMG_EmitDWord(sctx, 0);		//0x1C, export address table RVA
	BGBCC_CMG_EmitRelocTy(sctx, lbl_nametab, BGBCC_SH_RLC_RVA32);
	BGBCC_CMG_EmitDWord(sctx, 0);		//0x20, name pointer table RVA
	BGBCC_CMG_EmitRelocTy(sctx, lbl_ordtab, BGBCC_SH_RLC_RVA32);
	BGBCC_CMG_EmitDWord(sctx, 0);		//0x24, ordinal table RVA

	BGBCC_CMG_EmitLabel(sctx, lbl_dllname);
	BGBCC_CMG_EmitString(sctx, dllname);

	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];

		k=BGBCC_CMG_GenLabel(sctx);
		obj->fxnalgn=k;

		BGBCC_CMG_EmitLabel(sctx, k);
		BGBCC_CMG_EmitString(sctx, obj->name);
	}

	BGBCC_CMG_EmitBAlign(sctx, 4);

	BGBCC_CMG_EmitLabel(sctx, lbl_expaddrtab);
	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];
		BGBCC_CMG_EmitRelocTy(sctx,
			obj->fxoffs, BGBCC_SH_RLC_RVA32);
		BGBCC_CMG_EmitDWord(sctx, 0);	//export RVA
	}

	BGBCC_CMG_EmitLabel(sctx, lbl_nametab);
	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];
		BGBCC_CMG_EmitRelocTy(sctx,
			obj->fxnalgn, BGBCC_SH_RLC_RVA32);
		BGBCC_CMG_EmitDWord(sctx, 0);	//name RVA
	}

	BGBCC_CMG_EmitLabel(sctx, lbl_ordtab);
	for(i=0; i<nexps; i++)
	{
		BGBCC_CMG_EmitDWord(sctx, i+1);	//ordinal
	}
	
	return(0);
}

int BGBCC_CMGC_CoffBuildImports(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx)
{
	...
	for(i=0; i<nexps; i++)
	{
		...
		if(j>=ndlls)
		{
			...
			lodlls[j]=lobj;
			dllntab[j]=lobj->name;
			dllsytab[j]=BGBCC_CMG_GenLabel(sctx);

			dlliltab[j]=BGBCC_CMG_GenLabel(sctx);
			dlliatab[j]=BGBCC_CMG_GenLabel(sctx);
	...
	BGBCC_CMG_SetSectionName(sctx, ".idata");

	for(i=0; i<ndlls; i++)
	{
		BGBCC_CMG_EmitRelocTy(sctx, dlliltab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_CMG_EmitDWord(sctx, 0);	//import lookup table RVA
		BGBCC_CMG_EmitDWord(sctx, 0);	//time/date

		BGBCC_CMG_EmitDWord(sctx, 0);	//forwarder chain

		BGBCC_CMG_EmitRelocTy(sctx, dllsytab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_CMG_EmitDWord(sctx, 0);	//name RVA

		BGBCC_CMG_EmitRelocTy(sctx, dlliatab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_CMG_EmitDWord(sctx, 0);	//import address table RVA
	}
	...
	for(i=0; i<ndlls; i++)
	{
		k=dllsytab[i];
		BGBCC_CMG_EmitLabel(sctx, k);
		BGBCC_CMG_EmitString(sctx, dllntab[i]);
	}
	...
	for(i=0; i<nexps; i++)
	{
		...
		BGBCC_CMG_EmitBAlign(sctx, 2);
		BGBCC_CMG_EmitLabel(sctx, k);
		BGBCC_CMG_EmitWord(sctx, obj->fxmoffs);		//hint
		BGBCC_CMG_EmitString(sctx, obj->name);		//name
	}
	...
	for(i=0; i<ndlls; i++)
	{
		...
		{
			obj=exptab[j];
			BGBCC_CMG_EmitRelocTy(sctx, obj->fxnalgn, BGBCC_SH_RLC_RVA32);
			BGBCC_CMG_EmitDWord(sctx, 0);	//name
			if(sctx->is_addr64 && !sctx->is_addr_x32)
				BGBCC_CMG_EmitDWord(sctx, 0);
		}
		BGBCC_CMG_EmitDWord(sctx, 0);
		if(sctx->is_addr64 && !sctx->is_addr_x32)
			BGBCC_CMG_EmitDWord(sctx, 0);
	...
	for(i=0; i<ndlls; i++)
	{
		...
		{
			obj=exptab[j];
			BGBCC_CMG_EmitLabel(sctx, obj->fxnoffs);
			BGBCC_CMG_EmitRelocTy(sctx, obj->fxnalgn, BGBCC_SH_RLC_RVA32);
			BGBCC_CMG_EmitDWord(sctx, 0);	//name
			if(sctx->is_addr64 && !sctx->is_addr_x32)
				BGBCC_CMG_EmitDWord(sctx, 0);
...
int BGBCC_CMGC_CoffSectionFlags(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	char *name)
{
...
int bgbcc_cmgc_qrsort(u32 *arr, int cnt, int rd)
{
	int lo, hi, cn, pvt;
	u32 v, v0, v1, v2;
	int i, j, k;
	...
	bgbcc_cmgc_qrsort(arr, lo, rd+1);
	bgbcc_cmgc_qrsort(arr+lo, cnt-lo, rd+1);
	return(0);
}
...
int BGBCC_CMGC_PackBlockLZ4_Reset(BGBCC_TransState *ctx)
{
	int i, j, k;
...
int BGBCC_CMGC_PackBlockLZ4_LookupMatch(
	BGBCC_TransState *ctx,
...
int BGBCC_CMGC_PackBlockLZ4_HashByte(
...
int BGBCC_CMGC_PackBlockLZ4_HashString(
	BGBCC_TransState *ctx, byte *cs, int sz)
{
	int i;
	
	for(i=0; i<sz; i++)
		BGBCC_CMGC_PackBlockLZ4_HashByte(ctx, cs+i);
	return(0);
}

int BGBCC_CMGC_PackBlockLZ4(BGBCC_TransState *ctx,
	byte *obuf, byte *ibuf, int obsz, int ibsz)
{
	...
	while((ct<cte) && (cs<cse))
	{
		i0=BGBCC_CMGC_PackBlockLZ4_LookupMatch(ctx, cs, cse, &bl, &bd);
		if(i0>0)
		{
			i1=BGBCC_CMGC_PackBlockLZ4_LookupMatch(ctx, cs+1, cse, &bl1, &bd1);
			if(bl1>(bl+1))
				i0=0;
		}
		...
		if(i0>0)
		{
			...
			*(u16 *)ct=bd;
			...
			BGBCC_CMGC_PackBlockLZ4_HashString(ctx, cs, bl);
			cs+=bl;
		...
		{
			BGBCC_CMGC_PackBlockLZ4_HashByte(ctx, cs);
			cs++;
		}
	}
...
int BGBCC_CMGC_PackImagePEL4(BGBCC_TransState *ctx,
	byte *obuf, byte *ibuf, int obsz, int ibsz)
{
	byte *cs, *ct, *cse, *cte;
	int i;
	
	BGBCC_CMGC_PackBlockLZ4_Reset(ctx);
	...
	while(cs<cse)
	{
		i=BGBCC_CMGC_PackBlockLZ4(ctx, ct, cs, 1024, cse-cs);
		ct+=1024;
		cs+=i;
	}
...
int BGBCC_CMGC_VerifyImagePEL4(BGBCC_TransState *ctx,
	byte *obuf, byte *ibuf, int obsz, int ibsz)
{
	byte *tbuf;
	byte *cs, *ct, *cse;
...
ccxl_status BGBCC_CMGC_FlattenImagePECOFF(BGBCC_TransState *ctx,
...
//	is_pel4=0;
	...
	BGBCC_CMGC_CoffBuildExports(ctx, sctx);
	BGBCC_CMGC_CoffBuildImports(ctx, sctx);
...
//	img_base=0x0C000000;
	...
	bgbcc_cmgc_qrsort(rlctab, nrlce, 0);

	BGBCC_CMG_SetSectionName(sctx, ".reloc");
	BGBCC_CMG_EmitBAlign(sctx, 4);
	ofsrlc=BGBCC_CMG_EmitGetOffs(sctx);
	BGBCC_CMG_EmitRawBytes(sctx, NULL, szrlc+256);

	BGBCC_CMG_EmitGetStrtabLabel(sctx, "");
	...
	for(i=0; i<sctx->nsec; i++)
	{
		...
		if(s0)
		{
			BGBCC_CMG_EmitGetStrtabLabel(sctx, s0);
		}else
		{
			switch(i)
			{
	...
	lb_strt=0;
	if(lb_strt<=0)
		lb_strt=BGBCC_CMG_LookupNamedLabel(sctx, "_start");
	if(lb_strt<=0)
		lb_strt=BGBCC_CMG_LookupNamedLabel(sctx, "__start");
	
	if(lb_strt>0)
		{ va_strt=BGBCC_CMGC_LookupLabelImgVA(ctx, sctx, lb_strt); }
	else
	{
		printf("BGBCC_CMGC_FlattenImageELF: No Entry Point Found\n");
		va_strt=0;
	}

	ofsimp=0;	szimp=0;
	i=BGBCC_CMG_LookupSectionID(sctx, ".idata");
	if(i>=0)
	{	ofsimp=sctx->sec_rva[i];
		szimp=sctx->sec_lsz[i];		}

	ofsexp=0;	szexp=0;
	i=BGBCC_CMG_LookupSectionID(sctx, ".edata");
	if(i>=0)
	...
	for(i=0; i<sctx->nsec; i++)
	{
		...
		if(s0)
		{
			nm=BGBCC_CMG_EmitGetStrtabSecOfs(sctx, s0);
		...
		k=BGBCC_CMGC_CoffSectionFlags(ctx, sctx, s0);
...
//		imty=3;
	...
	BGBCC_CMGC_ApplyImageRelocs(ctx, sctx, obuf);
	...
	{
		lva=BGBCC_CMGC_LookupLabelImgVA(ctx, sctx, sctx->lbln_id[i]);
		map_lvatab[i]=lva;
	...
	if(is_pel4)
	{
		...
		j=BGBCC_CMGC_PackImagePEL4(ctx, ctb, obuf, ofs_iend*2, ofs_iend);
		ctb[2]='L';
		ctb[3]='4';
		
		BGBCC_CMGC_VerifyImagePEL4(ctx, ctb, obuf, j, ofs_iend);
