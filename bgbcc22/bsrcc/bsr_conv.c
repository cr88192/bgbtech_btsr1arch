int BGBCC_BSRC_EmitConvVRegVReg(
	...
	ccxl_register dreg, ccxl_register sreg)
{
	...
	if(dt==st)
	{
		return(BGBCC_BSRC_EmitMovVRegVReg(ctx, sctx,
			dtype, dreg, sreg));
	}
	...
	if(BGBCC_CCXL_TypePointerP(ctx, dtype))
	{
		...
		if(BGBCC_CCXL_TypeSmallIntP(ctx, stype))
		{
			if(sctx->is_addr64 && (st==CCXL_TY_UI))
			{
				...
				return(BGBCC_BSRC_EmitOpNmidVRegVReg(ctx, sctx, dtype,
...
//				BGBCC_CCXL_TagWarn(ctx, CCXL_TERR_CONV_PTRSIZEDIFF);
		...
		if(BGBCC_CCXL_TypeSmallLongP(ctx, stype))
		{
			...
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_R4);
			return(1);
		}
		...
		((dt==CCXL_TY_NL) && !sctx->is_addr64))
	{
		...
		if(st==CCXL_TY_I)
			{ return(BGBCC_BSRC_EmitMovVRegVReg(ctx, sctx, dtype,
				dreg, sreg)); }
		if(st==CCXL_TY_UI)
			{ return(BGBCC_BSRC_EmitMovVRegVReg(ctx, sctx, dtype,
				dreg, sreg)); }
...
//			BGBCC_BSRC_SetStatusFpscrFloat(ctx, sctx);
			return(BGBCC_BSRC_EmitFpConvVRegVReg(ctx, sctx,
...
//			BGBCC_BSRC_SetStatusFpscrDouble(ctx, sctx);
			...
			sctx->is_addr64)
		{
			...
			if(sctx->has_bjx1egpr)
			{
				...
				BGBCC_BSR_EmitOpRegReg(sctx, nm1, csreg, cdreg);
...
#if 1
			nm1=BGBCC_SH_NMID_EXTSL;
			tr1=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 
				BGBCC_SH_REGCLS_QGR);
			tr0=BGBCC_SH_REG_RD0+(tr1&31);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, tr1);
			BGBCC_BSR_EmitOpReg(sctx, nm1, tr0);
		...
		{
			return(BGBCC_BSRC_ConvLpRegisterNarrowing(ctx, sctx,
...
//			nm1=BGBCC_SH_NMID_EXTSL;
		...
		if((st==CCXL_TY_I128) || (st==CCXL_TY_UI128))
		{
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R4);
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R5);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R5);
			BGBCC_BSR_EmitOpLdRegReg(sctx, BGBCC_SH_NMID_MOVL,
		...
		{
			return(
				BGBCC_BSRC_EmitConvToVRegVRegVariant(ctx, sctx,
					dtype, dreg, sreg));
		}
...
//			{ return(CCXL_STATUS_ERR_UNHANDLEDTYPE); }
		...
		((dt==CCXL_TY_UNL) && !sctx->is_addr64))
	{
		if(BGBCC_CCXL_IsRegImmIntP(ctx, sreg))
		{
			...
			BGBCC_BSRC_EmitMovVRegImm(ctx, sctx, dtype, dreg, (u32)i);
			return(1);
		}
...
//			BGBCC_BSRC_SetStatusFpscrDouble(ctx, sctx);
	...
	if((dt==CCXL_TY_SB) || (dt==CCXL_TY_UB))
	{
		if(BGBCC_CCXL_IsRegImmIntP(ctx, sreg))
		{
			...
			BGBCC_BSRC_EmitMovVRegImm(ctx, sctx, dtype, dreg, i);
			return(1);
		}
...
//			tr0=BGBCC_SH_REG_R4;
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, tr1);
			BGBCC_BSR_EmitOpRegReg(sctx, nm1, tr0, tr0);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, tr0);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr1);
			return(1);
		}
...
//			BGBCC_BSRC_SetStatusFpscrFloat(ctx, sctx);
			...
			BGBCC_BSRC_EmitFpConvVRegVReg(ctx, sctx,
				BGBCC_SH_NMID_FTRC, BGBCC_SH_NMID_STS, dreg, sreg);
			BGBCC_BSRC_EmitOpNmidVRegVReg(ctx, sctx, dtype,
				nm1, dreg, dreg);
			return(1);
		}
		...
		sctx->is_addr64)
	{
			...
			(st==CCXL_TY_UI) || ((st==CCXL_TY_UNL) && !sctx->is_addr64))
		{
			...
			if((nm1==BGBCC_SH_NMID_EXTUL) || (nm1==BGBCC_SH_NMID_EXTSL))
				BGBCC_BSR_EmitOpReg(sctx, nm1, tr1);
			else
				BGBCC_BSR_EmitOpRegReg(sctx, nm1, tr1, tr1);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, tr1);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr1);
			return(1);
		}
		...
		if((st==CCXL_TY_I128) || (st==CCXL_TY_UI128))
		{
			tr1=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 
				BGBCC_SH_REGCLS_QGR);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, tr1);
			BGBCC_BSR_EmitOpLdRegDispReg(sctx,
...
//			((st==CCXL_TY_UNL) && sctx->is_addr64))
		...
		{
			BGBCC_BSRC_ConvLpRegisterWiden(ctx, sctx,
				stype, sreg, dreg);
			return(1);
		}
			...
			(st==CCXL_TY_I) || ((st==CCXL_TY_NL) && !sctx->is_addr64))
		{
			...
			BGBCC_BSRC_EmitLeaSarRegImm(ctx, sctx, BGBCC_SH_REG_R5, 31);
		...
		if((st==CCXL_TY_I128) || (st==CCXL_TY_UI128))
		{
			...
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R6);
...
//			BGBCC_BSRC_SetStatusFpscrDouble(ctx, sctx);
	...
	if(dt==CCXL_TY_VARIANT)
	{
		...
		return(
			BGBCC_BSRC_EmitConvFromVRegVRegVariant(ctx, sctx,
				stype, dreg, sreg));
	}
