int BGBCC_BSRC_NormalizeImmVRegInt(
...
int BGBCC_BSRC_EmitBinaryVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	...
	int opr, ccxl_register treg)
{
	...
	BGBCC_BSRC_NormalizeImmVRegInt(ctx, sctx, type, &treg);
...
//		if(sctx->has_bjx1ari && (nm1<0))
		...
		if(nm1>=0)
		{
			...
			i=BGBCC_BSR_TryEmitOpImmReg(sctx, nm1, k, cdreg);
			if(i<=0)
			{
				ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
				BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, ctreg, k);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, ctreg, cdreg);
		...
		if(nm1>=0)
		{
			...
			i=BGBCC_BSR_TryEmitOpImmReg(sctx, nm3, k, cdreg);
			if(i>0)
			{
				BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
				return(1);
			...
			ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
			BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, ctreg, j);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, ctreg, cdreg);
			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);
			return(1);
		...
		{
			cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
			BGBCC_BSRC_EmitLeaShlRegImm(ctx, sctx, cdreg, j);
			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			return(1);
		}
		...
		if((opr==CCXL_BINOP_SHR) || (opr==CCXL_BINOP_SHRR))
		{
			...
			if(BGBCC_CCXL_TypeUnsignedP(ctx, type) || (opr==CCXL_BINOP_SHRR))
				BGBCC_BSRC_EmitLeaShrRegImm(ctx, sctx, cdreg, j);
			else
				BGBCC_BSRC_EmitLeaSarRegImm(ctx, sctx, cdreg, j);
			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			return(1);
		}
		...
		{
			cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
			BGBCC_BSRC_EmitOpReg(ctx, sctx, nm1, cdreg);
			if(nm2>=0)
				BGBCC_BSRC_EmitOpReg(ctx, sctx, nm2, cdreg);
			if(nm3>=0)
				BGBCC_BSRC_EmitOpReg(ctx, sctx, nm3, cdreg);
			if(nm4>=0)
				BGBCC_BSRC_EmitOpReg(ctx, sctx, nm4, cdreg);
			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			return(1);
		}
...
//			ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
			...
			if(BGBCC_BSR_TryEmitOpRegImmReg(sctx, nm1, cdreg, k, cdreg)>0)
			{
				BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
//				BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);
				return(1);
...
//			k=(s32)((4294967296LL+(j/2))/j);
			...
			if(BGBCC_BSR_EmitCheckRegExt4(sctx, cdreg))
			{
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_STS,
					BGBCC_SH_REG_MACH, ctreg);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MOV,
					ctreg, cdreg);
			}else
			{
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_STS,
...
//			if(j==2)
			if((j>0) && !(j&(j-1)))
			{
				ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
				cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
				BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, ctreg, j-1);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_AND, ctreg, cdreg);
				BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
				BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);
				return(1);
			}
...
//			{ BGBCC_BSRC_CheckSetModeDqClear(ctx, sctx); }
		...
		if(BGBCC_CCXL_RegisterIdentEqualP(ctx, dreg, treg))
		{
			...
			{
				csreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, cdreg, csreg);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, csreg, cdreg);
				BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, csreg);
			}else
			{
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, cdreg, cdreg);
			}
...
//		cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
		...
		{
			csreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, ctreg, csreg);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, csreg, cdreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, csreg);
		}else
		{
...
#if 1
		i=BGBCC_BSR_TryEmitOpRegRegReg(sctx,
...
//		cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
		...
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MULL, ctreg, cdreg);
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_STS,
	...
	if(sn0)
	{
		...
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, BGBCC_SH_REG_R5);
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_R4);

//		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R0);
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R2);

		BGBCC_BSRC_EmitCallName(ctx, sctx, sn0);
...
//		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_R0);
		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_R2);
...
int BGBCC_BSRC_EmitBinaryVRegVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	int opr,
...
//					{ nm1=BGBCC_SH_NMID_SHLD; k=-j; }
			...
			if(nm1>=0)
			{
				...
				i=BGBCC_BSR_TryEmitOpRegImmReg(sctx, nm1, csreg, k, cdreg);
				if(i>0)
				{
					BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
...
//					{ BGBCC_BSRC_CheckSetModeDqClear(ctx, sctx); }
				...
				if((nm1==BGBCC_SH_NMID_AND) && ((k==255) || (k==65535)))				{
					...
					BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, csreg, cdreg);
				...
				ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
				BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, ctreg, k);
				BGBCC_BSR_EmitOpRegRegReg(sctx, nm1, csreg, ctreg, cdreg);
...
//			if(((sbyte)j)==j)
//			{
//				printf("BGBCC_BSRC_EmitBinaryVRegVRegVRegInt: "
...
//			}
		...
		if(!BGBCC_CCXL_RegisterIdentEqualP(ctx, dreg, sreg))
			BGBCC_BSRC_EmitMovVRegVReg(ctx, sctx, type, dreg, sreg);
		i=BGBCC_BSRC_EmitBinaryVRegVRegInt(ctx, sctx, type, dreg, opr, treg);
		return(i);
	}
	...
	if(BGBCC_CCXL_IsRegImmIntP(ctx, sreg))
	{
		if((opr==CCXL_BINOP_ADD) ||
			...
			(opr==CCXL_BINOP_XOR))
		{
			i=BGBCC_BSRC_EmitBinaryVRegVRegVRegInt(ctx, sctx, type,
				dreg, opr, treg, sreg);
			return(i);
		}
	}
...
//				BGBCC_BSRC_CheckSetModeDqClear(ctx, sctx);
			...
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, ctreg, tr0);
			if(csreg!=cdreg)
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MOV, csreg, cdreg);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, tr0, cdreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);

//			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, ctreg, BGBCC_SH_REG_R0);
//			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, BGBCC_SH_REG_R0, cdreg);
		}else if(ctreg==cdreg)
		{
//			if((nm1==BGBCC_SH_NMID_SHAD) || (nm1==BGBCC_SH_NMID_SHLD))
//				BGBCC_BSRC_CheckSetModeDqClear(ctx, sctx);
			...
			tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MOV, ctreg, tr0);
			if(csreg!=cdreg)
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MOV, csreg, cdreg);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, tr0, cdreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
...
//		BGBCC_BSRC_SetModeDqClear(ctx, sctx);
		...
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MULL, ctreg, csreg);
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_STS,
			BGBCC_SH_REG_MACL, cdreg);
	...
	if(opr==CCXL_BINOP_DIV)
	{
		if(sctx->is_addr64)
		{
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RD4);
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RD5);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, BGBCC_SH_REG_RD5);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_RD4);
//			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RD0);
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RD2);
			if(BGBCC_CCXL_TypeUnsignedP(ctx, type))
				{ BGBCC_BSRC_EmitCallName(ctx, sctx, "__udivsi3"); }
			else
				{ BGBCC_BSRC_EmitCallName(ctx, sctx, "__sdivsi3"); }
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RD4);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RD5);
//			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RD0);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RD2);
//			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RD0);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RD2);
		}else
		{
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R4);
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R5);
...
//			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RD0);
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RD2);
			if(BGBCC_CCXL_TypeUnsignedP(ctx, type))
				{ BGBCC_BSRC_EmitCallName(ctx, sctx, "__umodsi3"); }
			else
				{ BGBCC_BSRC_EmitCallName(ctx, sctx, "__smodsi3"); }
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RD4);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RD5);
//			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RD0);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RD2);
...
//			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R0);
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R2);
	//		BGBCC_BSRC_EmitCallName(ctx, sctx, "__moddi3");
			if(BGBCC_CCXL_TypeUnsignedP(ctx, type))
				{ BGBCC_BSRC_EmitCallName(ctx, sctx, "__umodsi3"); }
			else
				{ BGBCC_BSRC_EmitCallName(ctx, sctx, "__smodsi3"); }
...
int BGBCC_BSRC_EmitBinaryVRegVRegVReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	int opr,
	ccxl_register sreg, ccxl_register treg)
{
	if(BGBCC_CCXL_TypeSmallIntP(ctx, type))
	{
		return(BGBCC_BSRC_EmitBinaryVRegVRegVRegInt(ctx, sctx,
	...
	{
		return(BGBCC_BSRC_EmitBinaryVRegVRegVRegSoftFloat(ctx, sctx,
			type, dreg, opr, sreg, treg));
	}
	...
	if(BGBCC_CCXL_TypeSgLongP(ctx, type) &&
		!(sctx->is_addr64))
	{
		return(BGBCC_BSRC_EmitBinaryVRegVRegVRegLong(ctx, sctx,
			type, dreg, opr, sreg, treg));
	}
...
//		(sctx->is_addr64))
//	{
//		return(BGBCC_BSRC_EmitBinaryVRegVRegVRegQLong(ctx, sctx,
//			type, dreg, opr, sreg, treg));
//	}

	if(BGBCC_CCXL_TypeSgInt128P(ctx, type))
	{
		return(BGBCC_BSRC_EmitBinaryVRegVRegVRegInt128(ctx, sctx,
			type, dreg, opr, sreg, treg));
	}

	if(BGBCC_CCXL_TypeVariantP(ctx, type))
	{
		return(BGBCC_BSRC_EmitBinaryVRegVRegVRegVariant(ctx, sctx,
			type, dreg, opr, sreg, treg));
	}
	...
	int nmid, int dreg)
{
	...
	if(!BGBCC_BSR_EmitCheckRegBaseGPR(sctx, dreg))
	{
		...
		BGBCC_BSRC_EmitOpReg(ctx, sctx, nmid, treg);
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx,
	...
	BGBCC_BSRC_EmitOpReg(ctx, sctx, nmid, dreg);
	return(1);
}

int BGBCC_BSRC_EmitUnaryVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	int opr, ccxl_register sreg)
{
	...
	if(BGBCC_CCXL_IsRegImmIntP(ctx, sreg))
	{
		...
		{
			BGBCC_BSRC_EmitMovVRegImm(ctx, sctx, type, dreg, imm);
			return(1);
		}
	...
	if(nm1>=0)
	{
		if(BGBCC_CCXL_RegisterIdentEqualP(ctx, dreg, sreg))
		{
			...
			{
				ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, cdreg, ctreg);
				BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, ctreg, cdreg);
				BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);
			}else
			{
		...
		{
			ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm2, csreg, ctreg);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, nm1, ctreg, cdreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);
		}else
		{
	...
	if(opr==CCXL_UNOP_LNOT)
	{
		...
		{
			cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_TST, cdreg, cdreg);
//			BGBCC_BSRC_EmitOpReg(ctx, sctx, BGBCC_SH_NMID_MOVT, cdreg);
			BGBCC_BSRC_EmitDstRegOp(ctx, sctx, BGBCC_SH_NMID_MOVT, cdreg);
			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			return(1);
		}
...
//		BGBCC_BSRC_CheckSetModeDqClear(ctx, sctx);
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_TST, csreg, csreg);
//		BGBCC_BSRC_EmitOpReg(ctx, sctx, BGBCC_SH_NMID_MOVT, cdreg);
		BGBCC_BSRC_EmitDstRegOp(ctx, sctx, BGBCC_SH_NMID_MOVT, cdreg);
		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, sreg);
		...
		(opr==CCXL_UNOP_DEC))
	{
		if(BGBCC_CCXL_RegisterIdentEqualP(ctx, dreg, sreg))
		{
			...
			cdreg=BGBCC_BSRC_EmitGetRegisterDirty(ctx, sctx, dreg);
			BGBCC_BSR_EmitOpImmReg(sctx, BGBCC_SH_NMID_ADD, j, cdreg);
			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			return(1);
		}
...
int BGBCC_BSRC_EmitUnaryVRegVReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	int opr, ccxl_register sreg)
{
	if(BGBCC_CCXL_TypeSmallIntP(ctx, type))
	{
		return(BGBCC_BSRC_EmitUnaryVRegVRegInt(ctx, sctx,
			type, dreg, opr, sreg));
	}
	...
	if(BGBCC_CCXL_TypeFloatP(ctx, type) ||
		...
		BGBCC_CCXL_TypeDoubleP(ctx, type))
	{
		return(BGBCC_BSRC_EmitUnaryVRegVRegSoftFloat(ctx, sctx,
			type, dreg, opr, sreg));
	}
	...
	if(BGBCC_CCXL_TypeSgLongP(ctx, type) &&
		!(sctx->is_addr64))
	{
		return(BGBCC_BSRC_EmitUnaryVRegVRegLong(ctx, sctx,
			type, dreg, opr, sreg));
	}
...
//		(sctx->is_addr64))
//	{
//		return(BGBCC_BSRC_EmitUnaryVRegVRegQLong(ctx, sctx,
//			type, dreg, opr, sreg));
//	}

	if(BGBCC_CCXL_TypeSgInt128P(ctx, type))
	{
		return(BGBCC_BSRC_EmitUnaryVRegVRegInt128(ctx, sctx,
			type, dreg, opr, sreg));
	}

	if(BGBCC_CCXL_TypeVariantP(ctx, type))
	{
		return(BGBCC_BSRC_EmitUnaryVRegVRegVariant(ctx, sctx,
			type, dreg, opr, sreg));
	}
...
int BGBCC_BSRC_EmitCompareVRegVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//		nm1=-1; // k=0;
		...
		i=BGBCC_BSRC_EmitCompareVRegVRegVRegInt(ctx, sctx,
...
//		if(!BGBCC_BSR_EmitCheckRegBaseGPR(sctx, cdreg))
		if(0)
		{
			tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 0);
			BGBCC_BSRC_EmitOpReg(ctx, sctx, BGBCC_SH_NMID_MOVT, tr0);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_MOV,
				tr0, cdreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
		}else
...
//			BGBCC_BSRC_EmitOpReg(ctx, sctx, BGBCC_SH_NMID_MOVT, cdreg);
		...
		{
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx, BGBCC_SH_NMID_NEG, cdreg, cdreg);
			BGBCC_BSR_EmitOpImmReg(sctx, BGBCC_SH_NMID_ADD, 1, cdreg);
		}
...
int BGBCC_BSRC_EmitCompareVRegVRegVReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	int opr,
	ccxl_register sreg, ccxl_register treg)
{
	if(BGBCC_CCXL_TypeSmallIntP(ctx, type))
	{
		return(BGBCC_BSRC_EmitCompareVRegVRegVRegInt(ctx, sctx,
			type, dreg, opr, sreg, treg));
	}
...
//		sctx->is_addr64)
//	{
//		return(BGBCC_BSRC_EmitCompareVRegVRegVRegQLong(ctx, sctx,
//			type, dreg, opr, sreg, treg));
//	}
...
//		BGBCC_CCXL_TypeDoubleP(ctx, type))
//	{
//		return(BGBCC_BSRC_EmitCompareVRegVRegVRegFloat(ctx, sctx,
//			type, dreg, opr, sreg, treg));
//	}
	...
	BGBCC_BSR_Context *sctx)
{
	...
	i=BGBCC_BSR_EmitOpAutoLabel(sctx,
		BGBCC_SH_NMID_BRAN, sctx->lbl_ret);
//	BGBCC_BSRC_EmitOpNone(ctx, sctx,
//		BGBCC_SH_NMID_NOP);
	return(i);
}

int BGBCC_BSRC_EmitReturnVReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//	{
//		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R0);
//	}
	...
	{
		if(sctx->is_addr64)
//			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_RD0);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_RD2);
		else
//			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R0);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R2);
	}else if(
		(rcls==BGBCC_SH_REGCLS_VO_REF) ||
		(rcls==BGBCC_SH_REGCLS_AR_REF))
	{
		if(sctx->is_addr64)
//			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_RQ0);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_RQ2);
		else
//			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R0);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_R2);
	}else if(rcls==BGBCC_SH_REGCLS_GR2)
	{
//		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_LR0);
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_LR2);
	}else if(rcls==BGBCC_SH_REGCLS_FR)
	{
//		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_FR0);
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_FR2);
	}else if(rcls==BGBCC_SH_REGCLS_FR2)
	{
//		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_DR0);
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, BGBCC_SH_REG_DR2);
	}else if(rcls==BGBCC_SH_REGCLS_VO_GR)
	{
		BGBCC_BSRC_EmitLoadFrameVRegByValReg(ctx, sctx,
...
//			sreg, BGBCC_SH_REG_RQ0);
	...
	{
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx,
//			sreg, BGBCC_SH_REG_RQ0);
			sreg, BGBCC_SH_REG_RQ2);
//		BGBCC_BSRC_EmitLoadFrameVRegByValReg(ctx, sctx,
//			sreg, BGBCC_SH_REG_RQ0);
...
//		if(sctx->frm_size>56)
		...
		{
//			ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
//			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RQ3);
...
//			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RQ3);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_R2);
			return(1);
		}
#endif
...
//		if(1)
		...
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, ctreg);
		...
		sctx->is_addr64)
	{
			...
			BGBCC_CCXL_IsRegGlobalP(ctx, dreg))
		{
			...
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RQ3);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RQ3);
			return(1);
		}
#endif
...
//		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RQ0);
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RQ2);
//		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RQ0);
		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_RQ2);
//		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RQ0);
...
//		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_FR0);
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_FR2);
//		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_FR0);
		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_FR2);
//		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_FR0);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_FR2);
		return(1);
	}
...
//		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_DR0);
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_DR2);
//		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_DR0);
		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, BGBCC_SH_REG_DR2);
//		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_DR0);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_DR2);
		return(1);
	}
...
//			ctreg=BGBCC_SH_REG_RD0;
		...
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, ctreg);
//		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, ctreg);
		BGBCC_BSRC_EmitStoreFrameVRegByValReg(ctx, sctx, dreg, ctreg);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);
		return(1);
	}
...
int BGBCC_BSRC_EmitCallBuiltin(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	char *name)
{
	...
	{
		l0=BGBCC_BSR_GenLabelTemp(sctx);
		l1=BGBCC_BSR_GenLabelTemp(sctx);
		...
		BGBCC_BSR_EmitOpLabel(sctx, BGBCC_SH_NMID_BT, l1);
		BGBCC_BSRC_EmitOpRegReg(ctx, sctx,
			BGBCC_SH_NMID_CMPEQ, BGBCC_SH_REG_R2, BGBCC_SH_REG_R3);
		BGBCC_BSR_EmitOpLabel(sctx, BGBCC_SH_NMID_BT, l0);
...
int BGBCC_BSRC_EmitCallBuiltinArgs(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type,
	ccxl_register dst,
	...
	int narg, ccxl_register *args)
{
	...
	{
		BGBCC_BSRC_EmitOpNone(ctx, sctx, BGBCC_SH_NMID_BRK);
		return(1);
	}

	if(!strcmp(name, "__halt"))
	{
		BGBCC_BSRC_EmitOpNone(ctx, sctx, BGBCC_SH_NMID_SLEEP);
		return(1);
	}
	...
	{
		csreg=BGBCC_BSRC_EmitGetRegisterRead(ctx, sctx, args[0]);
		cdreg=BGBCC_BSRC_EmitGetRegisterWrite(ctx, sctx, dst);
		BGBCC_BSRC_EmitMovRegReg(ctx, sctx, csreg, cdreg);
		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dst);
		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, args[0]);
		return(1);
	}
...
//		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, args[0], tr0);
//		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, args[1], tr1);

		tr0=BGBCC_BSRC_EmitGetRegisterRead(ctx, sctx, args[0]);
		tr1=BGBCC_BSRC_EmitGetRegisterRead(ctx, sctx, args[1]);
		BGBCC_BSRC_EmitValueCopyRegRegSz(ctx, sctx, tr0, tr1, sz, al);
		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, args[0]);
		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, args[1]);
...
//				l0=BGBCC_BSR_EmitGetStrtabLabel(sctx, s0);
				sz=strlen(s0);
				sz=(sz+4)&(~3);
				al=4;
...
int BGBCC_BSRC_EmitCallFcnVReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register fcn)
{
...
//		BGBCC_BSR_EmitLoadRegLabelVarRel24(sctx, BGBCC_SH_NMID_JSR,
//			BGBCC_SH_REG_R3, k);
		BGBCC_BSR_EmitOpAutoLabel(sctx,
			BGBCC_SH_NMID_BSR, k);
//		BGBCC_BSRC_ResetModeDqUnknown(ctx, sctx);
	...
	BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx,
		fcn, BGBCC_SH_REG_R3);
	i=BGBCC_BSRC_EmitOpReg(ctx, sctx,
...
//	BGBCC_BSRC_SetModeDqClear(ctx, sctx);
	...
	sctx->sreg_live&=~1;
	BGBCC_BSRC_EmitLoadGblNameReg(ctx, sctx,
...
//	}
	...
	retrcls=BGBCC_BSRC_TypeGetRegClassP(ctx, tty);
...
//		BGBCC_BSRC_EmitLdaFrameVRegReg(ctx, sctx,
//			dst, BGBCC_SH_REG_R2);
...
//		}
		...
		if(sctx->is_addr64)
		{
			BGBCC_BSRC_EmitLdaValSyncFrameVRegReg(ctx, sctx,
...
//			BGBCC_CCXL_TypeValueObjectP(ctx, tty))
			...
			(rcls==BGBCC_SH_REGCLS_WGR))
		{
			if(sctx->is_addr64)
			{
				...
				{
					tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 
						BGBCC_SH_REGCLS_GR);
					BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, tr0);
					BGBCC_BSRC_EmitStoreStackOfsReg(ctx, sctx, ns*4, tr0);
					BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
					ns++;
					continue;
				}
		...
		if(rcls==BGBCC_SH_REGCLS_VO_GR)
		{
			if(sctx->is_addr64)
			{
				...
				}else
				{
					tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 
						BGBCC_SH_REGCLS_GR);
					BGBCC_BSRC_EmitLoadFrameVRegByValReg(ctx, sctx, treg, tr0);
					BGBCC_BSRC_EmitStoreStackOfsReg(ctx, sctx, ns*4, tr0);
					BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
					ns++;
					continue;
...
//				tr0=BGBCC_SH_REG_DR0;
				tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_FR2);
				BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, tr0);
				BGBCC_BSRC_EmitStoreStackOfsReg(ctx, sctx, ns*4, tr0);
				BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
				ns+=2;
	...
	if(fname && BGBCC_BSRC_EmitCallBuiltin(ctx, sctx, fname))
	{
		sctx->sreg_live&=~0x00F000F0;
		sctx->sfreg_live&=~0x0FF0;
		return(1);
...
//	BGBCC_BSRC_SetModeDqClear(ctx, sctx);
	...
	BGBCC_BSRC_EmitCallFcnVReg(ctx, sctx, fcn);
...
int BGBCC_BSRC_EmitCSeltSelectVRegVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	ccxl_register sreg, ccxl_register treg)
{
	...
	if(1)
	{
		...
		BGBCC_BSRC_EmitOpRegRegReg(ctx, sctx,
...
int BGBCC_BSRC_EmitCSeltCompareVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//		BGBCC_BSRC_CheckSetModeDqClear(ctx, sctx);
		...
		if(nm2==BGBCC_SH_NMID_BF)
			BGBCC_BSRC_EmitOpNone(ctx, sctx, BGBCC_SH_NMID_NOTT);
...
int BGBCC_BSRC_EmitCSeltVRegVRegVRegInt(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
		ccxl_register sreg, ccxl_register treg,
		...
		ccxl_register ureg, ccxl_register vreg)
{
	...
	{
		BGBCC_BSRC_EmitCSeltCompareVRegVRegInt(ctx, sctx,
			ptype, cmp, ureg, vreg);
		BGBCC_BSRC_EmitCSeltSelectVRegVRegVRegInt(ctx, sctx,
			type, dreg, sreg, treg);
		return(1);
	}
