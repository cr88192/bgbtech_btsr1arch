...
int BGBCC_BSRC_EmitLoadFrameOfsReg(
...
//	if(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg) ||
//		BGBCC_BSRC_EmitRegIsDpReg(ctx, sctx, dreg))
//		{ return(BGBCC_BSRC_EmitLoadFrameOfsFpReg(ctx, sctx, ofs, dreg)); }

	ofs1=ofs+(sctx->frm_size);

	if(BGBCC_BSRC_EmitRegIsLpReg(ctx, sctx, dreg))
	{
		dreg2=BGBCC_SH_REG_R0+(dreg&15);
	
		BGBCC_BSR_EmitOpLdRegDispReg(sctx, BGBCC_SH_NMID_MOVL,
	...
	BGBCC_BSR_EmitOpLdRegDispReg(sctx, nmid,
		BGBCC_SH_REG_SP, ofs1, dreg);
	return(1);
}

int BGBCC_BSRC_EmitStoreFrameOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int ofs, int dreg)
{
...
//	if(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg) ||
//		BGBCC_BSRC_EmitRegIsDpReg(ctx, sctx, dreg))
//		{ return(BGBCC_BSRC_EmitStoreFrameOfsFpReg(ctx, sctx, ofs, dreg)); }
	...
	if(BGBCC_BSRC_EmitRegIsLpReg(ctx, sctx, dreg))
	{
		...
		BGBCC_BSR_EmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVL,
	...
	BGBCC_BSR_EmitOpRegStRegDisp(sctx, nmid,
		dreg, BGBCC_SH_REG_SP, ofs1);
	return(1);
}

int BGBCC_BSRC_EmitLdaFrameOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int ofs, int dreg)
{
...
int BGBCC_BSRC_EmitStoreStackOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int ofs, int dreg)
{
...
//	if(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg) ||
//		BGBCC_BSRC_EmitRegIsDpReg(ctx, sctx, dreg))
//		{ return(BGBCC_BSRC_EmitStoreStackOfsFpReg(ctx, sctx, ofs, dreg)); }
	...
	if(BGBCC_BSRC_EmitRegIsLpReg(ctx, sctx, dreg))
	{
		...
		p0=BGBCC_BSR_TryEmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVL,
			dreg2+1, BGBCC_SH_REG_SP, ofs1+4);
		if(p0>0)
		{
			BGBCC_BSR_EmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVL,
		...
		BGBCC_BSR_EmitOpRegStReg2(sctx, BGBCC_SH_NMID_MOVL,
			dreg2+0, BGBCC_SH_REG_SP, BGBCC_SH_REG_R0);
		BGBCC_BSR_EmitOpImmReg(sctx, BGBCC_SH_NMID_ADD,
			4, BGBCC_SH_REG_R0);
		BGBCC_BSR_EmitOpRegStReg2(sctx, BGBCC_SH_NMID_MOVL,
	...
	BGBCC_BSR_TryEmitOpRegStRegDisp(sctx, nmid,
		dreg, BGBCC_SH_REG_SP, ofs1);
	return(1);
}


int BGBCC_BSRC_EmitLoadBRegOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int nmid, int breg, int ofs, int dreg)
{
	return(BGBCC_BSRC_EmitLoadOp2BRegOfsReg(
		ctx, sctx, nmid, -1, breg, ofs, dreg));
}

int BGBCC_BSRC_EmitLoadOp2BRegOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//	if(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg) ||
//		BGBCC_BSRC_EmitRegIsDpReg(ctx, sctx, dreg))
//	{	return(BGBCC_BSRC_EmitLoadBRegOfsFpReg(ctx, sctx,
//			nmid, breg, ofs, dreg));	}
...
//		p0=BGBCC_BSR_TryEmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVL,
//			dreg2+1, breg, ofs+4);
		p0=BGBCC_BSR_TryEmitOpLdRegDispReg(sctx, BGBCC_SH_NMID_MOVL,
			breg, ofs+4, dreg2+1);
		if(p0>0)
		{
//			BGBCC_BSR_EmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVL,
//				dreg2+0, breg, ofs+0);
		...
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
			BGBCC_SH_REG_R0, ofs);
		BGBCC_BSR_EmitOpLdReg2Reg(sctx, BGBCC_SH_NMID_MOVL,
	...
	BGBCC_BSR_EmitOpLdRegDispReg(sctx, nmid,
		breg, ofs, dreg);
	if(nmid2>=0)
		{ BGBCC_BSR_EmitOpRegReg(sctx, nmid2, dreg, dreg); }
	return(1);
}

int BGBCC_BSRC_EmitStoreBRegOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int nmid, int breg, int ofs, int dreg)
{
	...
	if(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg))
	{
		if(ofs==0)
		{
			BGBCC_BSR_EmitOpRegStReg(sctx, BGBCC_SH_NMID_FMOVS,
				dreg, breg);
			return(1);
		}

		p0=BGBCC_BSR_TryEmitOpRegStRegDisp(sctx, nmid,
			dreg, breg, ofs);
		if(p0>0)
			return(1);
		...
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
			BGBCC_SH_REG_R0, ofs);
		BGBCC_BSR_EmitOpRegStReg2(sctx, BGBCC_SH_NMID_FMOVS,
...
#if 0
	if(BGBCC_BSRC_EmitRegIsDpReg(ctx, sctx, dreg))
	{
...
int BGBCC_BSRC_EmitLeaBRegOfsReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int nmid, int breg, int ofs, int dreg)
{
...
//	p0=BGBCC_BSR_EmitOpLdRegDispReg(sctx, nmid,
//		breg, ofs, dreg);
//	if(p0>0)
//		return(1);
	...
	{
		if(breg!=dreg)
			BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_MOV, breg, dreg);
		return(1);
	}
...
//	if((ctx->arch_sizeof_ptr==8) || (breg!=dreg))
	if(sctx->is_addr64 || (breg!=dreg))
	{
		...
		i=BGBCC_BSR_TryEmitOpRegImmReg(sctx,
			opw1, breg, k, dreg);
		if(i>0)
			return(1);
...
//		BGBCC_DBGBREAK
	...
	if(breg!=dreg)
	{
		...
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, dreg, ofs);
		BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD, breg, dreg);
	}else
	{	
		i=BGBCC_BSR_TryEmitOpImmReg(sctx, BGBCC_SH_NMID_ADD, ofs, dreg);
		if(i>0)
			return(1);
	
		ctreg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, ctreg, ofs);
		BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD, ctreg, dreg);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, ctreg);

//		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R3);
//		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, BGBCC_SH_REG_R3, ofs);
//		BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD, BGBCC_SH_REG_R3, dreg);
//		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R3);
	}
...
int BGBCC_BSRC_CheckNmidScaleMatch(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int nmid, int sc)
{
...
int BGBCC_BSRC_EmitLoadBRegIRegScReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//		BGBCC_BSRC_EmitRegIsGpReg(ctx, sctx, dreg) &&
		BGBCC_BSRC_EmitRegIsExtGpReg(ctx, sctx, dreg) &&
//		BGBCC_BSR_EmitCheckRegExtGPR(sctx, dreg) &&
		BGBCC_BSRC_CheckNmidScaleMatch(ctx, sctx, nmid, sc))
	{
		i=BGBCC_BSR_TryEmitOpLdReg2Reg(sctx, nmid,
			breg, ireg, dreg);
		if(i>0)
			return(1);
	}
	...
	BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_MOV,
		ireg, BGBCC_SH_REG_R0);
	if(nm1>=0)
		BGBCC_BSR_EmitOpReg(sctx, nm1, BGBCC_SH_REG_R0);
	if(nm2>=0)
		BGBCC_BSR_EmitOpReg(sctx, nm2, BGBCC_SH_REG_R0);
	...
	{
//		dreg2=BGBCC_SH_REG_R0+(dreg&15);
		dreg2=BGBCC_BSRC_RegNormalizeReg(ctx, sctx, dreg);
//		sw=BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg2);
//		sw=(sctx->is_le!=0);
		sw=(sctx->is_le!=0)^(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg2)==0);

		BGBCC_BSR_EmitOpLdReg2Reg(sctx, nmid,
...
int BGBCC_BSRC_EmitStoreBRegIRegScReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int nmid, int breg, int ireg, int sc, int dreg)
{
...
//		BGBCC_BSR_EmitCheckRegExtGPR(sctx, dreg) &&
		BGBCC_BSRC_CheckNmidScaleMatch(ctx, sctx, nmid, sc))
	{
		i=BGBCC_BSR_TryEmitOpRegStReg2(sctx, nmid,
			dreg, ireg, breg);
		if(i>0)
			return(1);
	}
...
//		sw=(sctx->is_le!=0);
		...
		BGBCC_BSR_EmitOpRegStReg2(sctx, nmid,
...
#if 1
int BGBCC_BSRC_EmitLoadBRegIRegScDispReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//		BGBCC_BSRC_EmitRegIsGpReg(ctx, sctx, dreg) &&
		...
		BGBCC_BSRC_CheckNmidScaleMatch(ctx, sctx, nmid, sc))
	{
		i=BGBCC_BSR_TryEmitOpLdReg2DispReg(sctx, nmid,
			breg, ireg, disp, dreg);
		if(i>0)
			return(1);
	}
...
int BGBCC_BSRC_EmitStoreBRegIRegScDispReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int nmid, int breg, int ireg, int sc, int disp, int dreg)
{
...
//		BGBCC_BSRC_EmitRegIsGpReg(ctx, sctx, dreg) &&
		...
		BGBCC_BSRC_CheckNmidScaleMatch(ctx, sctx, nmid, sc))
	{
		i=BGBCC_BSR_TryEmitOpRegStReg2Disp(sctx, nmid,
			dreg, ireg, breg, disp);
		if(i>0)
			return(1);
	}
...
int BGBCC_BSRC_GetGblIndexGotIndex(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int gblidx)
{
...
int BGBCC_BSRC_GetGblIndexLabel(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	int gblidx)
{
	...
	if(i<=0)
	{
//		i=BGBCC_BSR_GenLabel(sctx);
		i=BGBCC_BSR_GetNamedLabel(sctx, bgbcc_strtab_i(gbl->name_ix));
		gbl->fxoffs=i;
	}
	return(i);
}

int BGBCC_BSRC_GetVRegGotIndex(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg)
{
	...
	i=BGBCC_BSRC_GetGblIndexGotIndex(ctx, sctx, j);
	return(i);
}

int BGBCC_BSRC_GetGblNameGotIndex(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	char *name)
{
	...
	if(k<=0)
	{
//		k=BGBCC_BSR_GenLabel(sctx);
		k=BGBCC_BSR_GetNamedLabel(sctx, bgbcc_strtab_i(gbl->name_ix));
		gbl->fxoffs=k;
	}
...
int BGBCC_BSRC_GetGblNameLabel(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	char *name)
{
...
int BGBCC_BSRC_EmitLoadGblNameReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	char *name, int dreg)
{
	...
	if(sctx->is_pic)
	{
		k=BGBCC_BSRC_GetGblNameGotIndex(ctx, sctx, name);
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R0);
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
			BGBCC_SH_REG_R0, k*4);
		BGBCC_BSR_EmitOpLdReg2Reg(sctx, BGBCC_SH_NMID_MOVL,
	...
	{
		k=BGBCC_BSRC_GetGblNameLabel(ctx, sctx, name);
		if(sctx->is_rom)
			BGBCC_BSR_EmitLoadRegLabelAbs(sctx, dreg, k);
		else
			BGBCC_BSR_EmitLoadRegLabelRel24(sctx, dreg, k);
		return(1);
	}
...
int BGBCC_BSRC_EmitLoadFrameVRegByValReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg, int dreg)
{
	...
	if(BGBCC_CCXL_IsRegLocalP(ctx, sreg))
	{
		...
		rcls=ctx->cur_func->ext->locals[j]->regcls;
		ctx->cur_func->ext->locals[j]->regflags|=BGBCC_REGFL_ACCESSED;
		...
		j=sreg.val&CCXL_REGID_BASEMASK;
		k=ctx->cur_func->ext->locals[j]->fxoffs;
	...
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		rcls=ctx->cur_func->ext->regs[j]->regcls;
...
//		k=ctx->cur_func->ext->regs[j]->fxoffs;

		tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 0);
		BGBCC_BSRC_EmitLoadFrameVRegReg(ctx, sctx, sreg, tr0);
	...
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		rcls=ctx->cur_func->ext->args[j]->regcls;
...
#if 1
		j=sreg.val&CCXL_REGID_BASEMASK;
		k=ctx->cur_func->ext->args[j]->fxoffs;
		i=BGBCC_BSRC_EmitLoadFrameOfsReg(ctx, sctx, k, dreg);
#endif
...
int BGBCC_BSRC_EmitStoreFrameVRegByValReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg, int dreg)
{
	...
	if(BGBCC_CCXL_IsRegLocalP(ctx, sreg))
	{
		...
		j=sreg.val&CCXL_REGID_BASEMASK;
		k=ctx->cur_func->ext->locals[j]->fxoffs;
		i=BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, k, dreg);
		return(i);
	}
...
int BGBCC_BSRC_EmitLoadFrameVRegReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg, int dreg)
{
	...
	if(BGBCC_CCXL_IsRegLocalP(ctx, sreg))
	{
		...
		tty=ctx->cur_func->ext->locals[j]->type;
...
//				{ k=-1; }
			...
			k=(ctx->cur_func->ext->locals[j]->fxmoffs)+(sctx->frm_offs_fix);
			i=BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, dreg);
			return(i);
		}
...
//		if(BGBCC_CCXL_TypeArrayP(ctx, tty) ||
//			BGBCC_CCXL_TypeValueObjectP(ctx, tty))
		if(ctx->cur_func->ext->locals[j]->regcls==BGBCC_SH_REGCLS_VO_REF)
		{
			k=(ctx->cur_func->ext->locals[j]->fxmoffs)+(sctx->frm_offs_fix);
			i=BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, dreg);
			return(i);
		...
		if((ctx->cur_func->ext->locals[j]->regcls==BGBCC_SH_REGCLS_VO_GR) ||
			(ctx->cur_func->ext->locals[j]->regcls==BGBCC_SH_REGCLS_VO_GR2) ||
			(ctx->cur_func->ext->locals[j]->regcls==BGBCC_SH_REGCLS_VO_QGR2) ||
			(ctx->cur_func->ext->locals[j]->regcls==BGBCC_SH_REGCLS_VO_QGR))
		{
			k=ctx->cur_func->ext->locals[j]->fxoffs;
			i=BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, dreg);
			return(i);
			...
			!BGBCC_CCXL_TypeSgIntP(ctx, tty))
		{
			...
			BGBCC_BSRC_EmitLoadTypeBRegOfsReg(ctx, sctx,
...
//		k=sctx->frm_offs_lcl+j*4;
	...
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		tty=ctx->cur_func->ext->regs[j]->type;
		if(	BGBCC_CCXL_TypeFloatP(ctx, tty) ||
			BGBCC_CCXL_TypeDoubleP(ctx, tty))
		{
			k=1;
...
//		k=sctx->frm_offs_tmp+j*4;
	...
	if(BGBCC_CCXL_IsRegArgP(ctx, sreg))
	{
		...
		if((ctx->cur_func->ext->args[j]->regcls==BGBCC_SH_REGCLS_VO_GR) ||
			(ctx->cur_func->ext->args[j]->regcls==BGBCC_SH_REGCLS_VO_GR2) ||
			(ctx->cur_func->ext->args[j]->regcls==BGBCC_SH_REGCLS_VO_QGR2) ||
			(ctx->cur_func->ext->args[j]->regcls==BGBCC_SH_REGCLS_VO_QGR))
		{
			k=ctx->cur_func->ext->args[j]->fxoffs;
			i=BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, dreg);
			return(i);
...
#if 0
		tty=ctx->cur_func->ext->args[j]->type;
		if(BGBCC_CCXL_TypeSmallIntP(ctx, tty) &&
			!BGBCC_CCXL_TypeSgIntP(ctx, tty))
		{
			k=ctx->cur_func->ext->args[j]->fxoffs;
	...
	if(BGBCC_CCXL_IsRegImmIntP(ctx, sreg))
	{
		if(BGBCC_BSRC_EmitRegIsFpReg(ctx, sctx, dreg))
		{
			...
			*(f32 *)(&k)=j;
			...
			BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
				BGBCC_SH_REG_R0, k);
			BGBCC_BSRC_EmitMovRegReg(ctx, sctx, BGBCC_SH_REG_R0, dreg);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R0);
//			i=BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, dreg, j);
			return(1);
		}
...
//			dreg2=BGBCC_SH_REG_FR0+((dreg&7)*2);
			...
			j=(*(u64 *)(&f))>>32;
			k=(*(u64 *)(&f));
			BGBCC_BSR_EmitLoadRegImmPair32(sctx, dreg2, k, j);
//			BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
//				BGBCC_SH_REG_R0, j);
//			BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
//				BGBCC_SH_REG_R1, k);
//			BGBCC_BSRC_EmitMovRegReg(ctx, sctx, BGBCC_SH_REG_R0, dreg2+0);
//			BGBCC_BSRC_EmitMovRegReg(ctx, sctx, BGBCC_SH_REG_R1, dreg2+1);
			return(1);
		}

		if(BGBCC_BSRC_EmitRegIsLpReg(ctx, sctx, dreg) ||
			BGBCC_BSR_EmitCheckRegQuad(sctx, dreg))
		{
			li=BGBCC_CCXL_GetRegImmLongValue(ctx, sreg);
			BGBCC_BSR_EmitLoadRegImm64P(sctx, dreg, li);

//			j=BGBCC_CCXL_GetRegImmIntValue(ctx, sreg);
//			k=j>>31;
//			BGBCC_BSR_EmitLoadRegImmPair32(sctx, dreg, j, k);
			return(1);
		}
	
		j=BGBCC_CCXL_GetRegImmIntValue(ctx, sreg);
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, dreg, j);
		return(1);
	}
...
//			i=BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
//				BGBCC_SH_REG_R0, k);
//			BGBCC_BSRC_EmitMovRegReg(ctx, sctx, BGBCC_SH_REG_R0, dreg);
			BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, dreg, k);
			return(1);
		}
...
//		k=BGBCC_BSR_EmitGetStrtabLabel(sctx, s0);
		...
		}else
		{
			...
			{
//				if(!BGBCC_BSR_EmitCheckRegBaseGPR(sctx, dreg))
				if(0)
				{
					treg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 0);
					BGBCC_BSR_EmitLoadRegLabelRel24(sctx, treg, k);
					BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_MOV,
						treg, dreg);
...
//		k=BGBCC_BSR_EmitGetStrtabLabel(sctx, s0);
	...
	if(BGBCC_CCXL_IsRegGlobalP(ctx, sreg))
	{
		...
		{
			treg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 0);
			BGBCC_BSRC_EmitLoadFrameVRegReg(ctx, sctx, sreg, treg);
			BGBCC_BSRC_EmitLoadBRegOfsReg(ctx, sctx,
...
//			!sctx->is_rom &&
//			BGBCC_BSRC_ScratchCheckRegFree(ctx, sctx,
//				BGBCC_SH_REG_R0) &&
//			BGBCC_BSR_EmitCheckRegBaseGPR(sctx, dreg))
		if(1)
		{
			if(BGBCC_CCXL_TypeSgIntP(ctx, tty))
			{
				j=BGBCC_CCXL_GetRegID(ctx, sreg);
				k=BGBCC_BSRC_GetGblIndexLabel(ctx, sctx, j);
				BGBCC_BSR_EmitLoadRegLabelVarRel24(sctx,
...
//			BGBCC_BSRC_EmitRegIsLpReg(ctx, sctx, dreg)	)
		if(!BGBCC_BSR_EmitCheckRegAddrGPR(sctx, dreg))
		{
			treg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 0);
		}
...
//		if(sctx->is_pic)
		...
		{
			j=BGBCC_CCXL_GetRegID(ctx, sreg);
			k=BGBCC_BSRC_GetGblIndexGotIndex(ctx, sctx, j);
...
//			BGBCC_BSR_EmitLoadRegLabelAbs(sctx, dreg, k);
			if(sctx->is_rom)
				BGBCC_BSR_EmitLoadRegLabelAbs(sctx, treg, k);
			else
				BGBCC_BSR_EmitLoadRegLabelRel24(sctx, treg, k);
		}
...
//				{ BGBCC_DBGBREAK }
		...
		{
//			p1=BGBCC_BSR_EmitOpLdRegReg(sctx, BGBCC_SH_NMID_MOVL,
//				dreg, dreg);
...
//			p1=BGBCC_BSR_EmitOpLdRegReg(sctx, BGBCC_SH_NMID_FMOVS,
//				dreg, dreg);
...
//				BGBCC_SH_NMID_FMOVS, dreg, 0, dreg);
		...
		BGBCC_CCXL_IsRegImmFloat128P(ctx, sreg))
	{
		...
		k=BGBCC_BSRC_IndexLitInt128(ctx, sctx, li, lj);
...
int BGBCC_BSRC_EmitStoreFrameVRegReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
//		if(BGBCC_CCXL_TypePointerP(ctx, ctx->cur_func->ext->locals[j]->type))
//			BGBCC_BSR_EmitOpMReg(sctx, BGBCC_SH_NMID_PREF, sreg);
		BGBCC_BSRC_EmitDebugCheckReg(ctx, sctx,
			ctx->cur_func->ext->locals[j]->type, sreg);
#endif

		if(BGBCC_CCXL_TypeValueObjectP(ctx, ctx->cur_func->ext->locals[j]->type))
		{
//			cdreg=BGBCC_BSRC_EmitGetRegisterRead(ctx, sctx, dreg);
			treg=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, 0);
			tsz=BGBCC_CCXL_TypeGetLogicalSize(ctx,
				ctx->cur_func->ext->locals[j]->type);

			k=(ctx->cur_func->ext->locals[j]->fxmoffs)+(sctx->frm_offs_fix);
			if(ctx->cur_func->ext->locals[j]->regcls!=BGBCC_SH_REGCLS_VO_REF)
				k=ctx->cur_func->ext->locals[j]->fxoffs;

			i=BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, treg);

			BGBCC_BSRC_EmitValueCopyRegRegSz(ctx, sctx, treg, sreg, tsz, 4);
...
#if 1	//Debug: Deref pointers to check validity
//		if(BGBCC_CCXL_TypePointerP(ctx, ctx->cur_func->ext->regs[j]->type))
//			BGBCC_BSR_EmitOpMReg(sctx, BGBCC_SH_NMID_PREF, sreg);
#endif
...
#if 1	//Debug: Deref pointers to check validity
//		if(BGBCC_CCXL_TypePointerP(ctx, ctx->cur_func->ext->args[j]->type))
//			BGBCC_BSR_EmitOpMReg(sctx, BGBCC_SH_NMID_PREF, sreg);
		BGBCC_BSRC_EmitDebugCheckReg(ctx, sctx,
			ctx->cur_func->ext->args[j]->type, sreg);
#endif
...
//			BGBCC_BSRC_ScratchCheckRegFree(ctx, sctx,
//				BGBCC_SH_REG_R0) &&
//			BGBCC_BSR_EmitCheckRegBaseGPR(sctx, sreg))
		if(1)
		{
#if 0
			if((BGBCC_CCXL_TypeSmallIntP(ctx, tty) ||
//			if((BGBCC_CCXL_TypeSgIntP(ctx, tty) ||
				...
				!BGBCC_CCXL_TypeArrayP(ctx, tty))
			{
				...
				k=BGBCC_BSRC_GetGblIndexLabel(ctx, sctx, j);
				BGBCC_BSR_EmitStoreRegLabelVarRel24(sctx,
...
//			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
		...
		if(BGBCC_CCXL_TypeSmallIntP(ctx, tty) ||
			...
			BGBCC_CCXL_TypePointerP(ctx, tty))
		{
//			p1=BGBCC_BSR_EmitOpRegStReg(sctx, BGBCC_SH_NMID_MOVL,
//				dreg, treg);
...
//			BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, dreg);
			...
			BGBCC_CCXL_TypeDoubleP(ctx, tty))
		{
			if(sctx->fpu_soft)
			{
				if(BGBCC_BSR_EmitCheckRegQuad(sctx, sreg))
				{
					BGBCC_BSRC_EmitStoreBRegOfsReg(ctx, sctx,
...
//			p1=BGBCC_BSR_EmitOpRegStReg(sctx, BGBCC_SH_NMID_FMOVS,
//				dreg, treg);
...
int BGBCC_BSRC_EmitLdaValSyncFrameVRegReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg, int dreg)
{
	...
	if(BGBCC_CCXL_IsRegTempP(ctx, sreg))
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		k=ctx->cur_func->ext->regs[j]->fxoffs;
		ctx->cur_func->ext->regs[j]->regflags|=BGBCC_REGFL_ALIASPTR;

		if(ctx->cur_func->ext->regs[j]->regcls==BGBCC_SH_REGCLS_VO_REF)
		{
			k=(ctx->cur_func->ext->regs[j]->fxmoffs)+(sctx->frm_offs_fix);
			BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, dreg);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, sreg, dreg);
			return(1);
		}
	...
	if(BGBCC_CCXL_IsRegArgP(ctx, sreg))
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		k=ctx->cur_func->ext->args[j]->fxoffs;
		ctx->cur_func->ext->args[j]->regflags|=BGBCC_REGFL_ALIASPTR;

		if(ctx->cur_func->ext->args[j]->regcls==BGBCC_SH_REGCLS_VO_REF)
		{
			k=(ctx->cur_func->ext->args[j]->fxmoffs)+(sctx->frm_offs_fix);
			BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, k, dreg);
			BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, sreg, dreg);
			return(1);
		}
	...
	i=BGBCC_BSRC_EmitLdaFrameVRegReg(ctx, sctx, sreg, dreg);
	return(i);
}

int BGBCC_BSRC_EmitLdaFrameVRegReg(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg, int dreg)
{
	...
	if(BGBCC_CCXL_IsRegLocalP(ctx, sreg))
	{
		...
		k=ctx->cur_func->ext->locals[j]->fxoffs;
		ctx->cur_func->ext->locals[j]->regflags|=BGBCC_REGFL_ACCESSED;
		ctx->cur_func->ext->locals[j]->regflags|=BGBCC_REGFL_ALIASPTR;
...
int BGBCC_BSRC_GetFrameVRegFlags(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg)
{
	...
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		if(!ctx->cur_func->ext->regs && !j)
			return(0);
		return(ctx->cur_func->ext->regs[j]->regflags);
	}

	if(BGBCC_CCXL_IsRegArgP(ctx, sreg))
	{
		j=sreg.val&CCXL_REGID_BASEMASK;
		return(ctx->cur_func->ext->args[j]->regflags);
	}
...
//		ctx->cur_func->ext->locals[j]->regflags|=BGBCC_REGFL_ACCESSED;
//		ctx->cur_func->ext->locals[j]->regflags|=BGBCC_REGFL_ALIASPTR;
		return(ctx->cur_func->ext->locals[j]->regflags);
	}
	
	return(0);
}

int BGBCC_BSRC_SetupFrameVRegSpan(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_register sreg)
{
	...
	if(!sctx->vspan)
	{
		sctx->vspan=bgbcc_malloc(1024*sizeof(BGBCC_BSR_VarSpan *));
		sctx->vspan_num=0;
		sctx->vspan_max=1024;
	}
	...
	if(!vsp)
	{
		vsp=bgbcc_malloc(sizeof(BGBCC_BSR_VarSpan));
		sctx->vspan[i]=vsp;
	}
...
int BGBCC_BSRC_SetupFrameLayout(BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	BGBCC_CCXL_RegisterInfo *obj)
{
	...
	for(i=0; i<obj->ext->n_vop; i++)
	{
		...
		if(vop->opn==CCXL_VOP_LDAVAR)
		{
			...
			if(BGBCC_CCXL_IsRegTempP(ctx, reg))
				{ obj->ext->regs[j]->regflags|=BGBCC_REGFL_ALIASPTR; }
			if(BGBCC_CCXL_IsRegArgP(ctx, reg))
				{ obj->ext->args[j]->regflags|=BGBCC_REGFL_ALIASPTR; }
			if(BGBCC_CCXL_IsRegLocalP(ctx, reg))
				{ obj->ext->locals[j]->regflags|=BGBCC_REGFL_ALIASPTR; }
		}
	}
	...
	for(i=0; i<obj->ext->n_vop; i++)
	{
		...
		while(i>=(vtr->b_ops+vtr->n_ops) && (trn<obj->ext->n_vtr))
		{
			trn++;
			vtr=obj->ext->vtr[trn];
		}
		...
		BGBCC_BSRC_SetupFrameVRegSpan(ctx, sctx, vop->dst);
		BGBCC_BSRC_SetupFrameVRegSpan(ctx, sctx, vop->srca);
		BGBCC_BSRC_SetupFrameVRegSpan(ctx, sctx, vop->srcb);
		...
		if(vop->opn==CCXL_VOP_CALL)
		{
			...
			{
				BGBCC_BSRC_SetupFrameVRegSpan(ctx, sctx,
					vop->imm.call.args[j]);
			}
		}
	}
...
//	if(sctx->has_bjx1egpr)
	...
	for(i=0; i<obj->ext->n_locals; i++)
	{
		...
		rcls=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->ext->locals[i]->type);
		obj->ext->locals[i]->regcls=rcls;
		switch(rcls)
		{
		...
		case BGBCC_SH_REGCLS_AR_REF:
			obj->ext->locals[i]->fxmoffs=kf;
			j=BGBCC_CCXL_TypeGetLogicalSize(ctx, obj->ext->locals[i]->type);
			if(!j)
				{ BGBCC_DBGBREAK }
			j=(j+3)&(~3);
			kf+=j;
		...
		if(BGBCC_CCXL_TypeArrayP(ctx, obj->ext->locals[i]->type) ||
			(BGBCC_CCXL_TypeValueObjectP(ctx, obj->ext->locals[i]->type) &&
			(rcls==BGBCC_SH_REGCLS_VO_REF))	)
		{
//			k-=4; obj->ext->locals[i]->fxoffs=k;
			obj->ext->locals[i]->fxmoffs=kf;
			j=BGBCC_CCXL_TypeGetLogicalSize(ctx, obj->ext->locals[i]->type);
			j=(j+3)&(~3);
			kf+=j;
	...
	{
		rcls=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->ext->regs[i]->type);
		obj->ext->regs[i]->regcls=rcls;
		...
		case BGBCC_SH_REGCLS_VO_REF:
		case BGBCC_SH_REGCLS_AR_REF:
			obj->ext->regs[i]->fxmoffs=kf;
			j=BGBCC_CCXL_TypeGetLogicalSize(ctx, obj->ext->regs[i]->type);
			j=(j+3)&(~3);
			kf+=j;
			break;
...
#if 0
		if(BGBCC_CCXL_TypeArrayP(ctx, obj->ext->regs[i]->type))
		{
//			k-=4; obj->ext->regs[i]->fxoffs=k;
			obj->ext->regs[i]->fxmoffs=kf;
			j=BGBCC_CCXL_TypeGetLogicalSize(ctx, obj->ext->regs[i]->type);
			j=(j+3)&(~3);
			kf+=j;
...
//			k-=4; obj->ext->regs[i]->fxoffs=k;
	...
	for(i=0; i<obj->ext->n_args; i++)
	{
		...
		rcls=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->ext->args[i]->type);
		if(rcls==BGBCC_SH_REGCLS_WGR)
			rcls=BGBCC_SH_REGCLS_GR;

		obj->ext->args[i]->regcls=rcls;
...
//		if(BGBCC_CCXL_TypeValueObjectP(ctx, obj->ext->args[i]->type) &&
//			(rcls==BGBCC_SH_REGCLS_VO_REF))
		if(rcls==BGBCC_SH_REGCLS_VO_REF)
		{
//			k-=4; obj->ext->args[i]->fxoffs=k;
			obj->ext->args[i]->fxmoffs=kf;
			j=BGBCC_CCXL_TypeGetLogicalSize(ctx, obj->ext->args[i]->type);
			j=(j+3)&(~3);
			kf+=j;
		}
	}
...
//			if(sctx->is_addr64)
			if(sctx->has_bjx1egpr)
			{
				if(ni<8)	{ k&=~3; k-=4; obj->ext->args[i]->fxoffs=k; ni++; }
				else		{ obj->ext->args[i]->fxoffs=ka; ka+=4; }
				break;
			}
		
			if(ni<4)	{ k&=~3; k-=4; obj->ext->args[i]->fxoffs=k; ni++; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=4; }
			break;
		...
		case BGBCC_SH_REGCLS_QGR:
			if(sctx->is_addr64)
			{
				...
				{
					k&=(~7); k-=8;
					obj->ext->args[i]->fxoffs=k; ni++;
				}
				else
				{
					ka=(ka+7)&(~7);
					obj->ext->args[i]->fxoffs=ka;
					ka+=8;
				}
				break;
			}
...
//				if(ni<7)
				...
				{
					k&=~7; k-=16;
					obj->ext->args[i]->fxoffs=k; ni+=2;
				}
				else
				{
					ka=(ka+7)&(~7);
					obj->ext->args[i]->fxoffs=ka; ka+=16;
				}
				break;
			}
			if(ni<3)	{ k&=~3; k-=8; obj->ext->args[i]->fxoffs=k; ni+=2; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=8; }
			break;
		case BGBCC_SH_REGCLS_FR:
			if(nf<8)	{ k&=~3; k-=4; obj->ext->args[i]->fxoffs=k; nf++; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=4; }
			sctx->use_fpr=1;
			break;
		...
		case BGBCC_SH_REGCLS_FR2:
			if(sctx->is_addr64)
			{
				...
				{
					k&=~7; k-=8;
					obj->ext->args[i]->fxoffs=k; nf+=2;
				}
				else
				{
					ka=(ka+7)&(~7);
					obj->ext->args[i]->fxoffs=ka; ka+=8;
				}
				sctx->use_fpr=1;
				sctx->use_dbr=1;
				break;
			...
			nf+=nf&1;
			if(nf<7)	{ k&=~3; k-=8; obj->ext->args[i]->fxoffs=k; nf+=2; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=8; }
			sctx->use_fpr=1;
			sctx->use_dbr=1;
			break;
...
#if 0
	for(i=0; i<obj->ext->n_locals; i++)
		{ obj->ext->locals[i]->fxoffs=0; }
	for(i=0; i<obj->ext->n_regs; i++)
		{ obj->ext->regs[i]->fxoffs=0; }
...
//		tty=BGBCC_CCXL_GetRegType(ctx, reg);
		...
		j=reg.val&4095;
		if(BGBCC_CCXL_IsRegTempP(ctx, reg))
			{ obj->ext->regs[j]->fxoffs=k; }
		if(BGBCC_CCXL_IsRegLocalP(ctx, reg))
			{ obj->ext->locals[j]->fxoffs=k; }
	}
	...
	for(i=0; i<obj->ext->n_locals; i++)
	{
		...
		j=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->ext->locals[i]->type);
		switch(j)
		{
		case BGBCC_SH_REGCLS_GR:
		case BGBCC_SH_REGCLS_VO_GR:
			k&=~3; k-=4; obj->ext->locals[i]->fxoffs=k;
			break;

		case BGBCC_SH_REGCLS_WGR:
			k&=~1; k-=2; obj->ext->locals[i]->fxoffs=k;
			break;
		...
		case BGBCC_SH_REGCLS_QGR:
			if(sctx->is_addr64)
				{ k&=~7; k-=8; obj->ext->locals[i]->fxoffs=k; break; }
			k&=~3; k-=4; obj->ext->locals[i]->fxoffs=k;
			break;
		case BGBCC_SH_REGCLS_GR2:
		case BGBCC_SH_REGCLS_VO_GR2:
		case BGBCC_SH_REGCLS_VO_QGR2:
			if(sctx->is_addr64)
				{ k&=~7; k-=16; obj->ext->locals[i]->fxoffs=k; break; }
			k&=~3; k-=8; obj->ext->locals[i]->fxoffs=k;
			break;
	...
	for(i=0; i<obj->ext->n_regs; i++)
	{
		...
		case BGBCC_SH_REGCLS_GR:
		case BGBCC_SH_REGCLS_VO_GR:
			k&=~3; k-=4; obj->ext->regs[i]->fxoffs=k;
			break;

		case BGBCC_SH_REGCLS_WGR:
			k&=~1; k-=2; obj->ext->regs[i]->fxoffs=k;
			break;
		...
		case BGBCC_SH_REGCLS_QGR:
			if(sctx->is_addr64)
				{ k&=~7; k-=8; obj->ext->regs[i]->fxoffs=k; break; }
			k&=~3; k-=4; obj->ext->regs[i]->fxoffs=k;
			break;
		...
		case BGBCC_SH_REGCLS_VO_QGR2:
			if(sctx->is_addr64)
				{ k&=~7; k-=16; obj->ext->regs[i]->fxoffs=k; break; }
			k&=~3; k-=8; obj->ext->regs[i]->fxoffs=k;
			break;
...
int BGBCC_BSRC_CalcFrameEpiKey(BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	BGBCC_CCXL_RegisterInfo *obj,
...
int BGBCC_BSRC_EmitFrameProlog(BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	BGBCC_CCXL_RegisterInfo *obj, int fcnlbl)
{
	...
	BGBCC_BSRC_CalcFrameEpiKey(ctx, sctx, obj, 1, &epik, &epix);
//	epix=(((((u64)epik)+1)*2147483647LL)>>32)&1023;
	...
	if(epilbl>0)
	{
		epij=BGBCC_BSR_EmitCheckAutoLabelNear16B(sctx, epilbl);
		if(epij<=0)epilbl=0;
	}
	...
	if(!sctx->is_simpass && epik && (epilbl<=0))
	{
		...
		epilbl=BGBCC_BSR_GenLabel(sctx);
		sctx->eprhash_key[epix]=epik;
		sctx->eprhash_lbl[epix]=epilbl;

		BGBCC_BSR_EmitLabel(sctx, epilbl);
	...
	//			break;
			...
			if(sctx->reg_vsave&(1<<i))
			{
				...
				if(sctx->is_addr64)
				{
					BGBCC_BSR_EmitOpReg(sctx, BGBCC_SH_NMID_PUSH,
						BGBCC_SH_REG_R0+i);
				}else
				{
					BGBCC_BSR_EmitOpRegStDecReg(sctx, BGBCC_SH_NMID_MOVL,
	...
	//			k++;
		...
		{
			BGBCC_BSR_EmitOpRegStDecReg(sctx, BGBCC_SH_NMID_STSL,
	...
	//		k++;
		...
		for(i=15; i>0; i--)
		{
			...
			{
				sctx->freg_save|=(1<<i);
				BGBCC_BSR_EmitOpRegStDecReg(sctx, BGBCC_SH_NMID_FMOVS,
...
//			ob=BGBCC_BSR_EmitGetOffs(sctx);
//			ov=BGBCC_BSR_EmitGetOffsWord(sctx, ob-2);
//			BGBCC_BSR_EmitSetOffsWord(sctx, ob-2, 0x000B);
//			BGBCC_BSR_EmitWord(sctx, ov);
//		}else
//		if(sctx->has_bjx1jmp)
		if(1)
		{
			BGBCC_BSR_EmitOpNone(sctx, BGBCC_SH_NMID_RTSN);
		}else
		{
			BGBCC_BSR_EmitOpNone(sctx, BGBCC_SH_NMID_RTS);
//			BGBCC_BSR_EmitOpNone(sctx, BGBCC_SH_NMID_NOP);
		}
#endif
	}
#endif

	BGBCC_BSR_EmitLabel(sctx, fcnlbl);
	...
	sctx->lbl_ret=BGBCC_BSR_GenLabelTemp(sctx);
...
//	sctx->vsp_rsv=3;
	...
	for(i=0; i<sctx->vsp_rsv; i++)
	{
		...
		reg=sctx->vspan[i]->reg;
		k=BGBCC_BSRC_GetFrameVRegFlags(ctx, sctx, reg);
...
//	j=BGBCC_BSR_EmitCheckAutoLabelNear16B(sctx, epilbl);
//	epij=BGBCC_BSR_EmitCheckAutoLabelNear16B(sctx, epilbl);
...
//		BGBCC_BSR_EmitOpRegStDecReg(sctx, BGBCC_SH_NMID_STSL,
//			BGBCC_SH_REG_PR, BGBCC_SH_REG_SP);
		BGBCC_BSR_EmitOpReg(sctx, BGBCC_SH_NMID_PUSH, BGBCC_SH_REG_PR);
		k+=sctx->is_addr64?2:1;
#endif
...
//		if((epij&3)>1)
		{
			BGBCC_BSR_EmitOpLabel(sctx,
				BGBCC_SH_NMID_BSR, epilbl);
//			BGBCC_BSR_EmitOpNone(sctx, BGBCC_SH_NMID_NOP);

		}else
		{
			BGBCC_BSR_EmitOpFar16Label(sctx,
...
//	BGBCC_BSRC_SetModeDqSet(ctx, sctx);
//	BGBCC_BSRC_ResetModeDqLocal(ctx, sctx);

//	BGBCC_BSR_EmitWord(sctx, 0x0858);	//Debug, Set Token
	
	sctx->frm_offs_save=sctx->frm_size-(k*4);

	p0=BGBCC_BSR_TryEmitOpImmReg(sctx, BGBCC_SH_NMID_ADD,
	...
	if(p0<=0)
	{
		...
		BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_SUB,
	...
	if(sctx->is_pic)
	{
//		BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, -16, BGBCC_SH_REG_R12);
		BGBCC_BSRC_EmitSaveFrameReg(ctx, sctx, BGBCC_SH_REG_R12);

		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R0);
		BGBCC_BSR_EmitWord(sctx, 0xC700);
		BGBCC_BSR_EmitLoadRegLabelRel(sctx, BGBCC_SH_REG_R12, sctx->lbl_got, 0);
		BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD,
...
//	BGBCC_BSRC_ResetFpscrUnknown(ctx, sctx);
	...
	{
		if(sctx->is_addr64)
			BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_RQ2);
		else
			BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_R2);
	}
	...
	for(i=0; i<obj->ext->n_args; i++)
	{
		...
		pr0=BGBCC_BSRC_GetVRegPriority(ctx, sctx, treg);
...
//		case BGBCC_SH_REGCLS_VO_GR:
			if(sctx->is_addr64)
			{
				...
				if(ni>=4)
				{
					if((sctx->vsp_rsv>0) && (pr0<sctx->vsp_rsv))
						BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx,
							treg, BGBCC_SH_REG_RD20+(ni-4));
					else
						BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx,
		...
		case BGBCC_SH_REGCLS_VO_GR:
			if(sctx->is_addr64)
			{
				...
				{
					if((sctx->vsp_rsv>0) && (pr0<sctx->vsp_rsv))
						BGBCC_BSRC_EmitStoreFrameVRegByValReg(ctx, sctx,
							treg, BGBCC_SH_REG_RD20+(ni-4));
					else
						BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx,
							k, BGBCC_SH_REG_RD20+(ni-4));
...
//		case BGBCC_SH_REGCLS_QGR:
		...
		case BGBCC_SH_REGCLS_FR:
			if(nf>=8)
				break;
			k=obj->ext->args[i]->fxoffs;
			BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_FR4+nf);
			nf++;
			break;
		case BGBCC_SH_REGCLS_FR2:
			nf+=nf&1;
	...
	for(i=0; i<obj->ext->n_locals; i++)
	{
			...
			(rcls==BGBCC_SH_REGCLS_AR_REF))
		{
			...
			if(sctx->is_addr64)
			{
				BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, j, BGBCC_SH_REG_RQ3);
				BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_RQ3);
			}else
			{
				BGBCC_BSRC_EmitLdaFrameOfsReg(ctx, sctx, j, BGBCC_SH_REG_R3);
				BGBCC_BSRC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_R3);
			}
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R3);
		}
...
//				if(0)
				{
					...
					BGBCC_BSR_EmitLoadRegImm(sctx,
						BGBCC_SH_NMID_MOVL, BGBCC_SH_REG_R0, 64+nf*4);
					BGBCC_BSR_EmitOpRegStRegDisp(sctx,
						BGBCC_SH_NMID_MOVL, BGBCC_SH_REG_R0,
						BGBCC_SH_REG_R3, 104);
					...
					BGBCC_BSRC_CheckSetModeDqSet(ctx, sctx);
					BGBCC_BSR_EmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVQ,
				...
				}else
				{
					...
					BGBCC_BSR_EmitOpRegStReg2(sctx, BGBCC_SH_NMID_MOVQ,
						BGBCC_SH_REG_R1, BGBCC_SH_REG_R0, BGBCC_SH_REG_R3);
				}

//				BGBCC_BSR_EmitOpNone(sctx, BGBCC_SH_NMID_BRK);
			}else
			{
				for(j=0; j<4; j++)
				{
...
//			{ sctx->vsp_rsv=i; break; }
	...
	{
		reg=sctx->vspan[i].reg;
		BGBCC_BSRC_EmitGetRegisterWrite(ctx, sctx, reg);
//		BGBCC_BSRC_EmitReleaseRegister(ctx, sctx, reg);
	}
#endif
...
int BGBCC_BSRC_EmitFrameEpilog(BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	BGBCC_CCXL_RegisterInfo *obj)
{
	ccxl_type tty;
	...
	{
//		BGBCC_BSRC_ResetFpscrLocal(ctx, sctx);
		BGBCC_BSR_EmitLabel(sctx, sctx->lbl_ret);
	}
	...
	if(k)
	{
		...
		if(sctx->is_addr64)
		{
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RQ2);
			BGBCC_BSRC_EmitLoadFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_RQ2);
			BGBCC_BSRC_EmitValueCopyRegRegSz(ctx, sctx,
				BGBCC_SH_REG_RQ2, BGBCC_SH_REG_RQ0, tsz, 4);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RQ2);
		}else
		{
			BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_R2);
			BGBCC_BSRC_EmitLoadFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_R2);
			BGBCC_BSRC_EmitValueCopyRegRegSz(ctx, sctx,
				BGBCC_SH_REG_R2, BGBCC_SH_REG_R0, tsz, 4);
			BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R2);
		}
	}

//	BGBCC_BSRC_ResetFpscrDefaults(ctx, sctx);

	BGBCC_BSRC_CalcFrameEpiKey(ctx, sctx, obj, 2, &epik, &epix);
//	epix=(((((u64)epik)+1)*2147483647LL)>>32)&1023;
	...
	if(p0<=0)
	{
		...
		BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, tr0, k);
		BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD, tr0, BGBCC_SH_REG_SP);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
	}
		...
		!sctx->is_simpass && j)
	{
		...
		{
//			BGBCC_BSR_EmitOpAutoLabel(sctx, BGBCC_SH_NMID_BRAN, epilbl);
			BGBCC_BSR_EmitOpFar16Label(sctx,
				BGBCC_SH_NMID_BRAN, epilbl);
		}
...
//	BGBCC_BSR_EmitWord(sctx, 0x0848);	//Debug, Clear Token
	...
	for(i=0; i<16; i++)
	{
		...
		{
			BGBCC_BSR_EmitOpLdIncRegReg(sctx, BGBCC_SH_NMID_FMOVS,
	...
	if(sctx->use_fpr && !sctx->no_fpu)
	{
		BGBCC_BSR_EmitOpLdIncRegReg(sctx, BGBCC_SH_NMID_LDSL,
...
//			if(1)
			{
				BGBCC_BSR_EmitOpReg(sctx, BGBCC_SH_NMID_POP,
					BGBCC_SH_REG_RQ0+i);
				k+=8;
			}else
			{
//				BGBCC_BSR_EmitOpLdIncRegReg(sctx, BGBCC_SH_NMID_MOVL,
//					BGBCC_SH_REG_SP, BGBCC_SH_REG_R0+i);
	...
	if(!(sctx->is_leaf&1))
	{
		...
		BGBCC_BSR_EmitOpReg(sctx, BGBCC_SH_NMID_POP, BGBCC_SH_REG_PC);

#if 0
//		BGBCC_BSR_EmitOpLdIncRegReg(sctx, BGBCC_SH_NMID_LDSL,
//			BGBCC_SH_REG_SP, BGBCC_SH_REG_PR);
		BGBCC_BSR_EmitOpReg(sctx, BGBCC_SH_NMID_POP, BGBCC_SH_REG_PR);
...
#if 0
	ob=BGBCC_BSR_EmitGetOffs(ctx);
	ov=BGBCC_BSR_EmitGetOffsWord(ctx, ob);
	BGBCC_BSR_EmitSetOffsWord(ctx, ob, ov);
#endif
