	...
	int cdreg, int shl)
{
	...
	if(shl<=3)
	{
		for(i=0; i<shl; i++)
		{
			...
			BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADDC, cdreg2+0, cdreg2+0);
			BGBCC_BSR_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADDC, cdreg2+1, cdreg2+1);
		}
		return(1);
	}
...
//		BGBCC_BSR_EmitOpImmReg(sctx, BGBCC_SH_NMID_MOV, -1,	cdreg2+1);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, tr0);
		return(1);
	}
