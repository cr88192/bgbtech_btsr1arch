int BGBCC_BSRC_CoffLoadBufferDLL(
	...
	byte *buf, int sz)
{
	...
	if((buf[0]!='M') || (buf[1]!='Z'))
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: MZ Sig Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: PE Header Offset Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: PE Sig Fail\n");
		return(-1);
	}
	i=bgbcc_getu16en(cspe+0x18, 0);
	if(i!=0x010B)
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: Opt Header Magic Fail\n");
		return(-1);
	}
	...
	if((expofs==0) && (expsz==0))
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: No Exports\n");
		return(0);
	}
	...
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: Exports Offsets Fail\n");
		return(-1);
	}
	...
	if((i<=0) || (i>=sz))
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: DllName RVA Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_BSRC_CoffLoadBufferDLL: Non Sane Exports Count\n");
		return(-1);
	}
...
int BGBCC_BSRC_CoffBuildExports(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx)
{
	...
	lbl_dllname=BGBCC_BSR_GenLabel(sctx);
	lbl_expaddrtab=BGBCC_BSR_GenLabel(sctx);
	lbl_nametab=BGBCC_BSR_GenLabel(sctx);
	lbl_ordtab=BGBCC_BSR_GenLabel(sctx);

	dllname=ctx->imgbasename;
	if(!dllname)
		dllname="default";

	BGBCC_BSR_SetSectionName(sctx, ".edata");
	BGBCC_BSR_EmitDWord(sctx, 0);		//0x00, export flags
	BGBCC_BSR_EmitDWord(sctx, 0);		//0x04, time/date
	BGBCC_BSR_EmitWord(sctx, 0);		//0x08, major version
	BGBCC_BSR_EmitWord(sctx, 0);		//0x0A, minor version

	BGBCC_BSR_EmitRelocTy(sctx, lbl_dllname, BGBCC_SH_RLC_RVA32);
	BGBCC_BSR_EmitDWord(sctx, 0);		//0x0C, name RVA

	BGBCC_BSR_EmitDWord(sctx, 1);		//0x10, ordinal base
	BGBCC_BSR_EmitDWord(sctx, nexps);	//0x14, address table entries
	BGBCC_BSR_EmitDWord(sctx, nexps);	//0x18, number of name pointers

	BGBCC_BSR_EmitRelocTy(sctx, lbl_expaddrtab, BGBCC_SH_RLC_RVA32);
	BGBCC_BSR_EmitDWord(sctx, 0);		//0x1C, export address table RVA
	BGBCC_BSR_EmitRelocTy(sctx, lbl_nametab, BGBCC_SH_RLC_RVA32);
	BGBCC_BSR_EmitDWord(sctx, 0);		//0x20, name pointer table RVA
	BGBCC_BSR_EmitRelocTy(sctx, lbl_ordtab, BGBCC_SH_RLC_RVA32);
	BGBCC_BSR_EmitDWord(sctx, 0);		//0x24, ordinal table RVA

	BGBCC_BSR_EmitLabel(sctx, lbl_dllname);
	BGBCC_BSR_EmitString(sctx, dllname);
	...
	for(i=0; i<nexps; i++)
	{
		...
		BGBCC_BSR_EmitLabel(sctx, k);
		BGBCC_BSR_EmitString(sctx, bgbcc_strtab_i(obj->name_ix));
	}

	BGBCC_BSR_EmitBAlign(sctx, 4);

	BGBCC_BSR_EmitLabel(sctx, lbl_expaddrtab);
	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];
		BGBCC_BSR_EmitRelocTy(sctx,
			obj->fxoffs, BGBCC_SH_RLC_RVA32);
		BGBCC_BSR_EmitDWord(sctx, 0);	//export RVA
	}

	BGBCC_BSR_EmitLabel(sctx, lbl_nametab);
	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];
		BGBCC_BSR_EmitRelocTy(sctx,
			obj->fxnalgn, BGBCC_SH_RLC_RVA32);
		BGBCC_BSR_EmitDWord(sctx, 0);	//name RVA
	}

	BGBCC_BSR_EmitLabel(sctx, lbl_ordtab);
	for(i=0; i<nexps; i++)
	{
		BGBCC_BSR_EmitDWord(sctx, i+1);	//ordinal
	}
	
	return(0);
}

int BGBCC_BSRC_CoffBuildImports(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx)
{
...
//	}
	...
	for(i=0; i<nexps; i++)
	{
		...
		if(j>=ndlls)
		{
			...
			lodlls[j]=lobj;
			dllntab[j]=lobj->name;
			dllsytab[j]=BGBCC_BSR_GenLabel(sctx);

			dlliltab[j]=BGBCC_BSR_GenLabel(sctx);
			dlliatab[j]=BGBCC_BSR_GenLabel(sctx);
	...
	BGBCC_BSR_SetSectionName(sctx, ".idata");

	for(i=0; i<ndlls; i++)
	{
		BGBCC_BSR_EmitRelocTy(sctx, dlliltab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_BSR_EmitDWord(sctx, 0);	//import lookup table RVA
		BGBCC_BSR_EmitDWord(sctx, 0);	//time/date

		BGBCC_BSR_EmitDWord(sctx, 0);	//forwarder chain

		BGBCC_BSR_EmitRelocTy(sctx, dllsytab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_BSR_EmitDWord(sctx, 0);	//name RVA

		BGBCC_BSR_EmitRelocTy(sctx, dlliatab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_BSR_EmitDWord(sctx, 0);	//import address table RVA
	}
	...
	for(i=0; i<ndlls; i++)
	{
		k=dllsytab[i];
		BGBCC_BSR_EmitLabel(sctx, k);
		BGBCC_BSR_EmitString(sctx, dllntab[i]);
	}
	...
	for(i=0; i<nexps; i++)
	{
		...
		BGBCC_BSR_EmitLabel(sctx, k);
//		BGBCC_BSR_EmitDWord(sctx, obj->fxmoffs);	//hint
		BGBCC_BSR_EmitWord(sctx, obj->fxmoffs);		//hint
		BGBCC_BSR_EmitString(sctx, bgbcc_strtab_i(obj->name_ix));		//name
	}
...
//		BGBCC_BSR_EmitString(sctx, dllntab[i]);
		...
		{
			obj=exptab[j];
			BGBCC_BSR_EmitRelocTy(sctx, obj->fxnalgn, BGBCC_SH_RLC_RVA32);
			BGBCC_BSR_EmitDWord(sctx, 0);	//name
			if(sctx->is_addr64)
...
//		BGBCC_BSR_EmitString(sctx, dllntab[i]);
		...
		{
			obj=exptab[j];
			BGBCC_BSR_EmitLabel(sctx, obj->fxnoffs);
			BGBCC_BSR_EmitRelocTy(sctx, obj->fxnalgn, BGBCC_SH_RLC_RVA32);
			BGBCC_BSR_EmitDWord(sctx, 0);	//name
			if(sctx->is_addr64)
				BGBCC_BSR_EmitDWord(sctx, 0);
...
int BGBCC_BSRC_CoffSectionFlags(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	char *name)
{
...
int bgbcc_bsrc_qrsort(u32 *arr, int cnt, int rd)
{
	...
	bgbcc_bsrc_qrsort(arr, lo, rd+1);
	bgbcc_bsrc_qrsort(arr+lo, cnt-lo, rd+1);
	return(0);
}

ccxl_status BGBCC_BSRC_FlattenImagePECOFF(BGBCC_TransState *ctx,
...
//	int rlc_sz_page[4096];
	...
	BGBCC_BSRC_CoffBuildExports(ctx, sctx);
	BGBCC_BSRC_CoffBuildImports(ctx, sctx);
	...
	bgbcc_bsrc_qrsort(rlctab, nrlce, 0);

	BGBCC_BSR_SetSectionName(sctx, ".reloc");
	BGBCC_BSR_EmitBAlign(sctx, 4);
	ofsrlc=BGBCC_BSR_EmitGetOffs(sctx);
	BGBCC_BSR_EmitRawBytes(sctx, NULL, szrlc+256);
...
//		lb_strt=BGBCC_BSR_LookupNamedLabel(sctx, "___start");
	...
	{
		printf("BGBCC_BSRC_FlattenImageELF: No Entry Point Found\n");
		va_strt=0;
	}

	ofsimp=0;	szimp=0;
	i=BGBCC_BSR_LookupSectionID(sctx, ".idata");
	if(i>=0)
	{	ofsimp=sctx->sec_rva[i];
		szimp=sctx->sec_lsz[i];		}

	ofsexp=0;	szexp=0;
	i=BGBCC_BSR_LookupSectionID(sctx, ".edata");
	if(i>=0)
...
//		bgbcc_setu32en(ct+0x70, en, j);			//mBaseOfData
	...
	for(i=0; i<sctx->nsec; i++)
	{
		...
		k=BGBCC_BSRC_CoffSectionFlags(ctx, sctx, s0);
