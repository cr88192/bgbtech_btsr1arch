...
int BGBCC_CCXL_IsBinaryLogicIP(BGBCC_TransState *ctx, BCCX_Node *l, int fl)
{
	...
	if(BCCX_TagIsCstP(l, &bgbcc_rcst_binary, "binary"))
	{
		...
		{
			if(	BGBCC_CCXL_IsBinaryLogicIP(ctx, ln, fl) &&
				BGBCC_CCXL_IsBinaryLogicIP(ctx, rn, fl) )
			{
				return(1);
			}
			return(0);
...
//			return(1);
		...
		if(!BGBCC_CCXL_InferExprCleanP(ctx, ln))
			return(0);
		if(BGBCC_CCXL_IsBinaryLogicP(ctx, ln))
			return(1);
#endif
...
{
	return(BGBCC_CCXL_IsBinaryLogicIP(ctx, l, 0));
}

int BGBCC_CCXL_IsBinaryLogicLeftP(BGBCC_TransState *ctx, BCCX_Node *l)
{
	return(BGBCC_CCXL_IsBinaryLogicIP(ctx, l, 1));
}
...
//			if(!bgbcp_strcmp1(s, "!"))
		...
		if(!bgbcp_strcmp1(s, "!") &&
//			BGBCC_CCXL_IsBinaryLogicP(ctx, t))
			BGBCC_CCXL_IsBinaryLogicLeftP(ctx, t))
		{
			x=BGBCP_UnaryExpr("!!", t);
			return(x);
		}
...
//			if(	BGBCC_CCXL_IsBinaryLogicP(ctx, ln) &&
			if(	BGBCC_CCXL_IsBinaryLogicLeftP(ctx, ln) &&
				BGBCC_CCXL_IsBinaryLogicP(ctx, rn) &&
				BGBCC_CCXL_InferExprLogicFoldCostP(ctx, ln) &&
				BGBCC_CCXL_InferExprLogicFoldCostP(ctx, rn))
			{
				x=BGBCP_BinaryExpr("&&&", ln, rn);
				return(x);
			}
...
//			g=BCCX_GetFloatCst(rn, &bgbcc_rcst_value, "value");
		...
		{
			i=BCCX_GetIntCst(ln, &bgbcc_rcst_value, "value");
			suf=BCCX_GetCst(ln, &bgbcc_rcst_tysuf, "tysuf");
			...
			if((i==0) && !suf)
			{
				...
				if(	!bgbcp_strcmp1(s, "*") ||
					...
					!bgbcp_strcmp1(s, ">>"))
				{
//					if(BGBCC_CCXL_InferExprCleanP(ctx, rn)&1)
//					return(BGBCC_CCXL_WrapInt(0));
					return(BGBCC_CCXL_WrapIntSuf(0, suf));
				}
			}
		}
#endif
			...
			BGBCC_CCXL_InferExprCleanP(ctx, ln))
		{
			...
			if((i==0) && !suf)
			{
				...
				if(	!bgbcp_strcmp1(s, "*") ||
					...
					!bgbcp_strcmp1(s, "%") )
				{
//					if(BGBCC_CCXL_InferExprCleanP(ctx, ln)&1)
//					return(BGBCC_CCXL_WrapInt(0));
					return(BGBCC_CCXL_WrapIntSuf(0, suf));
				}
			}
...
//		if(BCCX_TagIsCstP(t, &bgbcc_rcst_loadindex, "loadindex"))
	...
	if(BCCX_TagIsCstP(l, &bgbcc_rcst_cast, "cast"))
	{
		...
		if(BGBCC_CCXL_IsIntP(ctx, t))
		{
			...
			switch(*s0)
			{
			case 'a':	return(BGBCC_CCXL_WrapIntSuf((sbyte)i, "SB"));
			case 'b':	return(BGBCC_CCXL_WrapIntSuf(i!=0, "UB"));
			case 'c':	return(BGBCC_CCXL_WrapIntSuf((sbyte)i, "SB"));
			case 'd':	return(BGBCC_CCXL_WrapRealSuf(f, "D"));
			case 'f':	return(BGBCC_CCXL_WrapRealSuf(f, "F"));
			case 'h':	return(BGBCC_CCXL_WrapIntSuf((byte)i, "UB"));
			case 'i':	return(BGBCC_CCXL_WrapIntSuf((s32)i, "I"));
			case 'j':	return(BGBCC_CCXL_WrapIntSuf((u32)i, "UI"));
			case 'l':	return(BGBCC_CCXL_WrapIntSuf(i, "L"));
			case 'm':	return(BGBCC_CCXL_WrapIntSuf(i, "UL"));
			case 's':	return(BGBCC_CCXL_WrapIntSuf((s16)i, "SS"));
			case 't':	return(BGBCC_CCXL_WrapIntSuf((u16)i, "US"));
			case 'w':	return(BGBCC_CCXL_WrapIntSuf((u16)i, "US"));
			case 'x':	return(BGBCC_CCXL_WrapIntSuf((s64)i, "LL"));
			case 'y':	return(BGBCC_CCXL_WrapIntSuf((u64)i, "ULL"));
			}
		}
