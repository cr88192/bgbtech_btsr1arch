...
int BGBCC_SHXC_EmitConvOpRegReg(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx,
...
#if 0
	i=BGBCC_SHX_TryEmitOpRegReg(sctx,
		nmid, sreg, dreg);
	if(i>0)
		return(i);
#endif
...
//			BGBCC_SHXC_EmitScratchSyncRegistersSafeMask(ctx, sctx, 0xC0);
			...
			BGBCC_SHXC_EmitCallName(ctx, sctx, "__sfp_ldhf16");
...
//		BGBCC_SHXC_EmitScratchSyncRegisters(ctx, sctx);
		...
		BGBCC_SHX_EmitOpRegReg(sctx,
			BGBCC_SH_NMID_MOV, sreg, BGBCC_SH_REG_R4);
		BGBCC_SHXC_EmitCallName(ctx, sctx, "__ldhf16");
		BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R4);
		BGBCC_SHX_EmitOpRegReg(sctx,
...
//			BGBCC_SHXC_EmitScratchSyncRegistersSafeMask(ctx, sctx, 0xC0);
			...
			BGBCC_SHX_EmitOpRegReg(sctx,
				BGBCC_SH_NMID_MOV, sreg, BGBCC_SH_REG_R4);
			BGBCC_SHXC_EmitCallName(ctx, sctx, "__sfp_sthf16");
			BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R4);
			BGBCC_SHX_EmitOpRegReg(sctx,
				BGBCC_SH_NMID_MOV, BGBCC_SH_REG_R0, dreg);
			BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_R0);
...
//		BGBCC_SHXC_EmitScratchSyncRegisters(ctx, sctx);
		...
		BGBCC_SHXC_EmitCallName(ctx, sctx, "__sthf16");
...
//		k=-1;
	...
	{
		i=BGBCC_SHXC_EmitLoadSlotAddrVRegVRegImm(ctx, sctx,
	...
	if(nm1>=0)
	{
		...
		}else
		{
			...
			if(!BGBCC_SHX_EmitCheckRegExtAddrGPR(sctx, csreg))
			{
				BGBCC_DBGBREAK
				csreg=BGBCC_SHXC_EmitGetRegisterRead(ctx, sctx, sreg);
			}
...
//				ctreg=BGBCC_SHXC_ScratchAllocReg(ctx, sctx, 0);
		...
		if(nm2>=0)
			{ BGBCC_SHXC_EmitConvOpRegReg(ctx, sctx, nm2, ctreg, cdreg); }
...
//		}
		...
		if(nm2>=0)
			{ BGBCC_SHXC_EmitConvOpRegReg(ctx, sctx, nm2, csreg, ctreg); }
...
//			tr1=BGBCC_SHXC_ScratchAllocReg(ctx, sctx, 0);
			...
			if(sz>2)
			{
				...
				BGBCC_SHX_EmitOpLdRegDispReg(sctx, nm1,	sreg, 2, tr0);
				BGBCC_SHX_EmitOpRegStRegDisp(sctx, nm1, tr0, dreg, 2);
			}else
			{
				BGBCC_SHX_EmitOpLdRegReg(sctx, nm1, sreg, tr0);
				BGBCC_SHX_EmitOpRegStReg(sctx, nm1, tr0, dreg);
...
//		BGBCC_SHX_EmitOpRegStReg(sctx, nm2, BGBCC_SH_REG_R0, dreg);
	...
	if(al>=4)
	{
		...
		tr2=BGBCC_SHXC_ScratchAllocReg(ctx, sctx, 0);
		tr3=BGBCC_SHXC_ScratchAllocReg(ctx, sctx, 0);

		l0=BGBCC_SHX_GenLabelTemp(sctx);

		BGBCC_SHXC_EmitMovRegReg(ctx, sctx, dreg, tr2);
		BGBCC_SHXC_EmitMovRegReg(ctx, sctx, sreg, tr3);
		BGBCC_SHX_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV, tr1, (sz+3)/4);
		...
		BGBCC_SHX_EmitOpRegImm(sctx,
			BGBCC_SH_NMID_ADD, tr2, 4);
		BGBCC_SHXC_EmitOpReg(ctx, sctx, BGBCC_SH_NMID_DT, tr1);
		BGBCC_SHX_EmitOpLabel(sctx, BGBCC_SH_NMID_BF, l0);

		BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, tr0);
		BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, tr1);
		BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, tr2);
		BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, tr3);
		return(1);
	}
#endif
