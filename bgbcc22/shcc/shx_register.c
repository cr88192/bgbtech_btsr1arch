	...
	int reg)
{
	...
	if(!BGBCC_SHX_EmitCheckRegExtGPR(sctx, reg))
		return(0);
...
//	if(BGBCC_CCXL_IsRegDoubleP(ctx, reg))
	...
	{
		return(BGBCC_SHXC_EmitTryGetDpRegister(ctx, sctx, reg, fl));
	}
...
//			continue;
			...
			reg, sctx->regalc_map[i]))
		{
			...
			if(userq)
				creg=bgbcc_shx_qcachereg[i];
			else if(sctx->is_addr64)
				creg=bgbcc_shx_jcachereg[i];
			return(creg);
		}
	}
#endif
...
//						if((pr1>=0) && (pr0>=0) && (pr0>=pr1))
		...
		if(bi>=0)
		{
			...
			BGBCC_SHXC_StompLpRegisterIndex(ctx, sctx, i);
...
int BGBCC_SHXC_GetVRegPriority(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx,
	ccxl_register reg)
{
...
int BGBCC_SHXC_GetVRegLiveRange(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx,
	ccxl_register reg,
	int *rbeg, int *rend)
...
int BGBCC_SHXC_CheckVRegLiveRange(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx,
	ccxl_register reg)
{
...
//	}
	...
	bfl=BGBCC_SHXC_GetVRegLiveRange(ctx, sctx, reg, &bb, &be);
	if(bfl&1)
		return(1);
	if(bfl&2)
		return(1);
...
int BGBCC_SHXC_EmitSyncRegisterIndex(
	...
	int rgix)
{
	return(BGBCC_SHXC_EmitSyncRegisterIndex2(ctx, sctx, rgix, 3));
}

int BGBCC_SHXC_EmitSyncRegisterIndex2(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx,
	int rgix, int sfl)
{
	...
	BGBCC_SHX_Context *sctx)
{
	...
	sctx->maxreg_gpr=bgbcc_shx_maxreg;
	if(sctx->has_bjx1egpr && sctx->use_egpr)
		sctx->maxreg_gpr=bgbcc_shx_maxreg_egpr;
...
int BGBCC_SHXC_EmitScratchSyncRegistersSafeMask(
	BGBCC_TransState *ctx,
