int BGBCC_SHXC_CoffLoadBufferDLL(
	...
	byte *buf, int sz)
{
	...
	if((buf[0]!='M') || (buf[1]!='Z'))
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: MZ Sig Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: PE Header Offset Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: PE Sig Fail\n");
		return(-1);
	}
	i=bgbcc_getu16en(cspe+0x18, 0);
	if(i!=0x010B)
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: Opt Header Magic Fail\n");
		return(-1);
	}
	...
	if((expofs==0) && (expsz==0))
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: No Exports\n");
		return(0);
	}
	...
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: Exports Offsets Fail\n");
		return(-1);
	}
	...
	if((i<=0) || (i>=sz))
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: DllName RVA Fail\n");
		return(-1);
	}
	...
	{
		printf("BGBCC_SHXC_CoffLoadBufferDLL: Non Sane Exports Count\n");
		return(-1);
	}
...
int BGBCC_SHXC_CoffBuildExports(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx)
{
	...
	lbl_dllname=BGBCC_SHX_GenLabel(sctx);
	lbl_expaddrtab=BGBCC_SHX_GenLabel(sctx);
	lbl_nametab=BGBCC_SHX_GenLabel(sctx);
	lbl_ordtab=BGBCC_SHX_GenLabel(sctx);

	dllname=ctx->imgbasename;
	if(!dllname)
		dllname="default";

	BGBCC_SHX_SetSectionName(sctx, ".edata");
	BGBCC_SHX_EmitDWord(sctx, 0);		//0x00, export flags
	BGBCC_SHX_EmitDWord(sctx, 0);		//0x04, time/date
	BGBCC_SHX_EmitWord(sctx, 0);		//0x08, major version
	BGBCC_SHX_EmitWord(sctx, 0);		//0x0A, minor version

	BGBCC_SHX_EmitRelocTy(sctx, lbl_dllname, BGBCC_SH_RLC_RVA32);
	BGBCC_SHX_EmitDWord(sctx, 0);		//0x0C, name RVA

	BGBCC_SHX_EmitDWord(sctx, 1);		//0x10, ordinal base
	BGBCC_SHX_EmitDWord(sctx, nexps);	//0x14, address table entries
	BGBCC_SHX_EmitDWord(sctx, nexps);	//0x18, number of name pointers

	BGBCC_SHX_EmitRelocTy(sctx, lbl_expaddrtab, BGBCC_SH_RLC_RVA32);
	BGBCC_SHX_EmitDWord(sctx, 0);		//0x1C, export address table RVA
	BGBCC_SHX_EmitRelocTy(sctx, lbl_nametab, BGBCC_SH_RLC_RVA32);
	BGBCC_SHX_EmitDWord(sctx, 0);		//0x20, name pointer table RVA
	BGBCC_SHX_EmitRelocTy(sctx, lbl_ordtab, BGBCC_SH_RLC_RVA32);
	BGBCC_SHX_EmitDWord(sctx, 0);		//0x24, ordinal table RVA

	BGBCC_SHX_EmitLabel(sctx, lbl_dllname);
	BGBCC_SHX_EmitString(sctx, dllname);
	...
	BGBCC_SHX_EmitLabel(sctx, lbl_expaddrtab);
	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];
		BGBCC_SHX_EmitRelocTy(sctx,
			obj->fxoffs, BGBCC_SH_RLC_RVA32);
		BGBCC_SHX_EmitDWord(sctx, 0);	//export RVA
	}

	BGBCC_SHX_EmitLabel(sctx, lbl_nametab);
	for(i=0; i<nexps; i++)
	{
		obj=exptab[i];
		BGBCC_SHX_EmitRelocTy(sctx,
			obj->fxnalgn, BGBCC_SH_RLC_RVA32);
		BGBCC_SHX_EmitDWord(sctx, 0);	//name RVA
	}

	BGBCC_SHX_EmitLabel(sctx, lbl_ordtab);
	for(i=0; i<nexps; i++)
	{
		BGBCC_SHX_EmitDWord(sctx, i+1);	//ordinal
	}
	
	return(0);
}

int BGBCC_SHXC_CoffBuildImports(
	BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx)
{
...
//	}
	...
	for(i=0; i<nexps; i++)
	{
		...
		if(j>=ndlls)
		{
			...
			lodlls[j]=lobj;
			dllntab[j]=lobj->name;
			dllsytab[j]=BGBCC_SHX_GenLabel(sctx);

			dlliltab[j]=BGBCC_SHX_GenLabel(sctx);
			dlliatab[j]=BGBCC_SHX_GenLabel(sctx);
	...
	BGBCC_SHX_SetSectionName(sctx, ".idata");

	for(i=0; i<ndlls; i++)
	{
		BGBCC_SHX_EmitRelocTy(sctx, dlliltab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_SHX_EmitDWord(sctx, 0);	//import lookup table RVA
		BGBCC_SHX_EmitDWord(sctx, 0);	//time/date

		BGBCC_SHX_EmitDWord(sctx, 0);	//forwarder chain

		BGBCC_SHX_EmitRelocTy(sctx, dllsytab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_SHX_EmitDWord(sctx, 0);	//name RVA

		BGBCC_SHX_EmitRelocTy(sctx, dlliatab[i], BGBCC_SH_RLC_RVA32);
		BGBCC_SHX_EmitDWord(sctx, 0);	//import address table RVA
	}
	...
	for(i=0; i<ndlls; i++)
	{
		k=dllsytab[i];
		BGBCC_SHX_EmitLabel(sctx, k);
		BGBCC_SHX_EmitString(sctx, dllntab[i]);
	}
...
//	int rlc_sz_page[4096];
	...
	BGBCC_SHXC_CoffBuildExports(ctx, sctx);
...
//		lb_strt=BGBCC_SHX_LookupNamedLabel(sctx, "___start");
	...
	ofsimp=0;	szimp=0;
	i=BGBCC_SHX_LookupSectionID(sctx, ".idata");
	if(i>=0)
	{	ofsimp=sctx->sec_rva[i];
		szimp=sctx->sec_lsz[i];		}

	ofsexp=0;	szexp=0;
	i=BGBCC_SHX_LookupSectionID(sctx, ".edata");
	if(i>=0)
