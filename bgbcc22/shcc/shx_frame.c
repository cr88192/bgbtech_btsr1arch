	...
	int ofs, int dreg)
{
	...
	if(BGBCC_SHX_EmitCheckRegQuad(sctx, dreg))
	{
	...
	if(sctx->sreg_live&1)
	{
		...
		BGBCC_SHX_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD,
			BGBCC_SH_REG_SP, treg);
		BGBCC_SHX_EmitOpLdRegReg(sctx, nmid, treg, dreg);
...
//	if(1)
	{
		...
		BGBCC_SHX_EmitOpRegReg(sctx, BGBCC_SH_NMID_ADD,
			BGBCC_SH_REG_SP, treg);
		BGBCC_SHX_EmitOpRegStReg(sctx, nmid, dreg, treg);
...
//		BGBCC_SHXC_EmitRegIsGpReg(ctx, sctx, dreg) &&
		BGBCC_SHXC_EmitRegIsExtGpReg(ctx, sctx, dreg) &&
...
//		i=BGBCC_SHX_GenLabel(sctx);
		i=BGBCC_SHX_GetNamedLabel(sctx, bgbcc_strtab_i(gbl->name_ix));
		gbl->fxoffs=i;
	}
	return(i);
}
...
//		k=BGBCC_SHX_GenLabel(sctx);
		k=BGBCC_SHX_GetNamedLabel(sctx, bgbcc_strtab_i(gbl->name_ix));
		gbl->fxoffs=k;
	}
...
//			BGBCC_SHXC_EmitMovRegReg(ctx, sctx, BGBCC_SH_REG_R1, dreg2+1);
		...
		if(BGBCC_SHXC_EmitRegIsLpReg(ctx, sctx, dreg) ||
			BGBCC_SHX_EmitCheckRegQuad(sctx, dreg))
		{
	...
	if(BGBCC_CCXL_IsRegGlobalP(ctx, sreg))
	{
		...
		if(sctx->has_bjx1mov && !sctx->is_pic &&
			...
			BGBCC_SHXC_ScratchCheckRegFree(ctx, sctx,
				BGBCC_SH_REG_R0) &&
			BGBCC_SHX_EmitCheckRegBaseGPR(sctx, dreg))
		{
//			if(BGBCC_CCXL_TypeSmallIntP(ctx, tty))
...
//			BGBCC_SHXC_EmitRegIsLpReg(ctx, sctx, dreg)	)
		if(!BGBCC_SHX_EmitCheckRegAddrGPR(sctx, dreg))
		{
			treg=BGBCC_SHXC_ScratchAllocReg(ctx, sctx, 0);
		}
...
//			BGBCC_SHX_EmitOpMReg(sctx, BGBCC_SH_NMID_PREF, sreg);
	...
	if(BGBCC_CCXL_IsRegGlobalP(ctx, dreg))
	{
		...
		if(sctx->has_bjx1mov && !sctx->is_pic &&
			...
			BGBCC_SHXC_ScratchCheckRegFree(ctx, sctx,
				BGBCC_SH_REG_R0) &&
			BGBCC_SHX_EmitCheckRegBaseGPR(sctx, sreg))
		{
#if 0
			if((BGBCC_CCXL_TypeSmallIntP(ctx, tty) ||
//			if((BGBCC_CCXL_TypeSgIntP(ctx, tty) ||
...
//			BGBCC_SHXC_EmitReleaseRegister(ctx, sctx, dreg);
			...
			BGBCC_CCXL_TypeDoubleP(ctx, tty))
		{
			if(sctx->fpu_soft)
			{
				if(BGBCC_SHX_EmitCheckRegQuad(sctx, sreg))
				{
					BGBCC_SHXC_EmitStoreBRegOfsReg(ctx, sctx,
...
//	if(sctx->has_bjx1egpr)
	...
	for(i=0; i<obj->ext->n_locals; i++)
	{
		...
		rcls=BGBCC_SHXC_TypeGetRegClassP(ctx, obj->ext->locals[i]->type);
...
//			k-=4; obj->ext->locals[i]->fxoffs=k;
	...
	for(i=0; i<obj->ext->n_regs; i++)
	{
		rcls=BGBCC_SHXC_TypeGetRegClassP(ctx, 
			BGBCC_CCXL_TypeWrapBasicType(obj->ext->regs[i]->type_zb));
//		rcls=BGBCC_SHXC_TypeGetRegClassP(ctx, obj->ext->regs[i]->type);

#if 0		
		if(obj->ext->regs[i]->type_zb==4)
		{
			if(sctx->is_addr64)
...
//			k-=4; obj->ext->regs[i]->fxoffs=k;
	...
	for(i=0; i<obj->ext->n_args; i++)
	{
		...
		rcls=BGBCC_SHXC_TypeGetRegClassP(ctx, obj->ext->args[i]->type);
...
//			if(sctx->is_addr64)
			if(sctx->has_bjx1egpr)
			{
				if(ni<8)	{ k-=4; obj->ext->args[i]->fxoffs=k; ni++; }
				else		{ obj->ext->args[i]->fxoffs=ka; ka+=4; }
				break;
			}
		
			if(ni<4)	{ k-=4; obj->ext->args[i]->fxoffs=k; ni++; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=4; }
			break;
...
//				if(ni<7)
			...
			if(ni<3)	{ k-=8; obj->ext->args[i]->fxoffs=k; ni+=2; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=8; }
			break;
		case BGBCC_SH_REGCLS_FR:
			if(nf<8)	{ k-=4; obj->ext->args[i]->fxoffs=k; nf++; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=4; }
			sctx->use_fpr=1;
			break;
		...
		case BGBCC_SH_REGCLS_FR2:
			...
			nf+=nf&1;
			if(nf<7)	{ k-=8; obj->ext->args[i]->fxoffs=k; nf+=2; }
			else		{ obj->ext->args[i]->fxoffs=ka; ka+=8; }
			sctx->use_fpr=1;
			sctx->use_dbr=1;
			break;
...
//		tty=BGBCC_CCXL_GetRegType(ctx, reg);
	...
	for(i=0; i<obj->ext->n_locals; i++)
	{
		...
		j=BGBCC_SHXC_TypeGetRegClassP(ctx, obj->ext->locals[i]->type);
		switch(j)
		{
		case BGBCC_SH_REGCLS_GR:
		case BGBCC_SH_REGCLS_VO_GR:
...
int BGBCC_SHXC_CalcFrameEpiKey(BGBCC_TransState *ctx,
	BGBCC_SHX_Context *sctx,
	BGBCC_CCXL_RegisterInfo *obj,
	...
	BGBCC_CCXL_RegisterInfo *obj, int fcnlbl)
{
	...
	BGBCC_SHXC_CalcFrameEpiKey(ctx, sctx, obj, 1, &epik, &epix);
//	epix=(((((u64)epik)+1)*2147483647LL)>>32)&1023;
	...
	if(epilbl>0)
	{
		epij=BGBCC_SHX_EmitCheckAutoLabelNear16B(sctx, epilbl);
		if(epij<=0)epilbl=0;
	}
	...
	BGBCC_SHX_EmitLabel(sctx, fcnlbl);
...
//	j=BGBCC_SHX_EmitCheckAutoLabelNear16B(sctx, epilbl);
//	epij=BGBCC_SHX_EmitCheckAutoLabelNear16B(sctx, epilbl);
...
//		if((epij&3)>1)
		{
			BGBCC_SHX_EmitOpLabel(sctx,
				BGBCC_SH_NMID_BSR, epilbl);
			BGBCC_SHX_EmitOpNone(sctx, BGBCC_SH_NMID_NOP);

		}else
		{
			BGBCC_SHX_EmitOpFar16Label(sctx,
...
//		BGBCC_SHXC_EmitStoreFrameOfsReg(ctx, sctx, -16, BGBCC_SH_REG_R12);
	...
	{
		if(sctx->is_addr64)
			BGBCC_SHXC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_RQ2);
		else
	...
	for(i=0; i<obj->ext->n_args; i++)
	{
		...
		pr0=BGBCC_SHXC_GetVRegPriority(ctx, sctx, treg);
...
//		case BGBCC_SH_REGCLS_QGR:
	...
	for(i=0; i<obj->ext->n_locals; i++)
	{
			...
			(rcls==BGBCC_SH_REGCLS_AR_REF))
		{
			...
			if(sctx->is_addr64)
			{
				BGBCC_SHXC_EmitLdaFrameOfsReg(ctx, sctx, j, BGBCC_SH_REG_RQ3);
				BGBCC_SHXC_EmitStoreFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_RQ3);
			}else
			{
...
//				if(0)
				{
					...
					BGBCC_SHXC_CheckSetModeDqSet(ctx, sctx);
					BGBCC_SHX_EmitOpRegStRegDisp(sctx, BGBCC_SH_NMID_MOVQ,
				...
				}else
				{
					...
					BGBCC_SHX_EmitOpRegStReg2(sctx, BGBCC_SH_NMID_MOVQ,
	...
	BGBCC_CCXL_RegisterInfo *obj)
{
	...
	if(k)
	{
		...
		if(sctx->is_addr64)
		{
			BGBCC_SHXC_ScratchSafeStompReg(ctx, sctx, BGBCC_SH_REG_RQ2);
			BGBCC_SHXC_EmitLoadFrameOfsReg(ctx, sctx, k, BGBCC_SH_REG_RQ2);
			BGBCC_SHXC_EmitValueCopyRegRegSz(ctx, sctx,
				BGBCC_SH_REG_RQ2, BGBCC_SH_REG_RQ0, tsz, 4);
			BGBCC_SHXC_ScratchReleaseReg(ctx, sctx, BGBCC_SH_REG_RQ2);
		}else
		{
...
//	BGBCC_SHXC_ResetFpscrDefaults(ctx, sctx);
	...
	BGBCC_SHXC_CalcFrameEpiKey(ctx, sctx, obj, 2, &epik, &epix);
//	epix=(((((u64)epik)+1)*2147483647LL)>>32)&1023;
