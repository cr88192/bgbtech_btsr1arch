...
byte *BGBRASW_Img_ReadTGA(FILE *fd, int *w, int *h)
{
	PDGL_TGAHeader head;
	int pixlin, i, j;
...
int BGBRASW_Img_StoreTGA(FILE *fd, byte *buf, int w, int h)
{
	PDGL_TGAHeader head;
	int i;
	byte *s;
	...
	char *name, int *w, int *h)
{
	...
	fd=fopen(name, "rb");
	if(!fd)return(NULL);
	buf=BGBRASW_Img_ReadTGA(fd, w, h);
	fclose(fd);
	return(buf);
}

int BGBRASW_Img_SaveTGA(
	char *name, byte *buf, int w, int h)
{
	...
	fd=fopen(name, "wb");
	if(!fd)return(-1);
	BGBRASW_Img_StoreTGA(fd, buf, w, h);
	fclose(fd);
	return(0);
}

#if 0
int BGBRASW_Img_StoreTGAPixel(
...
bgbrasw_pixel *BGBRASW_Img_ReadTGAPixel(FILE *fd, int *w, int *h)
{
	byte *tbuf;
	bgbrasw_pixel *buf;
	int i, j, n;
	...
	buf=malloc((*w)*(*h)*sizeof(bgbrasw_pixel));
...
bgbrasw_pixel *BGBRASW_Img_LoadTGAPixel(
	char *name, int *w, int *h)
{
	bgbrasw_pixel *buf;
	FILE *fd;
