...
//			j=(i*3)>>4;
			...
			if((j!=img->tbc_pred0) && (j!=img->tbc_pred1))
			{
...
void TKFAT_SetupImageMBR(TKFAT_ImageInfo *img)
{
	...
	if(!img->nImgBlks)
	{
		printf("TKFAT_SetupImageMBR: Can't init, No blocks\n");
		return;
	}
...
void TKFAT_ReadImageMBR(TKFAT_ImageInfo *img)
{
	...
	if(i!=0xAA55)
	{
		if(i==0)
		{
			tk_printf("TKFAT_ReadImageMBR: Sig is 00-00\n");
			img->isnoninit=1;
			return;
		}
...
void TKFAT_ReadImageFAT(TKFAT_ImageInfo *img)
{
	...
	if(!clsz || (clsz&(clsz-1)))
	{
		if(!(img->lba_start))
		{
			tk_dbg_printf("TKFAT_ReadImageFAT: Invalid Cluster Size %d\n",
				clsz);
			return;
		}
