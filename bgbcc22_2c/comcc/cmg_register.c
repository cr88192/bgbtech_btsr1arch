...
int BGBCC_CMGC_ScratchCheckRegFree(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
	if(((reg&BGBCC_SH_REG_RTMASK5)==BGBCC_SH_REG_R0) ||
		((reg&BGBCC_SH_REG_RTMASK5)==BGBCC_SH_REG_RD0) ||
		((reg&BGBCC_SH_REG_RTMASK5)==BGBCC_SH_REG_RQ0))
	{
//		if((reg&15)>=8)
//		if(!(ctx->arch_gprmask_scra&(1<<(reg&31))))
		if(!((ctx->arch_gprmask_scra>>(reg&31))&1))
			return(0);
//		if(sctx->sreg_live&(1<<(reg&15)))
	...
	{
//		if((reg&15)>=7)
		if(((ctx->arch_gprmask_scra>>(reg&31))&3)!=3)
			return(0);
//		if(sctx->sreg_live&(3<<(reg&15)))
		if(sctx->sreg_live&(3<<(reg&31)))
			return(0);
		return(1);
	}
...
//			return(0);
		...
		if(!((ctx->arch_fprmask_scra>>(reg&31))&1))
			return(0);
		if(sctx->sfreg_live&(1<<(reg&31)))
			return(0);
...
int BGBCC_CMGC_ScratchSafeStompReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
		...
		((reg&BGBCC_SH_REG_RTMASK)==BGBCC_SH_REG_RQ0))
	{
		...
		if(sctx->sreg_live&(1<<(reg&15)))
			{ BGBCC_DBGBREAK }
		return(BGBCC_CMGC_ScratchStompReg(ctx, sctx, reg));
	}
...
int BGBCC_CMGC_ScratchStompReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
...
int BGBCC_CMGC_ScratchReleaseReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
...
int BGBCC_CMGC_ScratchAllocTsReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int cls)
{
	...
	i=BGBCC_CMGC_ScratchAllocReg(ctx, sctx, cls);
	return(i);
}

int BGBCC_CMGC_ScratchAllocReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int cls)
{
...
int BGBCC_CMGC_SaveFrameOfsForReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
	...
	if(!BGBCC_CMG_EmitCheckRegExtGPR(sctx, reg))
		return(0);
...
int BGBCC_CMGC_SaveFrameOfsForFpReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
...
int BGBCC_CMGC_EmitSaveFrameReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
#if 1
	int ofs;
	ofs=BGBCC_CMGC_SaveFrameOfsForReg(ctx, sctx, reg);
	if(ofs!=0)
	{
...
//		BGBCC_CMGC_EmitStoreFrameOfsReg(ctx, sctx, ofs, reg);
		return(1);
	}

	ofs=BGBCC_CMGC_SaveFrameOfsForFpReg(ctx, sctx, reg);
	if(ofs!=0)
	{
...
int BGBCC_CMGC_EmitReloadSavedFrameReg(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int reg)
{
...
//		sctx->reg_save|=1<<(reg&15);
		BGBCC_CMGC_EmitLoadFrameOfsReg(ctx, sctx, ofs, reg);
		return(1);
	}
 ...
 */
int BGBCC_CMGC_EmitTryGetRegister(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg, int fl)
{
	...
	rcls=BGBCC_CMGC_TypeGetRegClassP(ctx, tty);

//	if(BGBCC_CCXL_IsRegFloatP(ctx, reg) ||
//		BGBCC_CCXL_IsRegDoubleP(ctx, reg))
//	{
//		return(BGBCC_CMGC_EmitTryGetFpRegister(ctx, sctx, reg, fl));
//	}
...
//	{
//		return(BGBCC_CMGC_EmitTryGetDpRegister(ctx, sctx, reg, fl));
//	}
...
//	if(rcls==BGBCC_SH_REGCLS_GR2)
	if(rcls==BGBCC_SH_REGCLS_GR2)
	{
		return(BGBCC_CMGC_EmitTryGetLpRegister(ctx, sctx, reg, fl));
	}
...
//				creg=bgbcc_jx2_jcachereg[i];
	...
	pr0=BGBCC_CMGC_GetVRegPriority(ctx, sctx, reg);
...
//			if(!((sctx->regalc_live)&(1<<i)))
				...
				!((sctx->regalc_dirty)&(1<<i)))
			{
				...
				pr1=BGBCC_CMGC_GetVRegPriority(ctx, sctx, reg1);

				if(BGBCC_CMGC_CheckVRegIsZRegP(ctx, sctx, reg1))
					{ bi=i; break; }
				else
				{
//					if((bi<0) || 
...
//						if((pr1>=0) && (pr0>=0) && (pr0>=pr1))
		...
		if(bi>=0)
		{
			...
			if((nsv>=2) && (sctx->regalc_ltcnt[bi]<=2) &&
				sctx->is_simpass &&
				!BGBCC_CMGC_CheckVRegIsZRegP(ctx, sctx, reg1))
			{
				bi=-1;
			}
		}
		...
		if(bi>=0)
		{
			...
			BGBCC_CMGC_EmitSyncRegisterIndex(ctx, sctx, i);
			BGBCC_CMGC_StompLpRegisterIndex(ctx, sctx, i);
...
//				creg=bgbcc_jx2_jcachereg[i];
			if(userq)
				...
				creg=sctx->jcachereg[i];
			BGBCC_CMGC_EmitSaveFrameReg(ctx, sctx, creg);
//			if(fl&2)
//				{ BGBCC_CMGC_EmitLoadFrameVRegReg(ctx, sctx, reg, creg); }
			return(creg);
		}
#endif
	}
 ...
 */
int BGBCC_CMGC_EmitGetRegister(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg, int fl)
{
	...
	creg=BGBCC_CMGC_EmitTryGetRegister(ctx, sctx, reg, fl);
...
//		(rcls==BGBCC_SH_REGCLS_FR2))
	if(	(rcls==BGBCC_SH_REGCLS_FR) ||
		(rcls==BGBCC_SH_REGCLS_DR) )
	{
		return(BGBCC_CMGC_EmitGetFpRegister(ctx, sctx, reg, fl));
	}
...
//		(rcls==BGBCC_SH_REGCLS_VO_GR2))
	if(rcls==BGBCC_SH_REGCLS_GR2)
	{
		return(BGBCC_CMGC_EmitGetLpRegister(ctx, sctx, reg, fl));
	}
...
int BGBCC_CMGC_EmitGetRegisterDirty(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	return(BGBCC_CMGC_EmitGetRegister(ctx, sctx, reg, 3));
}

/* Write: Register is being written. Old value is not needed. */
int BGBCC_CMGC_EmitGetRegisterWrite(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	return(BGBCC_CMGC_EmitGetRegister(ctx, sctx, reg, 1));
}

/* Read: Register is being read. No writeback needed. */
int BGBCC_CMGC_EmitGetRegisterRead(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	return(BGBCC_CMGC_EmitGetRegister(ctx, sctx, reg, 2));
}

/* Dirty: Register is being modified. Needs old value and writeback. */
int BGBCC_CMGC_EmitTryGetRegisterDirty(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	return(BGBCC_CMGC_EmitTryGetRegister(ctx, sctx, reg, 3));
}

/* Write: Register is being written. Old value is not needed. */
int BGBCC_CMGC_EmitTryGetRegisterWrite(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	return(BGBCC_CMGC_EmitTryGetRegister(ctx, sctx, reg, 1));
}

/* Read: Register is being read. No writeback needed. */
int BGBCC_CMGC_EmitTryGetRegisterRead(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	return(BGBCC_CMGC_EmitTryGetRegister(ctx, sctx, reg, 2));
}

int BGBCC_CMGC_GetVRegPriority(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
//	BGBCC_CCXL_RegisterInfo *fcn;
	BGBCC_CMG_VarSpan *vsp, *vsp1;
	ccxl_register vreg;
	int v, v1, h;
	int i, j, k;
...
int BGBCC_CMGC_GetVRegLiveRange(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg,
	int *rbeg, int *rend)
...
int BGBCC_CMGC_CheckVRegLiveRange(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
...
//	}
	...
	bfl=BGBCC_CMGC_GetVRegLiveRange(ctx, sctx, reg, &bb, &be);
	if(bfl&1)
		return(1);
	if(bfl&2)
		return(1);
...
int BGBCC_CMGC_EmitReleaseRegister(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	ccxl_register reg)
{
	...
	if(!sctx->no_fpu && !sctx->fpu_soft)
	{
		if(BGBCC_CCXL_IsRegFloatP(ctx, reg) ||
			...
			BGBCC_CCXL_IsRegDoubleP(ctx, reg))
		{
			return(BGBCC_CMGC_EmitReleaseFpRegister(ctx, sctx, reg));
		}
	}

	if(BGBCC_CCXL_IsRegSgLongP(ctx, reg) && !sctx->is_addr64)
	{
		return(BGBCC_CMGC_EmitReleaseLpRegister(ctx, sctx, reg));
	}
...
//				sctx->regalc_utcnt[i]--;
			...
			regfl=BGBCC_CMGC_GetFrameVRegFlags(ctx, sctx, reg);
...
int BGBCC_CMGC_EmitSyncRegisterIndex(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int rgix)
{
	return(BGBCC_CMGC_EmitSyncRegisterIndex2(ctx, sctx, rgix, 3));
}

int BGBCC_CMGC_EmitSyncRegisterIndex2(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx,
	int rgix, int sfl)
{
...
//			!(regfl&BGBCC_REGFL_ALIASPTR))
		{
			if(!BGBCC_CMGC_CheckVRegLiveRange(ctx, sctx, reg))
			{
				//discard
				sctx->regalc_dirty&=~(1<<i);
				return(0);
		...
		{
			rchk=1;
			BGBCC_CMGC_EmitStoreFrameVRegReg(ctx, sctx, reg, creg);
			rchk=0;
		}
...
int BGBCC_CMGC_EmitSyncRegisters(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx)
{
	int i;

	BGBCC_CMGC_EmitSyncFpRegisters(ctx, sctx);
...
int BGBCC_CMGC_EmitLabelFlushRegisters(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx)
{
	int i;

	BGBCC_CMGC_EmitLabelFlushFpRegisters(ctx, sctx);
...
int BGBCC_CMGC_EmitScratchSyncRegisters(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx)
{
	if(sctx->sreg_live)
...
int BGBCC_CMGC_EmitScratchSyncRegistersSafeMask(
	BGBCC_TransState *ctx,
	BGBCC_CMG_Context *sctx, int mask)
{
