...
int BGBCC_BSRC_NormalizeImmVRegSoftFloat(
...
int BGBCC_BSRC_EmitBinaryVRegVRegSoftFloat(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	...
	int opr, ccxl_register treg)
{
	...
	BGBCC_BSRC_NormalizeImmVRegSoftFloat(ctx, sctx, type, &treg);
	...
	if(s0 && (lr0>=0))
	{
		...
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, lr5);
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, lr5);
		BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, dreg, lr4);
		BGBCC_BSRC_EmitCallName(ctx, sctx, s0);
//		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, lr4);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, lr5);
//		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, lr4);
		BGBCC_BSRC_ScratchSafeStompReg(ctx, sctx, lr0);
		BGBCC_BSRC_EmitMovRegReg(ctx, sctx, lr0, lr4);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, lr0);
		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, lr4);
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, lr4);
		return(1);
	}
...
int BGBCC_BSRC_EmitBinaryVRegVRegVRegSoftFloat(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
	int opr,
	ccxl_register sreg, ccxl_register treg)
{
	...
	BGBCC_BSRC_NormalizeImmVRegSoftFloat(ctx, sctx, type, &sreg);
	BGBCC_BSRC_NormalizeImmVRegSoftFloat(ctx, sctx, type, &treg);
...
int BGBCC_BSRC_EmitUnaryVRegVRegSoftFloat(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type, ccxl_register dreg,
...
//	}
	...
	if(opr==CCXL_UNOP_NEG)
	{
		...
		if(rcls==BGBCC_SH_REGCLS_QGR)
		{
			tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_QGR);
			BGBCC_BSR_EmitLoadRegImm64P(sctx, tr0, 0x8000000000000000LL);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx,
				BGBCC_SH_NMID_MOV, csreg, cdreg);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx,
				BGBCC_SH_NMID_XOR, tr0, cdreg);
		...
		{
			tr0=BGBCC_BSRC_ScratchAllocReg(ctx, sctx, BGBCC_SH_REGCLS_GR);
			tr1=BGBCC_BSR_GETLREG_LO(cdreg);
			tr2=BGBCC_BSR_GETLREG_HI(cdreg);
			BGBCC_BSR_EmitLoadRegImm(sctx, BGBCC_SH_NMID_MOV,
				tr0, 0x80000000);
			BGBCC_BSRC_EmitOpRegReg(ctx, sctx,
				BGBCC_SH_NMID_MOV, csreg, cdreg);
	...
	if((opr==CCXL_UNOP_INC) || (opr==CCXL_UNOP_DEC))
	{
		...
		BGBCC_BSRC_EmitBinaryVRegVRegVRegSoftFloat(ctx, sctx,
...
int BGBCC_BSRC_EmitCompareVRegVRegVRegSoftFloat(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
...
int BGBCC_BSRC_EmitJCmpVRegVRegSoftFloat(
	BGBCC_TransState *ctx,
	BGBCC_BSR_Context *sctx,
	ccxl_type type,
	ccxl_register sreg, ccxl_register treg,
...
//		case CCXL_CMP_GE:	s0="__sfp_cmpge_f32"; break;
	...
	if(s0 && (lr0>=0))
	{
		...
		if(flip)
		{
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, lr4);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, lr5);
		}else
		{
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, treg, lr5);
			BGBCC_BSRC_EmitLoadVRegReg(ctx, sctx, sreg, lr4);
...
//		BGBCC_BSRC_EmitStoreVRegReg(ctx, sctx, dreg, lr0);
		...
		BGBCC_BSRC_ScratchReleaseReg(ctx, sctx, lr0);
//		BGBCC_BSR_EmitOpAutoLabel(sctx, BGBCC_SH_NMID_BF, lbl);
		BGBCC_BSR_EmitOpAutoLabel(sctx, nm1, lbl);
		return(1);
	}
