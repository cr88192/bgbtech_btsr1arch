...
int BGBCC_BSR_TryDisassembleOpcodeI1(
	...
	BGBCC_BSR_OpcodeArg *arg2)
{
	...
	for(i=0; bgbcc_bsr_optab[i].opm!=0; i++)
	{
		if((opw&bgbcc_bsr_optab[i].opm)==bgbcc_bsr_optab[i].opn)
			break;
	}

	if(!bgbcc_bsr_optab[i].opm)
	{
	...
	nmid=bgbcc_bsr_optab[i].nmid;
	fmid=bgbcc_bsr_optab[i].fmid;
	*rnmid=nmid;
	*rfmid=fmid;
...
char *BGBCC_BSR_NameForNmid(BGBCC_BSR_Context *ctx, int nmid)
{
	char *s0;
...
int BGBCC_BSR_TryDisassembleOpcode_PrintArg(
	BGBCC_BSR_Context *ctx,
	BGBCC_BSR_OpcodeArg *arg)
{
	switch(arg->ty)
	{
	...
	case BGBCC_SH_OPVTY_REG:
		printf("%s", BGBCC_BSR_NameForReg(ctx, op->breg));
		break;
	}
}

int BGBCC_BSR_TryDisassembleOpcodeI0(
	BGBCC_BSR_Context *ctx,
	int pc, int opw)
{
	BGBCC_BSR_OpcodeArg arg0;
	...
	BGBCC_BSR_TryDisassembleOpcodeI1(ctx, opw,
		&nmid, &fmid, &arg0, &arg1, &arg2);
		
	printf("%06X %04X %s", pc, opw,
		BGBCC_BSR_NameForNmid(ctx, nmid));
}
