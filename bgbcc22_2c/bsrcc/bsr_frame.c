...
//		i=BGBCC_BSR_GenLabel(sctx);
		i=BGBCC_BSR_GetNamedLabel(sctx, gbl->name);
		gbl->fxoffs=i;
	}
	return(i);
}
...
//		k=BGBCC_BSR_GenLabel(sctx);
		k=BGBCC_BSR_GetNamedLabel(sctx, gbl->name);
		gbl->fxoffs=k;
	}
...
//	if(sctx->has_bjx1egpr)
	...
	for(i=0; i<obj->n_locals; i++)
	{
		...
		rcls=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->locals[i]->type);
...
//			k-=4; obj->locals[i]->fxoffs=k;
	...
	{
		rcls=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->regs[i]->type);
...
//		tty=BGBCC_CCXL_GetRegType(ctx, reg);
	...
	for(i=0; i<obj->n_locals; i++)
	{
		...
		j=BGBCC_BSRC_TypeGetRegClassP(ctx, obj->locals[i]->type);
		switch(j)
		{
		case BGBCC_SH_REGCLS_GR:
		case BGBCC_SH_REGCLS_VO_GR:
