	...
	int nmid, int rm, int rn)
{
	...
	case BGBCC_SH_NMID_MOVB:	
		...
		if(!BGBCC_BSR_EmitCheckRegAddrGPR(ctx, rn))		break;
	...
	int nmid, int rm, int rn)
{
	...
	case BGBCC_SH_NMID_MOVUB:
		if(!BGBCC_BSR_EmitCheckRegAddrGPR(ctx, rm))		break;
	...
	BGBCC_BSR_Context *ctx, int nmid, int rm, int ro, int rn)
{
	...
	}else if(ro!=BGBCC_SH_REG_R0)
	{
		...
		case BGBCC_SH_NMID_MOVB:
			if(!BGBCC_BSR_EmitCheckRegBaseGPR(ctx, rm))		break;
			if(!BGBCC_BSR_EmitCheckRegAddrGPR(ctx, rn))		break;
			if(!BGBCC_BSR_EmitCheckRegBaseGPR(ctx, ro))		break;
			opw1=0x1A40|(ro&15);
	...
	}else
	{
		...
		case BGBCC_SH_NMID_MOVB:
			...
			if(!BGBCC_BSR_EmitCheckRegAddrGPR(ctx, rn))		break;
			if(!BGBCC_BSR_EmitCheckRegBaseGPR(ctx, ro))		break;
			opw1=0x1A40|(ro&15);	opw2=0x360C|((rn&15)<<4);
	...
	BGBCC_BSR_Context *ctx, int nmid, int rm, int ro, int rn)
{
	...
	}else
	{
		...
		case BGBCC_SH_NMID_MOVUB:
			...
			if(!BGBCC_BSR_EmitCheckRegBaseGPR(ctx, rn))		break;
			if(!BGBCC_BSR_EmitCheckRegBaseGPR(ctx, ro))		break;
			opw1=0x1A40|(ro&15);	opw2=0x360C|((rm&15)<<4);
