...
//	isRom	= (addr>=0x00000000) && (addr<=0x00007FFF);
	isRom	= (addr>=0x00000000) && (addr<=0x0000BFFF);
...
//		main_buslines++;
		if(isRom)
		{
			l2data1a=rombuf[((addr>>2)+0)&0x3FFF];
			l2data1b=rombuf[((addr>>2)+1)&0x3FFF];
			l2data1c=rombuf[((addr>>2)+2)&0x3FFF];
			l2data1d=rombuf[((addr>>2)+3)&0x3FFF];
//			l2opm1=0x70;
...
//	rombuf=(uint32_t *)malloc(32768);
	...
	fd=fopen("../../tst_jx2boot_fx.bin", "rb");
	if(fd)
	{
//		t2=fread(rombuf, 1, 32768, fd);
		t2=fread(rombuf, 1, 49152, fd);
		fclose(fd);
	}
