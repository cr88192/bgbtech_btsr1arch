#include "VCoreUnitUc.h"
#include "verilated.h"

VCoreUnitUc *top = new VCoreUnitUc;
...
//	bit=top->uartTxD;
	...
	if(!pos && (frac<=0))
	{
		if(uart_send_txs==uart_send_txt)
		{
			top->uartRxD=1;
			return(0);
		}

		bval=uart_send_buf[uart_send_txs++];

//		if(!bit)
		if(bval)
		{
//			printf("uart_send: byte %02X\n", bval);
