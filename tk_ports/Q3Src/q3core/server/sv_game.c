...
//		return (int)strncpy( VMA(1), VMA(2), args[3] );
		return VM_WrapPtr(strncpy( VMA(1), VMA(2), args[3] ));
...
void SV_ShutdownGameProgs( void ) {
	...
	VM_Call_1( gvm, GAME_SHUTDOWN, qfalse );
	VM_Free( gvm );
	gvm = NULL;
}
...
static void SV_InitGameVM( qboolean restart ) {
	...
	VM_Call_3( gvm, GAME_INIT, svs.time, Com_Milliseconds(), restart );
}
...
void SV_RestartGameProgs( void ) {
	...
	VM_Call_1( gvm, GAME_SHUTDOWN, qtrue );
...
qboolean SV_GameCommand( void ) {
	...
	return VM_Call_0( gvm, GAME_CONSOLE_COMMAND );
}
