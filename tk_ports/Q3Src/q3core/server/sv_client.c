...
//			VM_Call_1( gvm, GAME_CLIENT_DISCONNECT, newcl - svs.clients );
			//
			goto gotnewcl;
		}
	}
...
//	denied = (char *)VM_Call_3( gvm, GAME_CLIENT_CONNECT, clientNum, qtrue, qfalse ); // firstTime = qtrue
	vpdenied = VM_Call_3( gvm, GAME_CLIENT_CONNECT,
		clientNum, qtrue, qfalse ); // firstTime = qtrue
//	if ( denied ) {
	if ( vpdenied ) {
		// we can't just use VM_ArgPtr, because that is only valid inside a VM_Call
//		denied = VM_ExplicitArgPtr( gvm, (int)denied );
		denied = VM_ExplicitArgPtr( gvm, vpdenied );
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	VM_Call_1( gvm, GAME_CLIENT_DISCONNECT, drop - svs.clients );
...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
	...
	VM_Call_1( gvm, GAME_CLIENT_BEGIN, client - svs.clients );
}
...
static void SV_UpdateUserinfo_f( client_t *cl ) {
	...
	VM_Call_1( gvm, GAME_CLIENT_USERINFO_CHANGED, cl - svs.clients );
}
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK ) {
	...
	if (clientOK) {
		...
		if (!u->name && sv.state == SS_GAME) {
			VM_Call_1( gvm, GAME_CLIENT_COMMAND, cl - svs.clients );
		}
	}
	else if (!bProcessed)
...
void SV_ClientThink (client_t *cl, usercmd_t *cmd) {
	...
	VM_Call_1( gvm, GAME_CLIENT_THINK, cl - svs.clients );
}
