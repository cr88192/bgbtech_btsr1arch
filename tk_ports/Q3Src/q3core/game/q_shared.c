...
{
	struct q_mallochead_s *hed;
	void *ptr;
	void *ptr1;
	
	q_totmalloc += sz;
	
//	if(q_totmalloc>64*1024*1024)
	if(q_totmalloc>80*1024*1024)
		{ __debugbreak(); }

	sz=(sz+15)&(~15);

//	ptr=malloc(sz);
	ptr=malloc(sz + sizeof(struct q_mallochead_s) + 32);
	hed=ptr;
	ptr=hed+1;
	
	hed->next=q_malloc_list;
	q_malloc_list=hed;
	
	hed->lfn=lfn;
	hed->lln=lln;
	hed->sz=sz;
	hed->check=0xB0F0BAADU;
	
	ptr1=(byte *)(ptr)+sz;
	*(int *)ptr1=0xB0F0BAADU;
...
int Q_MallocCheckSane(void)
{
	...
	while(hed)
	{
		if(hed->check!=0xB0F0BAADU)
			{ __debugbreak(); }

		sz=hed->sz;
		ptr0=hed+1;
		ptr1=(byte *)(ptr0)+sz;
		if(*(int *)ptr1!=0xB0F0BAADU)
			{ __debugbreak(); }
		hed = hed->next;
	}
	return(0);
