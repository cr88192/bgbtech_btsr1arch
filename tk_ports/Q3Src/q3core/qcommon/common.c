...
//	size = (size + 3) & ~3;		// align to 32 bit boundary
	size += 16;					// space for memory trash tester
	size = (size + 15) & ~15;	// align to 32 bit boundary
...
{
	return(((byte *)ptr) - s_hunkData);
}

void *Hunk_GetPtrForOffs(int offs)
{
	return(s_hunkData+offs);
}
...
extern int q_totmalloc;

#ifdef USE_TKRA
extern int		tkra_totbrk;
#endif
...
//	Com_Printf( "Com_TouchMemory: %i msec\n", end - start );
	Com_Printf( "Com_TouchMemory: %i msec sum=%d\n", end - start, sum );

	Com_Printf( "Com_TouchMemory: hunk %d/%d MB, tkra=%dMB, total=%dMB\n",
		hunk_high.permanent>>20, s_hunkTotal>>20,
		tot_tkra>>20,
		q_totmalloc>>20 );
}
...
//	s_smallZoneTotal = 512 * 1024;
...
#if 0
#ifdef _WIN32
	smallzone = VirtualAlloc(NULL, s_smallZoneTotal,
	...
	smallzone = Q_MallocBulk(s_smallZoneTotal);

	if ( !smallzone )
	{
		Com_Error( ERR_FATAL,
			"Small zone data failed to allocate %1.1f megs",
...
//	if ( cv->integer < 20 )
//	if ( cv->integer < 12 )
	if ( cv->integer < MIN_COMZONEMEGS )
	{
//		s_zoneTotal = 1024 * 1024 * 16;
//		s_zoneTotal = 1024 * 1024 * 8;
		s_zoneTotal = 1024 * 1024 * MIN_COMZONEMEGS;
	} else {
...
#if 0
#ifdef _WIN32
	mainzone = VirtualAlloc(NULL, s_zoneTotal,
		MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
#else
	...
	mainzone = Q_MallocBulk( s_zoneTotal );
	...
	if ( cv->integer < nMinAlloc )
{
...
#if 0
#ifdef _WIN32
	s_hunkData = VirtualAlloc(NULL, s_hunkTotal + 31,
		MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
#else
	...
	s_hunkData = Q_MallocBulk(s_hunkTotal + 31);
	...
	if ( !s_hunkData )
{
	...
	// cacheline align
	s_hunkData = (byte *) ( ( (nlint)s_hunkData + 31 ) & ~31 );
	Hunk_Clear();
