...
typedef struct fileInPack_s {
	...
	int						wadlump;	// BGB: Lump Number (WAD4)
} fileInPack_t;
...
typedef struct {
	...
	wad4context_t	*wad4;				//BGB: WAD4 Context
} pack_t;
...
static FILE	*FS_FileForHandle( fileHandle_t f ) {
	...
	if (fsh[f].wad4File == qtrue) {
		Com_Error( ERR_DROP, "FS_FileForHandle: can't get FILE on WAD file" );
	}
		...
		if ( search->pack && search->pack->hashTable[hash] ) {
			...
			do {
				...
				if ( !FS_FilenameCompare( pakFile->name, filename ) ) {
					...
					fsh[*file].wad4File = qfalse;
					fsh[*file].zipFile = qfalse;

					if(pak->wad4)
						fsh[*file].wad4File = qtrue;
					else
					...
					if(fsh[*file].zipFile)
					{
						...
						if ( uniqueFILE )
						{
							...
							fsh[*file].handleFiles.file.z = unzReOpen (
								pak->pakFilename, pak->handle);
						...
						// (also sets the current file info)
						unzSetCurrentFileInfoPosition(
							pak->handle, pakFile->pos);
					...
					{
						fsh[*file].handleFiles.file.w.wad4 =
							pak->wad4;
						fsh[*file].handleFiles.file.w.lump =
							pakFile->wadlump;
						fsh[*file].handleFiles.file.w.offs = 0;
						fsh[*file].handleFiles.file.w.size =
							FS_GetWad4LumpSize(
								pak->wad4, pakFile->wadlump);
						
						fsh[*file].zipFilePos = pakFile->wadlump;
						fsh[*file].fileSize = FS_GetWad4LumpSize(
							pak->wad4, pakFile->wadlump);
...
int FS_Read( void *buffer, int len, fileHandle_t f )
{
	...
	if (fsh[f].wad4File)
	{
		wctx = fsh[f].handleFiles.file.w.wad4;
		offs = fsh[f].handleFiles.file.w.offs;
		lump = fsh[f].handleFiles.file.w.lump;
		fsize = fsh[f].handleFiles.file.w.size;
		...
		FS_Wad4ReadFromLump(wctx, buf, offs, len1, lump);
		offs += len1;
		fsh[f].handleFiles.file.w.offs = offs;
		return(len1);
	}else
...
//	char	foo[65536];
	...
	if (fsh[f].wad4File)
	{
		toffs = fsh[f].handleFiles.file.w.offs;
		tsz = fsh[f].handleFiles.file.w.size;
		switch( origin )
		{
		case FS_SEEK_CUR:
			toffs += offset;
...
static void FS_AddGameDirectory( const char *path, const char *dir ) {
	...
	pakfiles = Sys_ListFiles( pakfile, ".pk3", NULL, &numpak, qfalse );
	wadfiles = Sys_ListFiles( pakfile, ".wad", NULL, &numwad, qfalse );
	numfiles = numpak + numwad;
...
//	qsort( sorted, numfiles, 4, paksort );
	...
	for ( i = 0 ; i < numfiles ; i++ ) {
		...
		ext = pakfile + strlen(pakfile) - 4;

		pak = NULL;
		if(!strcmp(ext, ".pk3"))
			pak = FS_LoadZipFile( pakfile, sorted[i] );
		if(!strcmp(ext, ".wad"))
			pak = FS_LoadWad4File( pakfile, sorted[i] );
