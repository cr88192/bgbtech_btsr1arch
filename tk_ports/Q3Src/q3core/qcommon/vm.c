...
void	*VM_UnwrapPtr(nlint vptr)
{
	...
	return( currentVM->dataBase + vptr );
//	return(Hunk_GetPtrForOffs(vptr));
}
...
nlint	VM_WrapPtr(void *ptr)
{
	...
	return( (byte *)ptr - currentVM->dataBase );
//	return(Hunk_GetOffsForPtr(ptr));
}
...
#if 1
nlint QDECL VM_DllSyscall( nlint *arg )
{
	return currentVM->systemCall( arg );
}
#endif
...
//	int (*systemCalls)(int *), 
	...
	if ( interpret == VMI_NATIVE ) {
		...
		vm->dllHandle = Sys_LoadDll(
			module, vm->fqpath , &vm->entryPoint, VM_DllSyscall );
		if ( vm->dllHandle ) {
			return vm;
		}
...
nlint	QDECL VM_Call_I( vm_t *vm, int callnum, nlint *args ) {
	vm_t	*oldVM;
	nlint		r;
	int i;
//	va_list ap;
...
nlint	QDECL VM_Call_0( vm_t *vm, int callnum )
{
	nlint args[16];
	return(VM_Call_I(vm, callnum, args));
}

nlint	QDECL VM_Call_1( vm_t *vm, int callnum, nlint a0 )
{
	nlint args[16];
	args[0]=a0;
	return(VM_Call_I(vm, callnum, args));
...
nlint	QDECL VM_Call_2( vm_t *vm, int callnum, nlint a0, nlint a1 )
{
	nlint args[16];
	args[0]=a0;	args[1]=a1;
	return(VM_Call_I(vm, callnum, args));
...
nlint	QDECL VM_Call_3( vm_t *vm, int callnum, nlint a0, nlint a1, nlint a2 )
{
	nlint args[16];
...
nlint	QDECL VM_Call_4( vm_t *vm, int callnum,
	nlint a0, nlint a1, nlint a2, nlint a3 )
{
	nlint args[16];
...
nlint	QDECL VM_Call_5( vm_t *vm, int callnum,
	nlint a0, nlint a1, nlint a2, nlint a3, nlint a4 )
{
	nlint args[16];
	args[0]=a0;	args[1]=a1;	args[2]=a2;	args[3]=a3;
	args[4]=a4;
...
nlint	QDECL VM_Call_6( vm_t *vm, int callnum,
	nlint a0, nlint a1, nlint a2, nlint a3, nlint a4, nlint a5 )
{
	nlint args[16];
	args[0]=a0;	args[1]=a1;	args[2]=a2;	args[3]=a3;
	args[4]=a4;	args[5]=a5;
...
nlint	QDECL VM_Call_7( vm_t *vm, int callnum,
	nlint a0, nlint a1, nlint a2, nlint a3, nlint a4, nlint a5, nlint a6 )
{
	nlint args[16];
	args[0]=a0;	args[1]=a1;	args[2]=a2;	args[3]=a3;
...
nlint	QDECL VM_Call_8( vm_t *vm, int callnum,
	nlint a0, nlint a1, nlint a2, nlint a3,
...
void VM_LogSyscalls( nlint *args )
{
	...
	callnum++;
	fprintf(f, "%i: %i (%i) = %lli %lli %lli %lli\n",
