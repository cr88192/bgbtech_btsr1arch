...
//	int (QDECL *systemcalls)(int, ...) )
	nlint (QDECL **entryPoint)(nlint, nlint *args),
...
//	void	(QDECL *dllEntry)( int (QDECL *syscallptr)(int, ...) );
	void	(QDECL *dllEntry)( nlint (QDECL *syscallptr)(nlint *) );
	char	*basepath;
	char	*cdpath;
	char	*gamedir;
	char	*fn;
...
#ifdef __BJX2__
	Com_sprintf( filename, sizeof( filename ), "%sbjx2.dll", name );
#endif
  ...
  if (libHandle)
	...
	dllEntry = ( void (QDECL *)( nlint (QDECL *)( nlint * ) ) )GetProcAddress( libHandle, "dllEntry" ); 
	*entryPoint = (nlint (QDECL *)(nlint, nlint *))GetProcAddress( libHandle, "vmMain" );
