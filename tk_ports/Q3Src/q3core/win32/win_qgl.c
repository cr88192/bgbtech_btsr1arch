...
static void ( APIENTRY * dllCompressedTexImage2D)(GLenum target,
	GLint level, GLenum internalformat,
	GLsizei width, GLsizei height, GLint border,
	GLsizei imageSize, const void * data);
...
void *QGL_wglGetProcAddress(char *name)
{
	void *ptr;
	...
	if(glw_state.hinstOpenGL)
	{
		ptr=GetProcAddress( glw_state.hinstOpenGL, name );
		if(ptr)
			return(ptr);
	}
...
#define GPA( a )	(QGL_wglGetProcAddress( a ))
