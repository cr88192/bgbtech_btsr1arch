...
void GL_EncodeSquareDxt1(unsigned *src, byte *dst, int szlg2)
{
	...
	{
		cs=src+((y*4*ystr)+(x*4));
		GL_EncodeBlockDxt1(cs, ct, ystr, 0);
		ct+=8;
	}
}
...
void GL_EncodeSquareDxt5(unsigned *src, byte *dst, int szlg2)
{
	...
	{
		cs=src+((y*4*ystr)+(x*4));
		GL_EncodeBlockBC4(cs, ct+0, ystr);
		GL_EncodeBlockDxt1(cs, ct+8, ystr, 1);
		ct+=16;
	}
}
...
===============
*/
void GL_EncodeDDS (unsigned *data, int width, int height,
	qboolean mipmap, qboolean alpha)
{
	...
	int			do_square, max_size, need_dxt5;
	int			px, blg2, epix;
	int			i, j, k;
...
//		return;
	...
	{
		samples = 256 * 256 * 4;
		gl_savetexbuf = Q_Malloc(samples);
	}
	...
	{
		samples = 256 * 256;
		scaled = Q_Malloc(samples * sizeof(unsigned));
	}
	...
	if(ctbuf)
	{
		...
		if(need_dxt5)
			tdds->ddspf.dwFourCC=QGL_FOURCC('D', 'X', 'T', '5');
	...
	isz=1<<(j+j+blg2);
	if(need_dxt5)
		GL_EncodeSquareDxt5(scaled, ctbuf, szlog2);
	else
	...
	{
		ri.FS_WriteFile(gl_savetexname, gl_savetexbuf, ctbuf - gl_savetexbuf);
	}
}
#endif
...
void R_UploadCompressed (
	byte *data, int width, int height,
	qboolean mipmap, int fmt)
{
	byte *cs, *css;
	int xs, ys;
	int xshl, xshl1;
	int yshl, yshl1;
	int isz, mip, txc, blg2;
	int i, j, k;
	
	txc=fmt;
	blg2=3;
	if(fmt==GL_COMPRESSED_RGBA_S3TC_DXT5_EXT)
		blg2=4;
	
	xs=width; ys=height; xshl=0; yshl=0;
	while(xs>1)
		{ xs>>=1; xshl++; }
	while(ys>1)
		{ ys>>=1; yshl++; }
	...
	while((xshl>=0) || (yshl>=0))
	{
		...
		isz=1<<(xshl1+yshl1+blg2);
		qglCompressedTexImage2D(
			GL_TEXTURE_2D, mip, txc,
			1<<xshl, 1<<yshl, 0, isz, cs);
		cs+=isz;
		xshl--; yshl--; mip++;
	}
...
image_t *R_CreateImage( const char *name,
	...
	int glWrapClampMode )
{
	return(R_CreateImage2(name, pic,
		width, height, mipmap,
...
#if 0
image_t *R_CreateImageDxt1( const char *name,
	const byte *pic, int width, int height, 
	qboolean mipmap, qboolean allowPicmip,
	int glWrapClampMode )
{
...
static void LoadDDS( const char *name,
	byte **rpic, int *rwidth, int *rheight, int *rfmt )
{
	int		xs, ys, xs1, ys1, xs2, ys2, xc, bsc;
	byte	*pixbuf, *css;
	byte	*buffer;
	int		length, txc;
	QGL_DDS_HEADER head;
	...
	*rpic = NULL;
	...
	tdds=&head;
	memcpy(tdds, buffer+4, sizeof(QGL_DDS_HEADER));

	if(tdds->ddspf.dwFourCC==QGL_FOURCC('D', 'X', 'T', '1'))
	{
		bsc=8;
		txc=GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
	//	if(!alpha)
	//		txc=GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
		...
		if(tdds->ddspf.dwFourCC==QGL_FOURCC('D', 'X', 'T', '5'))
	{
		bsc=16;
		txc=GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
	}else
	{
		ri.FS_FreeFile( buffer );
		return;
	...
	css=buffer+4+(tdds->dwSize);

	pixbuf = ri.Malloc( xc * bsc );

	memcpy(pixbuf, css, xc * bsc);
	ri.FS_FreeFile( buffer );
...
static void LoadTGA ( const char *name, byte **pic, int *width, int *height)
{
  ...
  if (targa_header.attributes & 0x20) {
    unsigned char *flip = (unsigned char*)Q_Malloc (columns*4);
...
image_t	*R_FindImageFile( const char *name,
	...
	int glWrapClampMode )
{
	char		tname[256], tname1[256], tid1[64];
	byte		*imgbuf;
	char		*cs;
	u64			h;
	int			i, p, s, imgbufsz;
	...
	{
#ifndef __BJX2__
		COM_StripExtension(name, tname1);
		cs=COM_SkipPath(tname1);
		if(cs!=tname1)
		{
			*cs=0;

			sprintf(tname, "baseq3/texcache/%s", tname1);
			Sys_Mkdir(tname);
		}
#endif

//		COM_StripExtension(name, tname1);
//		sprintf(tname, "texcache/%s.dds", tname1);
...
//		imgbufsz = ri.FS_ReadFile ( tname, (void **)(&imgbuf));
#endif

//		LoadDDS(tname, &pic, &width, &height, &fmt);

		COM_StripExtension(name, tname1);
//		sprintf(tname, "texcache/%s.dds", tname1);
		sprintf(tname, "%s.dds", tname1);
		LoadDDS(tname, &pic, &width, &height, &fmt);
	...
	image = R_CreateImage2( ( char * ) name, pic,
		width, height, mipmap, allowPicmip, glWrapClampMode, fmt );
	ri.Free( pic );
