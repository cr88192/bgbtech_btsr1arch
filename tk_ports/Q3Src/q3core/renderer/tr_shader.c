...
================
*/
// static long generateHashValue( const char *fname, int size )

static long generateHashValueB( char *fname, int hashsz )
{
	int		i;
	long	hash;
	char	letter;
	...
	const char *timeOffset)
{
	...
	hash = generateHashValueB(strippedName, FILE_HASH_SIZE);
...
static shader_t *GeneratePermanentShader( void ) {
	...
	*newShader = shader;
	...
	hash = generateHashValueB(newShader->name, FILE_HASH_SIZE);
...
static char *FindShaderInShaderText( const char *shadername ) {
	...
	hash = generateHashValueB(shadername, MAX_SHADERTEXT_HASH);
...
qhandle_t RE_RegisterShaderFromImage(const char *name, int lightmapIndex, image_t *image, qboolean mipRawImage) {
	...
	hash = generateHashValueB(name, FILE_HASH_SIZE);
...
//	buffers = q_alloca(MAX_SHADER_FILES*sizeof(char *));
//	shaderTextHashTableSizes = q_alloca(MAX_SHADERTEXT_HASH*sizeof(int));
	buffers = Q_AllocTemp(MAX_SHADER_FILES*sizeof(char *));
	shaderTextHashTableSizes = Q_AllocTemp(MAX_SHADERTEXT_HASH*sizeof(int));
...
//	Com_Memset(shaderTextHashTableSizes, 0, sizeof(shaderTextHashTableSizes));
	Com_Memset(shaderTextHashTableSizes, 0, MAX_SHADERTEXT_HASH*sizeof(int));
	...
	for ( i = 0; i < numShaders; i++ ) {
		...
		while ( 1 ) {
			...
			hash = generateHashValueB(token, MAX_SHADERTEXT_HASH);
...
//	Com_Memset(shaderTextHashTableSizes, 0, sizeof(shaderTextHashTableSizes));
	...
	Q_FreeTemp(buffers);
	Q_FreeTemp(shaderTextHashTableSizes);
