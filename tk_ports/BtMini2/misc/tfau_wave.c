...
short *BGBMID_LoadWAV_16Mono16(char *name, int *rlen)
{
	...
	{
		i=len*(16000.0/rt);
		bp1=bgbmid_malloc(i*sizeof(short)+1024);
...
short *BGBMID_LoadWAV_RateStereo16(char *name, int rate, int *rlen)
{
	short *bp, *bp1;
	int rt, len;
	int i, j, k;
...
{
	return(BGBMID_LoadWAV_RateStereo16(name, 44100, rlen));
}

short *BGBMID_LoadWAV_16Stereo16(char *name, int *rlen)
{
	return(BGBMID_LoadWAV_RateStereo16(name, 16000, rlen));
}

short *BGBMID_LoadWAV_32Stereo16(char *name, int *rlen)
{
	return(BGBMID_LoadWAV_RateStereo16(name, 32000, rlen));
}
...
int BGBMID_ScaleSampleTrilinearStereo(
	short *d, int dl, short *s, int sl)
{
	...
	tb=bgbmid_malloc(tl*2*sizeof(float));
	tb1=bgbmid_malloc(tl*2*sizeof(float));
...
int BGBMID_ScaleSampleStereo(short *d, int dl, short *s, int sl)
{
	...
	//downsample, trilinear
	BGBMID_ScaleSampleTrilinearStereo(d, dl, s, sl);
	return(0);
}
