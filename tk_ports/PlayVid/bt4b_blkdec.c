...
u32 BTIC4B_DecImageClr1_GDbDr16(int cy, int cu, int cv)
{
	u32 px;
	int cr, cg, cb;
...
u32 BTIC4B_DecImageClr1A_GDbDr16(int cy, int cu, int cv, int ca)
{
	u32 px;
	int cr, cg, cb;
	...
	cr=cr>>3;	cg=cg>>3;	cb=cb>>3; ca=!(ca>>7);
	px=(ca<<15)|(cr<<10)|(cg<<5)|cb;
	return(px);
}
...
//		ctx->DecBlock=BTIC4B_DecBlockRGBX;
	...
	case BTIC4B_CLRS_RGB555:
		ctx->DecBlock=BTIC4B_DecBlockRGB16;
		ctx->DecBlockHalf=BTIC4B_DecBlockHalfRGB16;
		xstr=2;
		sw=3;
		break;
	}
...
//	ctx->clrt=BTIC4B_CLRT_RCT;
	...
	case BTIC4B_CLRT_GDBDR:
		...
		if(sw==3)
		{
			ctx->ClrDecPack1=BTIC4B_DecImageClrPack1_RGB16;
			ctx->ClrDecPack1A=BTIC4B_DecImageClrPack1A_RGB16;
			ctx->ClrDecPack4=BTIC4B_DecImageClrPack4_RGB16;

			ctx->ClrDec1=BTIC4B_DecImageClr1_GDbDr16;
			ctx->ClrDec1A=BTIC4B_DecImageClr1A_GDbDr16;
			ctx->ClrDec2T=BTIC4B_DecImageClrDec2T_GDbDr;
			ctx->ClrDec4=BTIC4B_DecImageClrDec4_GDbDr16;
			ctx->ClrDec4B=BTIC4B_DecImageClrDec4B_GDbDr16;
			ctx->ClrDec4C=BTIC4B_DecImageClrDec4C_GDbDr16;
		}
		else if(sw)
		{
	...
	case BTIC4B_CLRT_RCT:
		...
		if(sw==3)
		{
			...
			ctx->ClrDec1=BTIC4B_DecImageClr1_RCT16;
			ctx->ClrDec1A=BTIC4B_DecImageClr1A_RCT16;
			ctx->ClrDec2T=BTIC4B_DecImageClrDec2T_RCT;
			ctx->ClrDec4=BTIC4B_DecImageClrDec4_RCT16;
			ctx->ClrDec4B=BTIC4B_DecImageClrDec4B_RCT16;
			ctx->ClrDec4C=BTIC4B_DecImageClrDec4C_RCT16;
		}else
			if(sw==1)
		{
