...
static int cram_decompress_frame_clrs(BGBBTJ_VidCodecCTX *ctx,
...
//	memset(dst, 255, info->ohead->biWidth*info->ohead->biHeight*4);
	...
	if(dst && (clrs==BGBBTJ_JPG_RGB555))
	{
		cram_decompress_inner_rgb555(src, dst, ssz, dsz, info);
	}else
		if(dst && (clrs==BGBBTJ_JPG_BGRA))
	{
		cram_decompress_inner_bgra(src, dst, ssz, dsz, info);
	}else if(clrs==BGBBTJ_JPG_RAWCON)
	{
...
static int cram_decompress_frame(BGBBTJ_VidCodecCTX *ctx,
...
static BGBBTJ_VidCodecCTX *cram_begin_decompress(int fcc,
