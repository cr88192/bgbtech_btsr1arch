static int bt5a_decompress_frame_clrs(BGBBTJ_VidCodecCTX *ctx,
...
//	printf("bt5a_decompress_frame_clrs: %p %p %d %d\n", src, dst, ssz, dsz);
...
//		info->ihead->biHeight, clrfl);
	...
	BTPIC0A_DecodeFrame(info->vctx, src, ssz, dst, info->ihead->biWidth);

	return(0);
}

static int bt5a_decompress_frame(BGBBTJ_VidCodecCTX *ctx,
	void *src, void *dst, int ssz, int dsz)
{
	return(bt5a_decompress_frame_clrs(ctx,
		src, dst, ssz, dsz, BGBBTJ_JPG_BGRA));
}

static BGBBTJ_VidCodecCTX *bt5a_begin_decompress(int fcc,
	BGBBTJ_BITMAPINFOHEADER *in, BGBBTJ_BITMAPINFOHEADER *out)
{
	BGBBTJ_VidCodecCTX *ctx;
	BTPIC0A_DecodeContext *btctx;
	cram_ctxinfo *info;
	int err;

	if(in->biCompression!=RIFF_TAG_bt5a)
		return(NULL);

	ctx=BGBBTJ_VidCodecCTX_New();
//	info=bgbrpi_malloc(sizeof(bt5a_ctxinfo));
	info=malloc(sizeof(cram_ctxinfo));
	memset(info, 0, sizeof(cram_ctxinfo));
	ctx->data=info;

	btctx=BTPIC0A_AllocDecodeContext();
	info->vctx=btctx;
...
//	info->ohead->biHeight		= -out->biHeight;
	...
	ctx->decompress_frame=&bt5a_decompress_frame;
	ctx->decompress_frame_clrs=&bt5a_decompress_frame_clrs;

	return(ctx);
}

static void bt5a_shutdown()
{
	int i;
}

int BGBBTJ_CodecBT5A_Init()
{
	...
	codec->begin_decompress=&bt5a_begin_decompress;
