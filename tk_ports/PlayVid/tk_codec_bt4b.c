...
static int bt4b_decompress_frame_clrs(BGBBTJ_VidCodecCTX *ctx,
...
static int bt4b_decompress_frame(BGBBTJ_VidCodecCTX *ctx,
	void *src, void *dst, int ssz, int dsz)
{
	return(bt4b_decompress_frame_clrs(ctx,
		src, dst, ssz, dsz, BGBBTJ_JPG_BGRA));
}

static BGBBTJ_VidCodecCTX *bt4b_begin_decompress(int fcc,
	BGBBTJ_BITMAPINFOHEADER *in, BGBBTJ_BITMAPINFOHEADER *out)
{
	BGBBTJ_VidCodecCTX *ctx;
	BTIC4B_Context *btctx;
	cram_ctxinfo *info;
	int err;

	if(in->biCompression!=RIFF_TAG_bt4b)
		return(NULL);

	ctx=BGBBTJ_VidCodecCTX_New();
//	info=bgbrpi_malloc(sizeof(bt4b_ctxinfo));
	info=malloc(sizeof(cram_ctxinfo));
	memset(info, 0, sizeof(cram_ctxinfo));
	ctx->data=info;

	btctx=BTIC4B_AllocContext();
	info->vctx=btctx;
...
static void bt4b_shutdown()
{
	int i;
}

int BGBBTJ_CodecBT4B_Init()
{
