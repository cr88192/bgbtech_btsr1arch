...
void DrawRottTitle ( void )
{
	...
	if (CheckParm("QUIET") == 0)
		...
		if (CheckParm ("SOUNDSETUP") == 0)
			...
			strcat (title,rt_itoa(ROTTMAJORVERSION,&buf[0],10));
			strcat (title,".");
//MED
//			strcat (title,itoa(ROTTMINORVERSION,&buf[0],10));
			...
			strcat (title,rt_itoa(ROTTMAJORVERSION,&buf[0],10));
			strcat (title,".");
			strcat (title,rt_itoa(ROTTMINORVERSION,&buf[0],10));
...
//		memcpy ((byte *)0xB8000, txtscn, 4000);
		...
		UL_printf ("á");
#else
		UL_printf (rt_itoa(ROTTMAJORVERSION,&buf[0],10));
#endif
		// Skip the dot
		px++;

		UL_printf (rt_itoa(ROTTMINORVERSION,&buf[0],10));
#endif
	}
#endif
...
//#endif
		...
		if ( Keyboard[ sc_M ] )
			...
			rt_itoa(mouse_ry_input_scale,str2,10);
			strcat( str, str2 );
			AddMessage( str, MSG_SYSTEM );
...
void CheckDevelopmentKeys
	...
	if ( Keyboard[ sc_8 ] )
		...
		rt_itoa( locplayerstate->player, str2, 10 );
		strcat( str, str2 );
		AddMessage( str, MSG_SYSTEM );
		}
...
//			memset((byte *)bufferofs+(ylookup[i+SSY])+(SSX>>2),0,46);
		...
		VW_DrawPropString(rt_itoa(ROTTMAJORVERSION,&buf[0],10));
		VW_DrawPropString(".");
		VW_DrawPropString(rt_itoa(ROTTMINORVERSION,&buf[0],10));
		px=SSX+13;
		py=SSY+8;
		VW_DrawPropString(" Episode ");
		VW_DrawPropString(rt_itoa(gamestate.episode,&buf[0],10));
		VW_DrawPropString(" Area ");
		VW_DrawPropString(rt_itoa(GetLevel(gamestate.episode, gamestate.mapon),&buf[0],10));
