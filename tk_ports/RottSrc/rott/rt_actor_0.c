...
//		new->shapenum = new->state->shapenum + new->shapeoffset;
		new->shapenum = new->state->shapenum + (int)(new->shapeoffset);
		new->which = ACTOR;
		if (new->flags & FL_ABP)
			MakeActive(new);
...
//	ob->shapenum = ob->state->shapenum + ob->shapeoffset;
	ob->shapenum = ob->state->shapenum + ((int)ob->shapeoffset);
}
...
void SpawnPatrol (classtype which, int tilex, int tiley, int dir)
{
	...
	if ((temp= UPDATE_STATES[path][(int)(which-lowguardobj)]) != NULL)
	{
		...
		if ((which == wallopobj) || (which == roboguardobj))
		{
			...
			new->dir <<= 1;
			ParseMomentum(new,dirangle16[(new->dir)&15]);
		}
		else
...
objtype		*swiz_objtype_list[8192];
short		swiz_objtype_chn[8192];
short		swiz_objtype_hash[64];
int			swiz_objtype_cnt;

statobj_t	*swiz_statobj_list[8192];
short		swiz_statobj_chn[8192];
	...
	else
	{
		if (!ob->target)
		{
			...
			for (tactor=firstactive;tactor;tactor=tactor->nextactive)
			{
				...
				if (	(tactor->obclass == bladeobj) ||
						(tactor->obclass == NMEsaucerobj))
					continue;
...
//	actrad = ACTORSIZE+0x4500;	//BGB: debug
	ocl = ob->obclass;
...
//		dx = abs(tryx - temp->x);
//		dy = abs(tryy - temp->y);
		dx = rt_abs(tryx - temp->x);
		dy = rt_abs(tryy - temp->y);
...
//			dzt = abs(tryz - (temp->z + 30));
	...
	//		(ocl == tcl))// missiles with same owner
		...
		if(ob->whatever == temp)	//BGB: Debug, Can't shoot with own missile.
			continue;
...
void MoveActor(objtype *ob)
{
	...
	{
//		SetVisiblePosition(ob,ob->x,ob->y);
//		printf("MoveActor: robo(%p) at %d,%d  mom=%d,%d\n",
//			ob, ob->x, ob->y, ob->momentumx, ob->momentumy);
	}
...
//		bestdir = angletodir[GameRandomNumber("collector theta",0) << 3];
		bestdir = angletodir[
			(GameRandomNumber("collector theta",0) << 3) & FINEANGLEMSK];
...
void CheckSteppingB1(
	objtype *ob, statobj_t *step, int minzdiff,
	int *rtryz, int *rdzt)
{
	int cz = (ob->z - step->z + minzdiff);
	if ((cz >= -MAXSTEPHEIGHT) && (cz <= MAXSTEPHEIGHT))
...
//		if(ocl == roboguardobj)			//BGB: debug 2022-12-06
//			continue;
...
movement_status CheckRegularWalls(objtype *ob, int tryx, int tryy, int tryz)
{
	...
	if(tilexhigh<tilexlow)
	{
		tilexlow = (int)(tryx >>TILESHIFT);
		tilexhigh = tilexlow;
	}

	if(tileyhigh<tileylow)
	{
		tileylow = (int)(tryy >>TILESHIFT);
		tileyhigh = tileylow;
	}
...
//					CheckStepping(ob,tempstat,MINSTATZDIFF);
					CheckSteppingB1(ob,tempstat,MINSTATZDIFF, &tryz, &dzt);
				}
...
{
	static movement_status (*reduced_movement_check[3])
		(objtype*,int,int,int)=
	{
		CheckRegularWalls,
		CheckMaskedWalls,
	...
	static movement_status (*complete_movement_check[5])
		(objtype*,int,int,int)=
	{
		CheckOtherActors,
		CheckRegularWalls,
...
//			printf("ActorTryMove: Robo C-2\n");
	...
	switch(ob->obclass)
	{
//	case roboguardobj:		//BGB: Debug 2022-12-06
	...
	for(xyblocked=false,i=0;i<numcheckfunctions;i++)
	{
		...
		else if (movement_check_result == NO_MOVEMENT)
		{
			if(ocl == roboguardobj)
			{
//				printf("ActorTryMove: Robo C-3 %d\n", i);
//				return true;		//BGB: Debug 2022-12-06
			}
...
//	if (/*(ob->state->think != T_Collide) &&*/ (ocl != b_robobossobj) &&
	...
	tryok=ActorTryMove (ob, tryx, tryy, tryz);

#if 0
	if(!tryok)
	{
		if(!ActorTryMove (ob, ob->x, ob->y, ob->z))
		{
			printf("ActorTryMove: Actor Unstick\n");

			for(dotry=0; dotry<16; dotry++)
			{
				tryx = ob->x;
				tryy = ob->y;

				if((tryok&0x3)==1)
					tryx = ob->x - (TILEGLOBAL/2);
				if((tryok&0x3)==2)
					tryx = ob->x + (TILEGLOBAL/2);
				if((tryok&0x3)==3)
					continue;
			...
			if(tryok)
			{
				printf("ActorTryMove: Actor Unstuck\n");
			}
		}
	}
#endif
...
//	if (!ActorTryMove (ob, tryx, tryy, tryz))
	if(!tryok)
	{
		...
		else
		{
			...
			if(ocl == roboguardobj)
			{
//				printf("ActorMovement: Robo B-2 Stuck\n");
				MoveActor(ob);
				return;
			}
#endif
...
void T_EsauWait(objtype*ob)
{
	...
	dist = FindDistance(
//		((int)ob->tilex)-PLAYER[0]->tilex,
//		((int)ob->tiley)-PLAYER[0]->tiley);
		ob->tilex-PLAYER[0]->tilex,
		ob->tiley-PLAYER[0]->tiley);
	MISCVARS->ESAU_SHOOTING = false;
	...
	if (!ob->ticcount)
		...
		if (CheckLine(ob,PLAYER[0],MISSILE))	// got a shot at player?
			...
			if (Near(ob,PLAYER[0],1))
				...
				{
//				dx = rt_abs(PLAYER[0]->tilex-(int)(ob->tilex));
//				dy = rt_abs(PLAYER[0]->tiley-(int)(ob->tiley));
...
// statetype *temp;
	...
	//		return;
		...
		{
//			dx = rt_abs((int)(ob->tilex) - PLAYER[0]->tilex);
//			dy = rt_abs((int)(ob->tiley) - PLAYER[0]->tiley);
...
//	straight = angletodir[angle];
	straight = angletodir[angle&FINEANGLEMSK];
	/*
...
//					ob->targettilex = (angletodir[atan2_appx(dx,dy)]<<1);
					ob->targettilex = (angletodir[
						(atan2_appx(dx,dy))&FINEANGLEMSK]<<1);
...
//		tdir = (((angletodir[angle])<<1) & 0xf);
		tdir = (((angletodir[angle&FINEANGLEMSK])<<1) & 0xf);
		}
	else
		{
		tdir = (ob->temp1 & 0xf);
	...
	ZEROMOM;
	ParseMomentum(ob,dirangle16[(head->dir)&15]);
...
//		new->dir = angletodir[new->angle];
		new->dir = angletodir[(new->angle)&FINEANGLEMSK];
		if (SNAKELEVEL == 1)
			new->speed = 0x5000;
...
//					(int)(ob->tilex)-dummy->tilex,
//					(int)(ob->tiley)-dummy->tiley);
					ob->tilex-dummy->tilex,
					ob->tiley-dummy->tiley);
...
//			dx = abs(ob->tilex - PLAYER[0]->tilex);
//			dy = abs(ob->tiley - PLAYER[0]->tiley);
//			dx = abs((byte)(ob->tilex - PLAYER[0]->tilex));
//			dy = abs((byte)(ob->tiley - PLAYER[0]->tiley));
			dist = (dx>dy)?(dx):(dy);
			if ((!dist) || (dist==1))
					chance = 300;
...
//				ob->dir = angletodir[AngleBetween(ob,PLAYER[0])];
				ob->dir = angletodir[
					(AngleBetween(ob,PLAYER[0]))&FINEANGLEMSK];
				STOPACTOR(ob);
	#if (SHAREWARE == 0)
...
//		shooter->y - FixedMul(offset,sintable[nangle]));
		...
		(shooter->obclass == b_robobossobj))
	{
		...
		new->z -= FixedMulShift(offset,
			sintable[(shooter->yzangle)&FINEANGLEMSK],26);
...
//		destdir = (angletodir[atan2_appx(dx,dy)] << 1);
		destdir = (angletodir[(atan2_appx(dx,dy))&FINEANGLEMSK] << 1);
		ob->temp1 = destdir;
...
{															\
	dx= player->x-ob->x;								\
	dy= ob->y-player->y;								\
	if ((rt_abs(dx) < 0xb000) && (rt_abs(dy) < 0xb000)) \
		return;												\
	dummy.x = player->x;								\
	dummy.y = player->y;								\
}
...
void SelectChaseDir (objtype *ob)
{
	...
	{
		//BGB: debug
		olddir = angletodir[(ob->angle)&FINEANGLEMSK];
		ob->dir = olddir;
	}
...
//	straight = angletodir[atan2_appx(dx,dy)];
	straight = angletodir[(atan2_appx(dx,dy))&FINEANGLEMSK];
//	straight &= 7;
	if(straight&(~7))
		__debugbreak();
...
//	tdir &= 7;
	if(tdir&(~7))
		__debugbreak();
	realdiff = dirdiff[straight][tdir];
	ZEROMOM;
	...
	if ((dtry1!=nodir) || (dtry2!=nodir))
	{
		...
		for(	tdir = dirorder[olddir][whichway];
				tdir != olddir;
				tdir = dirorder[tdir][whichway])
		{
			if (dirtried[tdir])
...
//			(set && (ob->dir == (spot<<1))));
	...
	if ((spot >= 0) && (spot<= 7) && (!done))
	{
		if(ocl == roboguardobj)
		{
			printf("SelectPathDir: Robo B-0-0  Spot=%d\n", spot);
		}
	...
	// new direction
		{
			if(ocl == roboguardobj)
			{
				printf("SelectPathDir: Robo B-0-1  Spot=%d\n", spot);
			}
...
//			ob->tilex,ob->tiley);
	...
	if (NOMOM)
	{
		if(ocl == roboguardobj)
		{
//			printf("SelectPathDir: Robo B-1  dir=%d\n", ob->dir);
		}
		...
		{
			printf("SelectPathDir: Robo B-2\n");
		}
	}

	if(ocl == roboguardobj)
	{
//		printf("SelectPathDir: Robo A\n");
	}
...
//	vx = costable[angle];
	...
	zincr[0]=-FixedMulShift(sintable[yzangle&FINEANGLEMSK],abs(vx),26);
	zincr[1]=-FixedMulShift(sintable[yzangle&FINEANGLEMSK],abs(vy),26);
