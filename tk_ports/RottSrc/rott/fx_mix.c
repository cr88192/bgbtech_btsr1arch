...
int FX_CheckExpandChan(int chn, int len)
{
	...
	if(fx_chan[chn]->magic!=0x12345678)
	{
	...
	if(!fx_chan[chn]->data)
	{
		fx_chan[chn]->magic=0x12345678;
		fx_chan[chn]->data=malloc(len+32);
...
//	printf("FX_PlayVOC3D %X prio=%d\n", callbackval, priority);
	...
	if(chn<=0)
	{
		printf("FX_PlayVOC3D: No Chan, %lX prio=%d\n", callbackval, priority);
		return(0);
	}
...
//	printf("FX_PlayWAV3D %X prio=%d\n", callbackval, priority);
	...
	if(chn<=0)
	{
		printf("FX_PlayWAV3D: No Chan, %lX prio=%d\n", callbackval, priority);
		return(0);
	}
