...
static int oldcontrolbuf[SZCONTROLBUF];
...
void InitializeGameCommands( void )
{
	...
	CommandState[0]=(CommandStatusType *)
		SafeLevelMalloc(sizeof(CommandStatusType));
...
//	oldpolltime=controlupdatetime+1;
...
//			while (ticcount<(controlupdatetime-10))
			while (oldtime<(controlupdatetime-10))
			{
				CalcTics();
//				I_PollTimer();
			}
...
//			(memcmp(&controlbuf[0],&oldcontrolbuf[0],sizeof(controlbuf))!=0) ||
			...
			(memcmp(controlbuf,oldcontrolbuf,sizeof(controlbuf))!=0) ||

//			(memcmp(controlbuf,oldcontrolbuf,3*sizeof(int))!=0) ||

//				(controlbuf[0]!=oldcontrolbuf[0]) ||
//				(controlbuf[1]!=oldcontrolbuf[1]) ||
//				(controlbuf[2]!=oldcontrolbuf[2]) ||
...
//				memcpy(&oldcontrolbuf[0],&controlbuf[0],sizeof(controlbuf));
				memcpy(oldcontrolbuf,controlbuf,sizeof(controlbuf));
//				memcpy(oldcontrolbuf,controlbuf,3*sizeof(int));
			...
			else
			{
				...
				{
#if 0
					tk_printf("UpdateClientControls A0 %X %X %X  Cmd=%p\n",
						controlbuf[0]>>1,
						controlbuf[1]>>1,
						controlbuf[2]>>11,
						Delta);
...
//			ComError("CheckForPacket: from=%ld\n",rottcom->remotenode);
		...
		{
			RequestPacket (
				LastCommandTime[rottcom->remotenode]+controldivisor,
				rottcom->remotenode, controldivisor);
		}
	}
	waminot();
}
...
//	pkt->time=lastcontrolupdatetime;
...
int GetPacketSize (void * pkt)
{
	...
	case COM_SOUNDANDDELTA:
		...
		break;
	default:
		Error("Unhandled packet type in GetPacketSize type=%ld",
			((MoveType *)pkt)->type);
		break;
	}
...
void FixupPacket (void * pkt, int src)
{
	...
	for (i=0;i<fix->numpackets;i++,time+=controldivisor)
	{
		...
		if (IsServer==true)
		{
			...
			{
				ComError("Server Received fixup with no bad packet "
					"time=%ld from %ld\n",time,src);
			}
			else
			{
		...
		else
		{
			...
			{
				ComError("Client Received fixup with no bad packet "
					"time=%ld from %ld\n",time,src);
			}
			else
			{
...
void SendSyncTimePacket ( void )
{
	...
	if (networkgame==true)
	{
		...
		{
			if ((PlayerStatus[i]!=player_ingame) ||
				( (i==consoleplayer) && (standalone==false) ) )
					continue;
...
//	{
		if(controldivisor!=1)
		{
			diff = (ticcount-controldivisor-LastCommandTime[server])/
				controldivisor;
		}
		else
		{
			diff = ticcount-1-LastCommandTime[server];
		}
		if (rt_abs(diff)>0)
			ISR_SetTime(ticcount-diff);
//	}
	...
	if (!((src==server) && (standalone==false) && (IsServer==true)))
	{
		...
		if (packet->time != LastCommandTime[src])
		{
				...
				)
			{
				...
				ComError("AddPacket: Request packet "
					"time=%ld lct=%ld numpackets=%ld\n",
...
void SendFullServerPacket ( void )
{
	...
	for (i=0;i<numplayers;i++)
	{
		...
		memcpy(pkt, ClientCommand(i, timeindex), size);
		pkt+=size;
...
//		I_PollTimer();
		...
		while (done == false)
		{
			...
			if ( (standalone==false) &&
				(serverupdatetime>=(controlupdatetime-controldivisor)) &&
				(done==false) )
					break;
...
//				}
		...
		if (exit==true)
			break;
		if ( (serverupdatetime>=(controlupdatetime-controldivisor)) &&
				(standalone==false))
			break;
		if (restartgame==true)
			break;
...
void ProcessPlayerCommand( int player )
{
	...
	else if (cmd->type==COM_QUIT)
	{
		...
		else
		{
			...
			strcat(str,rt_itoa(player+1,str2,10));
			strcat(str,", ");
...
//		return;
	...
	memcpy (pstate->buttonheld, pstate->buttonstate,
		sizeof(pstate->buttonstate));
...
//				break;	//BGB: debug
				...
				if (networkgame==true)
//					savetime=ticcount+NETWORKTIMEOUT;
					savetime=oldtime+NETWORKTIMEOUT;
				else
					savetime=oldtime+MODEMTIMEOUT;
			}
		}
	}
	...
	if (!(ob->flags&FL_DYING))
	{
		if (ob->flags&FL_PUSHED)
		{
			ob->flags&=~FL_PUSHED;
#if 0
			if (rt_abs(ob->momentumx)>0)
			{
				if (rt_abs(ob->momentumx+pstate->dmomx)>=rt_abs(ob->momentumx))
				{
					ob->momentumx += pstate->dmomx;
					ob->momentumy += pstate->dmomy;
				}
			}
			else if (rt_abs(ob->momentumy+pstate->dmomy)>=
				rt_abs(ob->momentumy))
			{
				ob->momentumx += pstate->dmomx;
				ob->momentumy += pstate->dmomy;
			...
			if (rt_abs(ob->momentumy+pstate->dmomy)>=rt_abs(ob->momentumy))
			{
				ob->momentumy += pstate->dmomy;
			}
		}
...
{
	if((pstate->player<0) || (pstate->player>5))
		{ __debugbreak(); }
...
//	MoveCmd=(MoveType *)PlayerCommand(player,CommandAddress(oldpolltime+1));
//	MoveCmd=(MoveType *)PlayerCommand(player,CommandAddress(polltime));
...
//	pstate=&PLAYERSTATE[player];
	...
	{
#if 0
		tk_printf("UpdatePlayerObj: %X %X %X  Cmd=%p\n",
			MoveCmd->momx,
			MoveCmd->momy,
			MoveCmd->dangle,
			MoveCmd);
...
//	pstate->dmomx = ((int)((short)(MoveCmd->momx)))<<1;
//	pstate->dmomy = ((int)((short)(MoveCmd->momy)))<<1;
//	pstate->angle = (MoveCmd->dangle)&65535;
//	pstate->angle = (MoveCmd->dangle)&FINEANGLEMSK;
...
//	__debugbreak();
	...
	if (demoplayback||demorecord)
		SoftError("  dmx=%4lx dmy=%4lx da=%4lx time=%5ld\n",
			pstate->dmomx,pstate->dmomy,
			pstate->angle>>11,oldpolltime);
...
void LoadDemo (int demonumber)
{
	...
	memset(locplayerstate, 0, sizeof(playertype));	//BGB: debug
...
// #ifdef __BJX2__
	...
	while(*s!='.')s--;
	strcpy(s, "_log.txt");
//	demoposlog=fopen(demo, "rt");
	demoposlog=fopen(demo, "rb");
	if(!demoposlog)
		printf("Fail open %s\n", demo);
#endif
}
...
void SetupDemo ( void )
{
	...
	printf("SetupDemo: sizeof(gamestate)==%d\n", (int)(sizeof(gamestate)));

	DemoHeader=(DemoHeaderType *)demoptr;
	demoptr+=sizeof(gamestate);
	if (gamestate.violence!=DemoHeader->demostate.violence)
	{
		Error ("This demo has a different difficulty "
			"level than your current settings\n");
	}
...
void AddDemoCmd (void)
{
	char tbuf[256];
	DemoType * dtime;
	int px, py, pa, pmx, pmy, pmh, dmx, dmy;
	int dtmx, dtmy, dtma, dtmb, dtmt;
...
//		controlbuf[0]=dtime->momx<<1;
		controlbuf[0]=((int)(dtime->momx))<<1;
//		controlbuf[1]=dtime->momy<<1;
		controlbuf[1]=((int)(dtime->momy))<<1;
//		controlbuf[2]=dtime->dangle<<11;
		controlbuf[2]=((int)(dtime->dangle))<<11;
		buttonbits	=dtime->buttons;
		demoptr+=sizeof(DemoType);
		
		if((controlbuf[0]>>1)!=dtime->momx)
			__debugbreak();
		if((controlbuf[1]>>1)!=dtime->momy)
			__debugbreak();
		if((controlbuf[2]>>11)!=dtime->dangle)
			__debugbreak();

#if 0			
		tk_printf("AddDemoCmd: Control Buf %X %X %X dt=%d\n",
			(controlbuf[0]>>1),
			(controlbuf[1]>>1),
			(controlbuf[2]>>11),
			dtime->time);
...
//		__debugbreak();
		...
		if(demoposlog)
		{
			fprintf(demoposlog, "XY %d %d %d  %d %d  %d %d %d "
					" %d %d %d %d %d\n",
				player->x, player->y, player->angle,
				player->momentumx, player->momentumy,
				locplayerstate->health,
				locplayerstate->dmomx,		locplayerstate->dmomy,
				dtime->momx,	dtime->momy,
				dtime->dangle,	dtime->buttons,
				dtime->time);
		}
#endif
		...
		if(demoposlog)
		{
			dmx=0;	dmy=0; pmx=0; pmy=0; pmh=0;
			px=0; py=0; pa=0;
			
			dtmx=0;	dtmy=0;	dtma=0;	dtmb=0;	dtmt=0;
		
			memset(tbuf, 0, 16);
			fgets(tbuf, 255, demoposlog);
...
//			puts(tbuf);
			...
			sscanf(tbuf, "XY %d %d %d  %d %d  %d %d %d  %d %d %d %d %d\n",
				&px, &py, &pa,  &pmx, &pmy,  &pmh, &dmx, &dmy,
				&dtmx, &dtmy, &dtma, &dtmb, &dtmt);
			
			if((dtmx!=dtime->momx) || (dtmy!=dtime->momy) ||
				(dtma!=dtime->dangle) ||
				(dtmt!=dtime->time))
			{
				printf("DT Mismatch, (%d %d %d %d %d) (%d %d %d %d %d)\n",
					dtmx, dtmy, dtma, dtmb, dtmt,
					dtime->momx,	dtime->momy,
					dtime->dangle,	dtime->buttons,
					dtime->time);
			...
			if((px!=player->x) || (py!=player->y) || (pa!=player->angle))
			{
				printf("Demo-Mismatch dx=%d dy=%d da=%d\n",
					px-player->x,
					py-player->y,
					pa-player->angle);
...
//				__debugbreak();
			...
			if((pmx!=player->momentumx) || (pmy!=player->momentumy))
			{
				printf("Demo-Mismatch dmx=%d dmy=%d\n",
					pmx-player->momentumx,
					pmy-player->momentumy);

//				__debugbreak();
				player->momentumx=pmx;
				player->momentumy=pmy;
			}

			if(locplayerstate->health!=pmh)
			{
				locplayerstate->health=pmh;
				player->hitpoints=pmh;
				if(pmh>0)
				{
					player->flags &= ~(FL_DYING | FL_HBM);
				}
			}

#if 1
			if((dmx!=locplayerstate->dmomx) || (dmy!=locplayerstate->dmomy))
			{
				printf("Demo-Mismatch dmox=%d (%X %X) dmoy=%d  (%X %X)\n",
					(dmx-locplayerstate->dmomx)>>1,
					dmx>>1, locplayerstate->dmomx>>1,
					(dmy-locplayerstate->dmomy)>>1,
					dmy>>1, locplayerstate->dmomy>>1);

//				__debugbreak();
				locplayerstate->dmomx=dmx;
				locplayerstate->dmomy=dmy;
			}
#endif
		}
#endif
...
void UpdateDemoPlayback (int curtime)
{
	if (demoplayback)
	{
		if (GetNextDemoTime()==curtime)
			AddDemoCmd();
	}
}
