...
ROTTCHARS characters[10]={
	...
	{0x2200,0x5200,150,3,32}	// BJ Blaz
	};
...
//	ob->y = (((long)tiley)<<TILESHIFT)+(TILEGLOBAL/2);
	ob->x = (((int)tilex)<<TILESHIFT)+(TILEGLOBAL/2);
	ob->y = (((int)tiley)<<TILESHIFT)+(TILEGLOBAL/2);
...
void  SpawnBlood(objtype * ob, int angle)
{
	...
	SpawnInertActor(ob->x-(costable[angle&FINEANGLEMSK]>>5),
						ob->y+(sintable[angle&FINEANGLEMSK]>>5),ob->z);
...
void  SpawnMetalSparks(objtype * ob, int angle)
{
	...
	SpawnInertActor(ob->x-(costable[angle&FINEANGLEMSK]>>3)+dispx,
						ob->y+(sintable[angle&FINEANGLEMSK]>>3)+dispy,ob->z);
...
void MissileAutoTarget(objtype *ob,missile_stats *mdata)
	...
	for(temp = firstactive;temp;temp=temp->nextactive)
		...
		magangle = rt_abs(ob->angle - angle);
		if (magangle > VANG180)
			...
			for(i=0;i<4;i++)
			{
					...
					new->y+new->momentumy,new->z))
				{
					new->x = new->drawx = ob->x +
						(costable[(new->angle)&FINEANGLEMSK]>>3);
					new->y = new->drawy = ob->y -
						(sintable[(new->angle)&FINEANGLEMSK]>>3);

					ob->momentumx = -FixedMul(0x5000l,
						costable[(ob->angle)&FINEANGLEMSK]);
					ob->momentumy = FixedMul(0x5000l,
						sintable[(ob->angle)&FINEANGLEMSK]);
				}

				SpawnMissile(ob,newmissiledata->obclass,
					newmissiledata->speed,ob->angle,
					newmissiledata->state,newmissiledata->offset);
...
void DogAttack(objtype*ob)
{
	...
	SD_PlaySoundRTP(
		SD_DOGMODEBITE1SND+(RandomNumber("DogAttack",0)>>7),
		ob->x,ob->y);
	for(temp=firstareaactor[ob->areanumber];temp;temp=temp->nextinarea)
	{
		...
		dx = rt_abs(temp->x - ob->x);
		if (dx > 0xc000)
			continue;

		dy = rt_abs(temp->y - ob->y);
		if (dy > 0xc000)
			continue;

		dz = rt_abs(temp->z - ob->z);
		if (dz > (0xc000>>10))
			continue;
...
void BatAttack(objtype*ob)
{
	...
	magangle = rt_abs(ob->angle - AngleBetween(ob,temp));
	if (magangle > VANG180)
		magangle = ANGLES - magangle;
	...
	{momx = FixedMul(0x3000l,costable[angle&FINEANGLEMSK]);
		momy = -FixedMul(0x3000l,sintable[angle&FINEANGLEMSK]);
		if (levelheight > 2)
	...
	else // find target to hit grenade back at
	{
		...
		else if (rand < 160) // hit back at first eligible
		{
			...
			for(temp2 = firstareaactor[ob->areanumber];temp2;temp2 = temp2->nextinarea)
				{magangle = rt_abs(ob->angle-AngleBetween(ob,temp2));
				if (magangle > VANG180)
					magangle = ANGLES - magangle;
	...
	dx = rt_abs(tstat->x - ob->x);
	dy = rt_abs(tstat->y - ob->y);
	dz = rt_abs(tstat->z - ob->z);

	if ((dx > 0xc000) || (dy > 0xc000) || (dz > 20))
	continue;

	magangle = rt_abs(ob->angle - AngleBetween(ob,(objtype*)tstat));
	if (magangle > VANG180)
		magangle = ANGLES - magangle;
...
void PollMouseMove (void)
{
	...
	if (rt_abs(mousexmove)>rt_abs(mouseymove))
		mouseymove/=2;
	else
		mousexmove/=2;
	MX = 0;
...
//				( DoubleClickCount[ i ] != 2 ) )
	...
	if ((rt_abs(SWIFTStatus.x)) > CYBERDEADRANGE)
	{
		CX = -(SGN (SWIFTStatus.x) * (( (rt_abs(SWIFTStatus.x)-CYBERDEADRANGE) ) << 10));
		turnheldtime += tics;
	}
	else
	...
	if ((rt_abs(SWIFTStatus.y)) > CYBERDEADRANGE)
	{
		CY = SWIFTStatus.y >> 2;
	}
	else
...
//				( DoubleClickCount[ i ] != 2 ) )
	...
	if ( rt_abs(SWIFTStatus.pitch) < (20<<6) )
		{
		SWIFTStatus.pitch = 0;
		}
	else
	...
	if ( rt_abs(SWIFTStatus.pitch) > (60<<6) )
		{
		buttonpoll[bt_run] = 1;
		}

	if ( rt_abs(SWIFTStatus.roll) > (80<<6) )
		{
		buttonpoll[bt_run] = 1;
		}


	if ( rt_abs(SWIFTStatus.roll) < (20<<6) )
		{
		SWIFTStatus.roll = 0;
		}
	else
	...
	controlbuf[0] += -(FixedMulShift (SWIFTStatus.pitch, viewcos,16))+
						FixedMulShift (-SWIFTStatus.roll,
							costable[strafeAngle&FINEANGLEMSK], 16);

	controlbuf[1] +=  FixedMulShift (SWIFTStatus.pitch, viewsin,16) -
						FixedMulShift (-SWIFTStatus.roll,
							sintable[strafeAngle&FINEANGLEMSK], 16);

	yaw = rt_abs(SWIFTStatus.yaw);
	acc = 0;
...
keys:
	...
	case stat_oneup:
		if (rt_abs(pstate->health - MaxHitpointsForCharacter(pstate))
...
void Thrust ( objtype * ob )
{
	...
	if (index && (rt_abs(ob->z - nominalheight) < 5))
	{
//		if (!TRIGGER[index-1])
	...
	if (tstat)
	{
		dx = rt_abs(ob->x - tstat->x);
		dy = rt_abs(ob->y - tstat->y);
		dz = rt_abs(ob->z - tstat->z);
...
#define WHERE_PLAYER_DROPPED_WEAPON( ob, pstate ) ( ( ( ob )->tilex == ( pstate )->weaponx ) && ( ( ob )->tiley == ( pstate )->weapony ) )
			...
			)
		{
			...
			(tstat->itemnumber == stat_heatgrate) &&
...
//================ Check special player/actor links ======================
	...
	if (ob->whatever)
	{
		...
		dx = rt_abs(ob->x - temp->x);
		dy = rt_abs(ob->y - temp->y);
		dz = rt_abs(ob->z - temp->z);
...
//		if (ob->momentumx>HITWALLSPEED)
		if (rt_abs(ob->momentumx)>HITWALLSPEED)
...
//		if (ob->momentumy>HITWALLSPEED)
		if (rt_abs(ob->momentumy)>HITWALLSPEED)
			SD_PlaySoundRTP(SD_HITWALLSND,ob->x,ob->y);
		ob->momentumy=0;
	}
	else
...
void PlayerTiltHead (objtype * ob)
{
	...
	if ((yzangle!=pstate->horizon) && (dyz==0))
	{
		...
		if (yzangle<pstate->horizon)
			...
			yzangle-=speed;
		if ((rt_abs(yzangle-pstate->horizon))<SNAPBACKSPEED)
...
//	ob->dir = angletodir[ob->angle];
	...
	if (IsWindow(ob->tilex,ob->tiley) && !player_noclip)
	{
...
void  T_Tag (objtype *ob)
{
	...
	if ((ob->ticcount > 1) && (ob->ticcount < 8) && (!(ob->flags & FL_DIDTAG)))
	{
		...
		for(i=0;i<numplayers;i++)
		{
			...
			dx = rt_abs(PLAYER[i]->x - ob->x);
			dy = rt_abs(PLAYER[i]->y - ob->y);
			if ((dx > 0xc000) || (dy > 0xc000))
				continue;
			magangle = rt_abs(ob->angle - AngleBetween(ob,PLAYER[i]));
			if (magangle > VANG180)
				magangle = ANGLES - magangle;
