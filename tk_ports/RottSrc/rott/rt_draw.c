...
void TransformPoint (int x, int y,
	int * screenx, int * height, int * texture, int vertical)
{
	fixed gxt,gyt,nx,ny;
	fixed gxtt,gytt;
	int gx,gy;
...
boolean TransformSimplePoint (int x, int y,
	int * screenx, int * height, int * texture, int vertical)
{
	fixed gxt,gyt,nx,ny;
	fixed gxtt,gytt;
...
boolean TransformPlane (int x1, int y1, int x2, int y2, visobj_t * plane)
{
	...
	if (result1==true)
	{
		...
		if (result2==false)
		{
			...
			TransformPoint(x2,y2,&(plane->x2),&(plane->h2),
				&(plane->textureend),vertical);
		}
	}
	else
...
//		angle =  (viewangle-ANG180)- dirangle16[ob->dir];
		angle =  (viewangle-ANG180)- dirangle16[(ob->dir)&15];
	else if(ob->dir==8)
		angle =  (viewangle-ANG180)- dirangle8[8];
	else
//		angle =  (viewangle-ANG180)- dirangle8[ob->dir];
		angle =  (viewangle-ANG180)- dirangle8[(ob->dir)&7];
...
//		memset((byte *)bufferofs+(ylookup[i+PLY])+(PLX>>2),0,6);
	...
	px=PLX;
	py=PLY;
	VW_DrawPropString(w_strupr(rt_itoa(player->x,&buf[0],16)));
	px=PLX;
	py=PLY+6;
	VW_DrawPropString(w_strupr(rt_itoa(player->y,&buf[0],16)));
	px=PLX;
	py=PLY+12;
	VW_DrawPropString(w_strupr(rt_itoa(player->angle,&buf[0],16)));
}
...
//				DrawSkyPost((byte *)bufferofs + (x>>2) + ylookup[ylow],src,postheight);
				DrawSkyPost(bufferofs + (x>>2) + ylookup[ylow],
					src, postheight);
			}
		FlipPage();
		CalcTics();
...
//			*((byte *)bufferofs+ylookup[y]+(x>>2))=*tmp++;
			VGAWRITE(bufferofs+ylookup[y]+(x>>2), *tmp++);
		}
	}
	tmp=sky;
	for (x=256;x<320;x++)
