...
//				k=tkra_morton8(x, y)+img->tex_mipofs[mip];
				k=tkra_morton16(x, y)+img->tex_mipofs[mip];
				pix=buf[j];
				img->tex_img[k]=pix;
				if(pix&0x8000)
...
static u64 *tkra_btc_tmpbuf=NULL;
static int tkra_btc_xsz_tmpbuf;
...
//	if(xs>256)
	...
	if(xs1>tkra_btc_xsz_tmpbuf)
	{
		tkra_free(tkra_btc_tmpbuf);
		tkra_btc_tmpbuf=NULL;
	}
...
//		tkra_btc_tmpbuf=tkra_malloc(64*64*8);
//		tkra_btc_tmpbuf=tkra_malloc(256*256*8);
//		tkra_btc_xsz_tmpbuf=256;
		tkra_btc_tmpbuf=tkra_malloc(ys1*ys1*8);
		tkra_btc_xsz_tmpbuf=ys1;
	}
...
//			z=tkra_morton8(x, y);
	...
	TKRA_UpdateTexImgUtx2(img, tkra_btc_tmpbuf, xs, ys, mip);
}
