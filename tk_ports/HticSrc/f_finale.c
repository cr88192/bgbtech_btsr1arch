...
#if 0
void F_MemCpy_ScrPix(dt_scrpix *dst, byte *src, int count)
{
	while (count--)
	{
		*dst++ = colormaps[*src++];
...
//			memcpy (dest, src+((y&63)<<6), SCREENWIDTH&63);
			V_MemCpy_ScrPix (dest, src+((y&63)<<6), SCREENWIDTH&63);
			dest += (SCREENWIDTH&63);
		}
	}
...
void F_DemonScroll(void)
{
	...
	if(finalecount < 70)
	{
//		memcpy(screen, p1, SCREENHEIGHT*SCREENWIDTH*sizeof(dt_scrpix));
		V_MemCpy_ScrPix(screen, p1, SCREENHEIGHT*SCREENWIDTH);
...
//		memcpy(screen, p2+SCREENHEIGHT*SCREENWIDTH-yval, yval);
		V_MemCpy_ScrPix(screen, p2+SCREENHEIGHT*SCREENWIDTH-yval, yval);
//		memcpy(screen+yval, p1, SCREENHEIGHT*SCREENWIDTH-yval);
		V_MemCpy_ScrPix(screen+yval, p1, SCREENHEIGHT*SCREENWIDTH-yval);
		yval += SCREENWIDTH;
...
//		memcpy(screen, p2, SCREENWIDTH*SCREENHEIGHT);
		V_MemCpy_ScrPix(screen, p2, SCREENWIDTH*SCREENHEIGHT);
	}
}
...
//				memcpy(screen, W_CacheLumpName("E2END", PU_CACHE),
				V_MemCpy_ScrPix(screen, W_CacheLumpName("E2END", PU_CACHE),
