...
void V_MemCpy_ScrPix(dt_scrpix *dest, byte *source, int count)
{
#if 1
	register dt_scrpix *dst;
	register byte *src;
//	register dt_scrpix *dste;
	dt_scrpix *dste;
	register lighttable_t *cmap;
...
//	register int i0, i1;
	...
	while(dst<dste)
	{
		...
		i2=src[2];		i3=src[3];
		src+=4;
		i0=cmap[i0];	i1=cmap[i1];
		i2=cmap[i2];	i3=cmap[i3];
		dst[0]=i0;		dst[1]=i1;
		dst[2]=i2;		dst[3]=i3;
		dst+=4;
#endif
