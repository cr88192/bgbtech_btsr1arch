...
//			+ sizeof(texpatch_t)*(SHORT(mtexture->patchcount)-1), PU_STATIC,
			+ sizeof(texpatch_t)*SHORT(mtexture->patchcount), PU_STATIC,
			0);
...
//	W_ReadLump (lump,colormaps);
	...
	colormaps = Z_Malloc (length*sizeof(dt_scrpix), PU_STATIC, 0);
	colormaps = (dt_scrpix *)( ((nlint)colormaps + 511)&(~511));
	
	t_clrmap = Z_Malloc (length, PU_STATIC, 0);

	W_ReadLump (lump, t_clrmap);

	for(i=0; i<baselen; i++)
		{ colormaps[i] = d_8to16table[t_clrmap[i]]; }
