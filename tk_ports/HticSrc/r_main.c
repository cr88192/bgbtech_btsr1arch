...
//	focallength = FixedDiv (centerxfrac
	focallength = FixedDivSoft (centerxfrac
...
//			scale = FixedDiv ((SCREENWIDTH/2*FRACUNIT), (j+1)<<LIGHTZSHIFT);
			scale = FixedDivSoft ((SCREENWIDTH/2*FRACUNIT), (j+1)<<LIGHTZSHIFT);
...
//		yslope[i] = FixedDiv ( (viewwidth<<detailshift)/2*FRACUNIT, dy);
		yslope[i] = FixedDivSoft ( (viewwidth<<detailshift)/2*FRACUNIT, dy);
	}
...
//		distscale[i] = FixedDiv (FRACUNIT,cosadj);
		distscale[i] = FixedDivSoft (FRACUNIT,cosadj);
	}
